...
- 두 개 이상의 작업을 진행할 땐 서브 에이전트를 할당해서 진행
- PR을 올리고 난 뒤에는 코드 리뷰에 대응해서 approved까지 반복 후 머지
- 코드 리뷰가 동작하지 않는 경우, 자체적으로 코드 리뷰를 에이전트를 할당해서 코드 리뷰 진행
- 코드 작업을 진행하기 전, 어떻게 진행할 지를 쉽게 설명된 시각화 자료를 html로 제공, 이후 사용자 승인을 받고 코드 작업 진행
