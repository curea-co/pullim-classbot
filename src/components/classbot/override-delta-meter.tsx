  ...
  return (
    <section
      className={cn(
        ...
        isOver ? 'border-pullim-warn/40 bg-pullim-warn-bg' : 'bg-card',
      )}
    >
      <header className="mb-2 flex items-center gap-2">
        <TrendingUp className={cn('h-4 w-4', isOver ? 'text-pullim-warn' : 'text-pullim-blue-600')} />
        ...
        <div className="text-right">
          <div className={cn('font-mono text-lg font-bold', isOver ? 'text-pullim-warn' : 'text-pullim-slate-900')}>
            {cumulative}%
          </div>
      ...
      <div className="bg-pullim-slate-200 h-1.5 overflow-hidden rounded-full">
        <div
          className={cn('h-full rounded-full', isOver ? 'bg-pullim-warn' : 'bg-pullim-blue-500')}
          style={{ width: `${pct}%` }}
        />
      </div>
      ...
      {currentDelta > 0 && (
        <p className="text-pullim-slate-500 mt-2 text-[11px]">
          ...
          {currentDelta >= threshold && <span className="text-pullim-warn font-bold"> · 단독으로도 임계 초과</span>}
        </p>
      )}
      ...
      {isOver && (
        <button
          type="button"
          className="bg-pullim-warn hover:bg-pullim-warn/90 mt-3 w-full rounded-lg py-1.5 text-[11px] font-bold text-white"
        >
          루브릭 재학습 제안 보기
        </button>
      )}
