...
export function WellbeingGauge({ studentId, compact }: { studentId: string; compact?: boolean }) {
  ...
  const tone =
    score >= 80 ? { color: 'bg-pullim-success',  text: 'text-pullim-success',  label: '좋아요' }
    : score >= 60 ? { color: 'bg-pullim-blue-500', text: 'text-pullim-blue-600', label: '괜찮아요' }
    : score >= 40 ? { color: 'bg-pullim-warn',     text: 'text-pullim-warn',     label: '신경 써요' }
  ...
  return (
    <section className="bg-card rounded-2xl border p-4">
      ...
      <div className="mt-3">
        ...
        <div className="flex h-12 items-end gap-1">
          {trend.map((t) => {
            ...
            const c =
              t.score >= 80 ? 'bg-pullim-success'
              : t.score >= 60 ? 'bg-pullim-blue-500'
              : t.score >= 40 ? 'bg-pullim-warn'
              : 'bg-pullim-danger';
            return (
              <div
                key={t.daysAgo}
      ...
      {today.flag && (
        <div className="border-pullim-warn/30 bg-pullim-warn-bg mt-3 rounded-lg border p-2">
          <p className="text-pullim-warn text-[11px] font-bold">
