...
export interface AuthUser {
   ...
   *
   * ⛔ **본인-조회 한정 PII 를 담게 될 칸이다 — 넷을 지킨다.** 이 앱의 FE 가 이 칸을 채울 값은
   * pullim-api `GET /me` 의 `name` 이고(그 배선은 이 칸에 기대는 **FE PR 이 세운다**), 그 응답
   * DTO 가 「KCB 실명(users.name — AES-256-GCM 복호화 PII). **본인-조회 한정 · 로그/토큰 금지.**」
   * 라고 못박아 두었다. 그러니 이 칸을 읽는 쪽은
   *  ① **로그에 남기지 않는다** — `console.*` 도, 에러 메시지에 끼워 넣는 것도 금지.
   *  ② **서버로 되보내지 않는다** — 요청 명의는 세션 쿠키가 진다. 토큰에도 싣지 않는다.
   *  ③ **저장하지 않는다** — localStorage·sessionStorage·쿠키 어디에도. persist 되는 상태에
   *    넣으면 **로그아웃해도 남는다** — 공용 PC 라면 다음 사람이 읽는다. 이름이 필요한 자리는
   *    저장은 id·템플릿으로 하고 이름은 그릴 때 세션에서 읽는다.
   *  ④ **본인 화면 전용이다.**
   *
   ...
   * FE 가 세울 순서는 `/me` 의 `name`(KCB 실명) → 비면 같은 응답의 `displayName` 인데,
   * 그 둘의 실제 값이 이렇다:
