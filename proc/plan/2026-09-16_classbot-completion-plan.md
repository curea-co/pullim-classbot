...
- [ ] **PR 8** · 정리 — B 세계 은퇴(겹치는 `app/api/**` · Drizzle 표 · 시드 · 개발 쿠키 경로 · `lib/mock` 반·봇·챗·과제 · 스토어 persist) ← PR 5b · 5c · 5d · 6 · 7 *(`[2026-09-17 정정]` — 종전 「PR 5 · 6 · 7」 · 5d 는 밤에 더함)* · **맨 마지막** — **#358 열림**(`refactor/classbot-retire-world-b` · base `dev` · **세션 리뷰 Approve** · 이 문서 PR 이 먼저 머지된 뒤에 머지한다 — 은퇴 범위 정정이 base 스펙에 들어가야 리뷰가 같은 사실을 본다). 실물 은퇴 범위: 라우트 10개 삭제(`/api/assignments` · `/api/assignments/[id]` · `/api/bots` · `/api/chat` · `/api/enrollments` · `/api/grades` · `/api/me/classrooms` · `/api/teacher/assignments` · `/api/teacher/assignments/[id]` · `/api/wellness`) · 표 넷 DROP(`0009_retire_world_b_tables`) · Drizzle 31표 → 27표
...
- [ ] pullim-api **PR 4 = #672** 리뷰·머지(소유자) — `feat/classbot-class-and-bot-reads` · base 그 리포 `dev` · 문서 `0f08bdb0` + 코드 `6dcac2ba` · **열림 · 머지 대기**(2026-09-17 실측) · 스키마 무변경이라 RDS 작업 없음 · FE 5d 의 앞
...
PR 0✓ ─┬─→ PR 1✓ ─┬─→ PR 2✓ ─┬─→ PR 5b✓ → PR 5c✓ ┐     ✓ = dev 머지(2026-09-17 밤 실측) · PR 0✓ 는 이 리포 #349 와 pullim-api #667 둘 다
       ...
       └─→ PR 4-ci✓ ─→ PR 4✓ ─┬─→ PR 5a✓         │     PR 7✓ = #354(21:16) · 5b✓ = #355(21:36) · 5c✓ = #357(22:28) · 열린 PR 둘 — #358 · #359(이 문서)
                              └─→ PR 6✓ ─────────┘
```
