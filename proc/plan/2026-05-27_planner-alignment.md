...
**패배 사례** (codex R1~R20 누적 지적 흡수):
- **`apps/classbot/` 편집은 단일 도메인 허용 (R20 — 과잉 게이트 철회)** — base `CLAUDE.md`:90 이 `apps/classbot/` 페이지·컴포넌트·mock·lib 수정/신규 허용. 교사 라우트 포함 별도 게이트 불필요. R8·R12 게이트는 과잉 해석.
...
- **교사 라우트 편집 — authority 충돌, 사용자 확인** (R20·R21 — 양쪽 단정 모두 철회) — base `CLAUDE.md` 가 자기 충돌: `:37-39` 는 자유편집 예시를 `{classbot,builder}` 로만, `:89-90` 은 `apps/classbot/` 전체 허용. 어느 쪽으로도 단정 불가 → Phase η PR 본문에 충돌 명시 + 사용자 확인(보수적). R8·R12 강한게이트도 R20 "게이트 불필요" 도 둘 다 과함 (§5 Phase η). 가이드 문서 수정만 §4.2 글로벌 게이트.
...
- **교사 라우트 편집 — authority 자기충돌 (codex R20·R21 최종)** — base `CLAUDE.md` 가 `:37-39`(자유편집 예시 = `{classbot,builder}` 만) vs `:89-90`(`apps/classbot/` 전체 허용)로 충돌. "게이트 불필요" 도 "게이트 필요" 도 권위만으로 단정 불가 → **Phase η PR 본문에 충돌 명시 + 사용자 확인**(보수적). 가이드 문서(root) 수정만 §4.2 글로벌 게이트.
...
> **⚠ boundary — authority 간 해석 충돌, 진입 전 사용자 확인 (codex R20·R21)**: base `CLAUDE.md` 가 교사 라우트 편집 범위에서 **자기 충돌**한다 — 한쪽(`.codex-runtime/base-CLAUDE.md:37-39`)은 자유 편집 예시를 `app/(student)/classbot/*` + `app/(teacher)/teacher/{classbot,builder}/*` 로만 들고, 다른 쪽(`:89-90`)은 `apps/classbot/` 전체 페이지·컴포넌트·mock·lib 수정을 넓게 허용한다. 따라서 `/teacher/{grading,reports,replay,assignment/new}` 등의 재편이 "게이트 불필요" 인지 "승인 필요" 인지 **권위 문서만으로 단정할 수 없다**. 본 plan 은 어느 쪽으로도 단정하지 않고, **Phase η 진입 PR 본문에 이 authority 충돌을 명시하고 사용자 확인을 받는다**(보수적). (R8·R12 의 강한 게이트 단정도, R20 의 "게이트 불필요" 단정도 둘 다 과함 — 충돌 사실 기록 + 사용자 확인이 정답.) root `CLAUDE.md`/`AGENTS.md` 등 *가이드 문서* 수정은 별개로 §4.2 글로벌 게이트.
