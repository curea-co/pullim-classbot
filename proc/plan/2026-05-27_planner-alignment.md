...
- **루트 가이드(root `CLAUDE.md`/`AGENTS.md`/`README.md`) 수정** — 루트 `CLAUDE.md` 가 사용자 명시 확인 필요한 글로벌 작업으로 분류. 본 plan 머지만으로 진입 불가 — 룰 PR 또는 사용자 승인 선행. **단 app-local 문서(`apps/classbot/{CLAUDE,AGENTS}.md`)는 글로벌 게이트 아님** (codex R16 — 일반 작업). app-local 가이드는 classbot 도메인-구체 룰의 *우선 참조* 권위 (§4.2).
...
> **⚠ 가이드 문서 수정 승인 게이트 (codex R9·R16 — 범위 정정)**: base `CLAUDE.md` 가 **글로벌 작업(사용자 명시 확인 필요)으로 묶는 것은 루트 가이드(`CLAUDE.md`/`AGENTS.md`/`README.md`)와 저장소 전역 파일** 이다. **app-local 문서(`apps/classbot/CLAUDE.md`·`apps/classbot/AGENTS.md`)는 이 글로벌 게이트 대상이 아니다** (codex R16 — app-local 가이드까지 동일 게이트로 승격하면 Phase ζ·η·θ 의 app-local 규칙 갱신이 과도하게 잠긴다). 따라서 아래 항목 중 **루트 가이드(root `CLAUDE.md`/`AGENTS.md`) 갱신만 글로벌 승인 게이트 대상**이고, app-local 문서 갱신은 일반 작업이다.
