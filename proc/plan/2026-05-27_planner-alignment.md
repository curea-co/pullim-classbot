...
- **`apps/classbot/` 단일 도메인 편집 허용** (R20 — 과잉 게이트 철회) — base `CLAUDE.md`:90 "해도 되는 것" 이 `apps/classbot/` 내 페이지·컴포넌트·mock·lib 수정/신규를 허용. 교사 라우트(`teacher/*`)도 `apps/classbot/app/(teacher)/` 안이므로 단일 도메인 작업 — 별도 게이트 불필요. R8·R12 의 `{classbot,builder}` 근거 게이트는 과잉 해석, 철회 (§5 Phase η). 가이드 문서 수정만 글로벌 게이트.
...
- **교사 라우트 편집 — `apps/classbot/` 단일 도메인 허용** (codex R20 — R8·R12·R19 과잉 게이트 최종 철회) — base `CLAUDE.md`:90 "해도 되는 것" 이 `apps/classbot/` 내 페이지·컴포넌트·mock·lib 수정/신규를 단일 도메인 범위로 허용한다. 교사 라우트(`teacher/page.tsx`, `grading`/`replay`/`reports`/`assignment/new`)는 모두 `apps/classbot/app/(teacher)/` 안 → **권위 허용 작업, 별도 승인 게이트 불필요**. `{classbot,builder}` 는 한 섹션의 예시였을 뿐, 그것으로 나머지 `apps/classbot` 편집을 잠그는 것은 오류 (§5 Phase η). 가이드 문서(root) 수정만 §4.2 글로벌 게이트.
...
### Phase α — spec §2 SOT 라인 갱신 + Phase β 진입 준비 (codex R20 — spec/가이드/코드 분리 다중 PR, §9 참조)
...
> **boundary — `apps/classbot/` 단일 도메인 편집은 허용 (codex R20 — 과잉 게이트 제거)**: base `CLAUDE.md` 의 "해도 되는 것" 섹션(`.codex-runtime/base-CLAUDE.md:90`)은 **`apps/classbot/` 내 페이지·컴포넌트·mock·lib 의 수정/신규를 단일 도메인 범위에서 허용**한다. 따라서 본 Phase η 가 재편하는 교사 라우트(`teacher/page.tsx`, `teacher/{grading,grading/[id],replay,replay/[id],reports,reports/[id],assignment/new}`)는 모두 `apps/classbot/app/(teacher)/` 안이므로 **권위가 허용하는 단일 도메인 작업** — 별도 승인 게이트 불필요. 이전 라운드(R8·R12)에서 `{classbot,builder}` 예시 경계를 근거로 나머지 교사 라우트에 게이트를 세운 것은 *과잉 해석* 이었고 본 라운드(R20)에서 철회한다. (단 root `CLAUDE.md`/`AGENTS.md` 등 *가이드 문서* 수정만 글로벌 게이트 — §4.2.)
...
6. **⚠ PR 분리 필수 (codex R20 — "1개 PR" 철회)**: base `CLAUDE.md`(`:93-99`)는 가이드(`CLAUDE.md`/`AGENTS.md`/`README.md`) 편집을 **글로벌 작업으로 분리**하고 *일반 작업과 한 PR 에 섞지 말라* 고 못박는다. 따라서 Phase α 는 **단일 PR 이 아니라 권위 분리 장치를 따라 나눈다**:
   - (a) **read-only spec 수정 PR** — 항목 2(spec §2 SOT 라인) — read-only 예외 승인 후
   - (b) **root 가이드 수정 PR** — 항목 3(root `CLAUDE.md`/`AGENTS.md` cross-link) — 가이드 수정 승인 후, 위 (a)와 별도
   - (c) **코드 PR** — 항목 4(`apps/backend/main.ts` PORT) 등 일반 코드 작업
   각 PR 은 Codex Review 통과 후 머지. spec·가이드·코드를 한 PR 에 섞지 않는다.
