...
- **가이드 문서(`CLAUDE.md`/`AGENTS.md`/`README.md`) 수정** — 루트 `CLAUDE.md` 가 사용자 명시 확인 필요한 글로벌 작업으로 분류. §4.2 docs 갱신 항목 전부 본 plan 머지만으로 진입 불가 — 룰 PR 또는 사용자 승인 선행.
...
> **⚠ 가이드 문서 수정 승인 게이트 (codex R9 — 필수)**: 루트 `CLAUDE.md` 는 가이드 문서(`CLAUDE.md`/`AGENTS.md`/`README.md`) 수정 자체를 **사용자 명시 확인이 필요한 글로벌 작업**으로 분류한다. 아래 docs 갱신 항목은 모두 그 범주에 속하므로, **본 plan 머지만으로 자동 진입 불가**다. 각 항목은 진입 전 (a) 별도 룰 갱신 PR 머지, 또는 (b) 해당 PR 에 대한 사용자 명시 승인 게이트가 선행되어야 한다 (§0 권위 순서 — 글로벌 파일 수정은 본 plan 만으로 승인 게이트 아님).

D-Lite 머지 시 이미 root `CLAUDE.md` / `AGENTS.md` + `apps/classbot/{CLAUDE,AGENTS,README}.md` 분리 셋업됨. 본 plan 진행 중 갱신 (각 항목 = 위 승인 게이트 대상):
...
- **완료 기준** (codex R13 — *코드 경로 한정*): `grep -rn "drizzle" apps/classbot/ --include='*.ts' --include='*.tsx' --include='package.json'` 0건 (코드·의존성 import·스크립트 잔존 없음). **마크다운 가이드 문서(`*.md`)는 검사 대상에서 제외** — `apps/classbot/{CLAUDE,AGENTS,README}.md` 등의 `drizzle` 언급 정리는 별도 가이드 문서 수정 승인 게이트(§4.2) 대상이라 본 Phase γ' 코드 정리 PR 범위 밖. Playwright 7 spec 회귀 0
