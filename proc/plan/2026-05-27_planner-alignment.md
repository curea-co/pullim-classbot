...
**패배 사례** (codex R1~R22 누적 지적 흡수):
- **🔻 게이트 범위 최종 확정 (codex R22 — R8~R21 과잉 게이트 철회)**: base `CLAUDE.md` 가 *허용* 하는 작업에 본 plan 이 근거 없는 승인 게이트를 덧씌웠던 것을 철회한다. (a) `apps/classbot/` 내 페이지·컴포넌트·mock·lib 수정/신규(교사 라우트 포함)는 단일 도메인 허용 작업 — 게이트 없음(`:90`). (b) `apps/backend/src/modules/classbot/**` 신설은 base 가 예정·허용한 Phase β BE 경로 — 게이트 없음(`:45`,`:91`). (c) app-local `apps/classbot/{CLAUDE,AGENTS}.md` 정비는 "우선 참조" 일반 작업 — 게이트 없음(`:34`,`:90`). **글로벌 게이트(사용자 명시 확인)는 오직: root `CLAUDE.md`/`AGENTS.md`/`README.md` + 저장소 전역 파일 + `apps/backend/src/{common,config,database}/*` + `packages/*` + read-only spec(`proc/spec/*`).**
- **prod-verify spec 수는 디렉토리 스캔 (R20)** — "7 spec" 고정 표기 금지. 현재 워크트리 ~10개 — 진입 시 `apps/classbot/tests/e2e` 스캔 기준.
- **Phase α PR 분리 (R20)** — 가이드/spec/코드를 한 PR 에 섞지 말 것 (base `CLAUDE.md`:93-99). spec 수정·root 가이드 수정·코드를 별도 PR 로 (§9).
- **`BotTone` 권위 라벨 충돌** (R19) — 권위 문서끼리 `정중/친근/스파르타`(07핸드오프) vs `정석/친구톤/스파르타`(03·05) 불일치. SOT 단정 금지, spec 정합화 전까지 충돌 상태 기록 (§4.2).
- **교사 라우트 편집** → 위 R22 항목 (a) 로 확정: `apps/classbot/app/(teacher)/` 안이므로 권위 허용 작업, 게이트 없음 (§5 Phase η). (R20·R21 의 "충돌·사용자 확인" 은 철회.)
- **PR 분리는 권위 규칙 아닌 본 plan 권고** (R21) — base 는 가이드/spec 을 글로벌 작업으로 분류할 뿐 "한 PR 에 섞지 말라" 의무 없음. Phase α spec/가이드/코드 분리는 *운영 권고* (§9).
- **mock 제거 = 디렉토리 전체** (R21) — `lib/mock/` 데이터 10 파일 + `index.ts` 배럴 모두 제거. 파일 개수 아닌 디렉토리·import grep 0 기준 (§5 Phase θ).
...
- **app-local 가이드 게이트** → 위 R22 항목 (c) 로 확정: `apps/classbot/{CLAUDE,AGENTS}.md` 정비는 게이트 없는 일반 작업 (§4.2). (R18 의 "도메인 규칙 변경 사용자 확인" 은 철회.)
...
- **app-local 가이드 = 우선 참조 권위, 게이트는 아님 (R16·R18·R22)** — `apps/classbot/{CLAUDE,AGENTS}.md` 는 classbot 도메인-구체 룰의 *우선 참조* 권위(비권위는 planner plan 문서뿐). 단 정비 자체는 글로벌 게이트 아닌 일반 작업 (위 R22 항목 c). §4.2.
...
- **`apps/backend/src/modules/classbot/**` 신설** → 위 R22 항목 (b) 로 확정: base `CLAUDE.md`:45/91 이 예정·허용한 Phase β BE 경로 — 게이트 없음. `{common,config,database}/*` 만 글로벌 (§5 Phase β). (R12 의 도메인-모듈 게이트는 철회.)
...
- **교사 라우트 편집** (중복 정리) → 위 R22 항목 (a). `apps/classbot/app/(teacher)/` = base:90 허용, 게이트 없음. 가이드 문서(root) 수정만 §4.2 글로벌 게이트.
- **루트 가이드(root `CLAUDE.md`/`AGENTS.md`/`README.md`) 수정** — 루트 가이드 갱신만 사용자 명시 확인 필요한 글로벌 작업. 본 plan 머지만으로 진입 불가 — 룰 PR 또는 사용자 승인 선행. app-local `apps/classbot/{CLAUDE,AGENTS}.md` 는 게이트 아님(위 R22 항목 c, §4.2).
...
- ❌ Mock 잔존: `apps/classbot/lib/mock/{persona,family,tutor,classbot,chat,live-content,classbot-dynamic-replies,classbot-greeting,classbot-home-preview,classbot-wellness-bot}.ts` (10 데이터 파일) **+ `index.ts` 배럴** (codex R21 — 다수 화면이 `@/lib/mock` 배럴로 import) — **Phase θ 에서 디렉토리 전체 제거** (Phase η 는 교사 Container/Presenter 구조 분리만, mock 제거 아님)
...
│   │   │   ├── mock/                        # 10 데이터 파일 + index.ts 배럴 — Phase θ 에서 디렉토리 전체 제거
...
│   │   ├── tests/                           # Playwright e2e spec (현재 워크트리 ~10개 — 진입 시 디렉토리 스캔, codex R20)
...
│       │   └── modules/
│       │       └── classbot/                # ★ root CLAUDE.md §2 도메인 래퍼 — Phase β 이후 모든 BE 도메인 작업은 이 안. base CLAUDE.md:45/91 이 Phase β modules/classbot/ 추가를 예정·허용 작업으로 명시 → 권위 허용 경로, 별도 게이트 없음 (codex R22). 단 common/config/database 만 글로벌
...
`.github/workflows/prod-verify.yml` 은 5개 자매 리포 중 classbot 유일 보유 자산이며, classbot 핵심 회귀 안전장치(`bun --filter @pullim-classbot/classbot test:e2e` — 전체 spec, 진입 시 `apps/classbot/tests/e2e` 디렉토리 스캔 기준 ~10개)다. 본 정렬 plan은 이 워크플로우를 **건드리지 않는다**:
...
- production URL (`https://pullim-classbot.vercel.app`), x-build-sha 검증 메커니즘, Playwright spec 전체(디렉토리 스캔 기준) 모두 보존
...
> **가이드 문서 수정 승인 게이트 (codex R9·R16·R18·R22 — 최종 확정)**: base `CLAUDE.md` 의 글로벌 게이트(사용자 명시 확인 필요)는 **루트 가이드(`CLAUDE.md`/`AGENTS.md`/`README.md`)와 저장소 전역 파일만** 명시한다(`:93-99`). **app-local 문서(`apps/classbot/{CLAUDE,AGENTS}.md`)는 "우선 참조" 대상일 뿐 추가 승인 규칙이 정의되어 있지 않다**(`:34`, `:90`). 따라서 (codex R22) **app-local 가이드 정비는 글로벌 게이트 대상이 아니며 일반 작업**이다 — 이전 라운드(R18)에서 "도메인 규칙 변경은 사용자 확인" 으로 올린 것은 base 보다 강한 과잉 해석이라 철회한다. **루트 가이드(root `CLAUDE.md`/`AGENTS.md`/`README.md`) 갱신만 글로벌 게이트 대상.**
...
> **⚠ read-only 문서 수정 승인 게이트 (codex R12·R22)**: base `CLAUDE.md` 의 **`공통 문서 (read only)` 목록에 `proc/spec/2026-05-18_be-api-design.md` 가 포함**된다. 따라서 아래 spec 결정 항목 갱신 PR(Phase β·γ 동봉)은 root docs·`packages/*` 와 동일하게 **read-only 예외 승인이 선행 조건**이다 (단 `modules/classbot/` 코드 작업 자체는 게이트 아님 — R22). 본 plan 머지만으로는 spec 직접 수정 흐름이 열리지 않는다. 각 spec 갱신 동봉 PR 은 (a) 별도 룰 갱신/예외 승인 PR, 또는 (b) 사용자 명시 확인을 먼저 받아야 하며, 그 전까지 plan 의 새 결정은 *제안* 상태에 머문다.
...
- main 머지 후 prod-verify 워크플로우 자동 트리거 — Vercel 빌드 → x-build-sha polling → Playwright prod hit 전체 spec (디렉토리 스캔 기준)
...
### Phase α — spec §2 SOT 라인 갱신 + Phase β 진입 준비 (spec/가이드/코드 PR 분리는 본 plan 운영 권고 — codex R21, §9 참조)
...
> **backend 작업 진입 — `modules/classbot/` 은 권위 허용 경로 (codex R22 — 게이트 철회)**: base `CLAUDE.md` 는 `apps/backend` 절(`:45`)에서 **Phase β 의 `apps/backend/src/modules/classbot/` 추가를 *예정된 작업*** 으로 적어 두고, 허용 작업에도 **"`apps/backend/src/modules/classbot/` 내 BE 작업 (Phase β 이후)"** 을 명시한다(`:91`). 즉 classbot 모듈은 *이 리포의 기존 단일 도메인 구현 경로* 이지 "새 도메인 추가" 가 아니다 — 따라서 **`modules/classbot/**` 신설에 별도 승인 게이트를 두지 않는다** (이전 R12 의 게이트는 base 가 이미 허용한 BE 진입 경로를 잘못 막는 과잉 해석이라 철회). **단 `apps/backend/src/{common,config,database}/*` (도메인-비종속 글로벌 토대)는 base `CLAUDE.md` 가 글로벌로 분류하므로 그 영역 신규/변경만 사용자 명시 확인 대상.**
...
Phase γ entity·마이그레이션·seed 가 prod-verify Playwright 전체 spec(디렉토리 스캔 기준) 회귀 0 으로 main 머지 + production 정상 동작이 확인된 *후* 별도 PR 로 폐기:
...
- **완료 기준**: 학생 14 라우트 중 **실질 로직 보유 10개**(위 9개 + `wellness/check-in` route-local form) 모두 Container/Presenter 분리. `bun --filter @pullim-classbot/classbot test:e2e` (Playwright 전체 spec — 디렉토리 스캔 기준) 회귀 0.
...
> **boundary — 교사 라우트 재편은 권위 허용 단일 도메인 작업 (codex R22 — 최종 확정)**: base `CLAUDE.md`(`:90`)는 **`apps/classbot/` 내 페이지·컴포넌트·mock·lib 의 수정·신규를 단일 도메인 범위에서 허용**한다. `:37` 의 `{classbot,builder}` 는 *요약 예시* 일 뿐 제한이 아니다. 따라서 Phase η 가 재편하는 교사 라우트(`teacher/page.tsx`, `teacher/{grading,grading/[id],replay,replay/[id],reports,reports/[id],assignment/new}`)는 모두 `apps/classbot/app/(teacher)/` 안 → **권위가 허용한 FE 작업, 별도 승인 게이트 없음**. 이전 라운드(R8·R12 강한 게이트, R20·R21 의 "충돌·사용자 확인")는 base 가 이미 허용한 범위를 임의 축소한 것이라 모두 철회한다. (root `CLAUDE.md`/`AGENTS.md` 등 *가이드 문서* 수정만 §4.2 글로벌 게이트 — 그것과 별개.)
...
- **`apps/classbot/lib/mock/` 디렉토리 전체 정리** (codex R21 — *파일 개수 기준 금지*): 10개 데이터 파일 외에 **`index.ts` 배럴 파일도 존재**하고 다수 화면이 `@/lib/mock` 경로로 이 배럴을 통해 import 한다. "10개 파일" 만 제거하면 `index.ts` 가 고아로 남아 stale export 를 계속 제공한다. 따라서 제거 대상은 *디렉토리 전체*(데이터 파일 + `index.ts` 배럴) — 마지막 PR 에서 제거(회귀 안전망), 완료 판정은 import grep 0건 기준(아래)
- **prod-verify 영향 검증** (codex R20 — *고정 숫자 금지, 디렉토리 스캔 기준*): 진입 시점에 `ls apps/classbot/tests/e2e/*.spec.ts` 로 **실제 spec 전체를 스캔**(현재 워크트리 기준 약 10개 — "7개" 고정 표기는 과소 산정)하고, 그 중 어느 spec 이 mock 에 의존하는지 사전 확인. 의존 spec 이 있으면 해당 spec 을 BE 응답 expect 로 갱신 (prod-verify 워크플로우 자체는 무변경)
- **완료 기준**: `grep -rn "from '@/lib/mock" apps/classbot/` 0건 + **`apps/classbot/lib/mock/` 디렉토리(데이터 10 파일 + `index.ts` 배럴) 완전 제거** (codex R21 — 파일 개수가 아니라 디렉토리·import 기준). **단 spec 14 개정 전에는 assignment surface 제외** (assignment 경로 별도 추적). spec 14 가 BE 연동으로 개정되면 그때 assignment mock 도 제거하여 0건 달성. `bun --filter @pullim-classbot/classbot test:e2e` 통과. prod-verify 다음 main push 시 정상 동작.
...
6. **PR 분리 — 본 plan 의 *운영 권고* (codex R21 — authority 필수 규칙 아님)**: base `CLAUDE.md`(`:93-99`)는 가이드(`CLAUDE.md`/`AGENTS.md`/`README.md`)·spec 수정을 **사용자 명시 확인 필요 글로벌 작업**으로 분류할 뿐, *"한 PR 에 섞지 말라"는 PR 분리 의무까지 정의하지는 않는다*. 따라서 아래 분리는 **권위가 강제하는 규칙이 아니라 본 plan 의 운영 권고**다 (리뷰·승인 추적을 깔끔히 하려는 목적). 권고 형태:
   - (a) read-only spec 수정 (항목 2) — read-only 예외 승인 후
   - (b) root 가이드 수정 (항목 3) — 가이드 수정 승인 후
   - (c) 일반 코드 (항목 4 등)
   → 가능하면 (a)/(b) 글로벌 승인 대상 변경과 (c) 코드를 **별도 PR 로 나누길 권고**(승인 게이트 추적 용이). 단 이는 저장소 규칙이 아니라 본 plan 권고이므로, 한 PR 로 묶더라도 (a)/(b)의 사용자 명시 확인만 충족되면 권위 위반은 아니다.
