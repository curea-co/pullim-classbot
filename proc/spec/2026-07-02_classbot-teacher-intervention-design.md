...
> **문제**: 내기→제출→진행률까지는 있지만 그 다음 교사 액션이 비어 있다 — 낸 뒤 교사가 할 일이 없음.
...
| **requiz** | 오답률 높은 문항만 골라 복습 과제로 다시 내기 — 기존 내기 파이프라인 재사용 |
...
```
교사: [리마인드]/[코멘트]/[다시 내기]/[응원] ──send()──▶ pullim-interventions (persist)
                                                           │
...
export interface InterventionEvent {
  ...
  assignmentId?: string;      // remind/comment 는 대상 과제, requiz 는 새로 낸 과제 id
...
| requiz | 제출 현황 시트 하단 (PR-2) | 문항별 오답률(submissions 답안 파생) → [오답 문항 다시 내기] = 기존 내기 파이프라인으로 "복습: {제목}" 과제 생성 + 대상 학생들에게 requiz 이벤트 |
...
2. **PR-2**: 제출 현황 시트 + comment 작성 + requiz 다시 내기.
...
- e2e(검토): 리마인드 루프 1개 — 교사 내기→학생 미제출→리마인드→학생 벨 배지 확인.
