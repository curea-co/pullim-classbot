...
> **여기에 `[예정]` 을 붙이지 않는다.** [03 § 2](03-features-and-ia.md) 의 `[예정]` 은 「`dev` 에 아직 없다 → **온다**」는 뜻이고, 이 문서가 **모티프 글리프**에 쓰는 표기가 바로 그것이다 — **원하는 것이 정해져 있고 인도만 없는** 자리다. M4 는 다르다: 전제가 사라져 **재검토 대상**이라 인도도, **올지 안 올지도 아직 정해지지 않았다.** 같은 표기를 붙이면 한 표기가 두 뜻을 지고, 다음 사람이 M4 를 「모티프 글리프처럼 언젠가 온다」로 읽는다. **정해지지 않은 것에는 표기 대신 상태어를 쓴다.**
> **행과 번호는 그대로 둔다.** 지우거나 M5~M9 를 당기면 번호로 M1~M9 를 부르는 자리(§ 15.2.1 소제목 · § 15.3 · § 15.8 · [04 § 9](04-ux-flow.md) 여러 곳)와 § 14 의 reduced-motion 목록이 함께 흔들린다 — 그 목록은 **무한 loop 모션을 세는 인벤토리**라 보류 항목이 남아 있어도 거짓이 되지 않으므로 손대지 않았다. 반면 [04 § 9.13](04-ux-flow.md) P3 행은 **작업 로드맵**이라 보류 항목이 서 있으면 안 되는 자리다 — 그래서 거기서만 뺐다.
