...
| `md` | 14px | **카드·버튼·인풋 표준** |
| `lg` | 20px | 히어로, 큰 컨테이너 |
| `pill` | 9999px | **사람** 아바타, FAB, status chip |

> 16/18/26 사용 금지(lint 룰화 대상). 회귀 사례: 클래스봇 카드 14·말풍선 18·다크 카드 20·이모지 원 26 혼재.

> **봇 아바타 배지는 `pill` 이 아니다 — `sm`(8) / `md`(14) / `lg`(20) 을 쓴다**(**`[예정]` #308**).
...
> 그래서 `pill` 은 **사람 아바타**만 갖는다 — 사람 얼굴이 원으로 서는 것은 오래된 관습이라 그대로 둔다. 봇 배지가 **둥근 사각**인 것은 크기 통일 때문만이 아니라 **사람과 봇을 형태로 가르는 신호**다 — 같은 목록에 사람과 봇이 나란히 설 때 원/사각이 둘을 먼저 구분한다.
...
- **예외 2 — 봇 페르소나 식별 (데이터 계약 한정)**: 봇 페르소나의 식별 emoji는 **데이터 자리에서만** 예외다 — DB 컬럼(`class_bots.avatar_emoji`)·직렬화·API 응답·mock 픽스처가 emoji 문자열을 그대로 보존한다. 그 자리에서 emoji 를 지우는 것은 계약을 깨는 일이라 하지 않는다. 적용 사례: `ClassBot.avatarEmoji`, `LiveSessionRow.botEmoji`, `BotSettingsState.identity.avatarEmoji`. 위치: `apps/classbot/lib/db/schema.ts`, `apps/classbot/lib/mock/classbot.ts`.
  ...
  **값의 출처는 [`03 § 4.4.3`](03-features-and-ia.md) 과 같은 계약이다** — 교사 봇 관리 목록(`/teacher/bots`)의 아바타 행이 `subject` 첫 글자 → `name` 첫 글자 → lucide `Bot` 로 적혀 있고, 두 자리를 함께 고쳐야 모순이 나지 않는다. **면의 모양·크기는 [§ 4.1](#41-통일-권고-통합-시스템-v01) 이, 면의 배경색은 [`07 § 4.6.2`](07-branding.md) 가 갖는다** — 봇 배지는 `pill` 이 아니라 `sm`/`md`/`lg` 이고, 배경은 봇별 색이 아니라 브랜드 블루 단색이다(둘 다 **`[예정]` #308**).
