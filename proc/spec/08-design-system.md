...
> **여기에 `[예정]` 을 붙이지 않는다.** `[예정]` 은 이 문서가 **하겠다고 적은 것**에 붙는다 — [03 § 2](03-features-and-ia.md) 의 정의가 「`dev` 에 아직 없다」 + 「**인도할 자리**」다. **모티프 글리프**가 그 경우이고, 인도만 아직 없다. **M4 는 다르다 — 전제가 사라져 이 문서가 그 약속을 거둔 상태**라, 「`dev` 에 아직 없다」는 같아도 **앞으로 하겠다고 적을 수 없다.** 「아직 없다」만으로 둘을 가르면 다음 사람이 M4 를 「모티프 글리프처럼 언젠가 온다」로 읽는다. 그래서 표기가 아니라 **상태어**로 적는다.
> **행과 번호는 그대로 둔다.** 지우거나 M5~M9 를 당기면 번호로 M1~M9 를 부르는 자리(§ 15.2.1 소제목 · § 15.3 · § 15.8 · [04 § 9](04-ux-flow.md) 여러 곳)와 § 14 의 reduced-motion 조건부 룰이 함께 흔들린다 — 그 룰은 「M4 가 있다면 reduce 시 정적」이라 보류여도 거짓이 되지 않으므로 손대지 않았다.
