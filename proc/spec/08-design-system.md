...
| **M4** | 봇 아바타 blink | eye-mask scaleY 1→0.1→1 | 140ms / linear, 4–7s 랜덤 간격 | 사람 같은 디테일 — **보류: 전제 소멸**(재검토 전까지 구현하지 않는다). 이니셜 배지·모티프 글리프 **어느 쪽에도 눈이 없다**. 표 아래 주 |
...
> **M4(봇 아바타 blink)는 보류한다 — 전제가 사라졌다. 재검토 전까지 구현하지 않는다.**
> M4 는 아바타에 **눈이 있다**는 전제 위에 서 있다(`eye-mask scaleY`). `dev` 에서는 아바타가 **얼굴 emoji** 라 그 전제가 섰는데, **그 전제를 없앤 것이 이 변경**이다 — [§ 14.1.1 예외 2](#1411-아이콘-일관성-예외-emoji-허용-영역) 가 세운 아바타 면은 **과목 이니셜 한 글자 배지**이고, 그 너머의 목표인 [`07 § 4.6.2`](07-branding.md) **모티프 글리프**(`√ x² ÷ 수식 도트` · `한글 자모 ㅎㄱㅁ` · `지도 라인`)에도 눈이 없다. **가릴 눈이 어느 쪽에도 없다.**
> 「**사람 같은 디테일**」은 방향도 반대다. 예외 2 를 좁힌 근거가 「사람이 아니라는 AI 고지 옆에 **사람 얼굴**을 둔다」였다 — 봇을 사람처럼 보이게 하는 것을 아바타 면에서 걷어내면서 **눈 깜빡임만 남길 수는 없다.**
> **여기에 `[예정]` 을 붙이지 않는다.** `[예정]` 은 이 문서가 **하겠다고 적은 것**에 붙는다 — [03 § 2](03-features-and-ia.md) 의 정의가 「`dev` 에 아직 없다」 + 「**인도할 자리**」다. **모티프 글리프**는 **대상이 정해졌고 인도만 미정**이라 그 경우다. **M4 는 다르다 — 전제가 사라져 대상 자체가 재검토 대상**이고, 이 문서가 그 약속을 거둔 상태다. **「`dev` 에 아직 없다」는 둘이 같다** — 그것만으로 가르면 다음 사람이 M4 를 「모티프 글리프처럼 언젠가 온다」로 읽는다. 둘을 가르는 것은 **대상이 정해졌는가**이고, 정해지지 않은 쪽은 표기가 아니라 **상태어**로 적는다.
> **행과 번호는 그대로 둔다.** 지우거나 M5~M9 를 당기면 번호로 M1~M9 를 부르는 자리(§ 15.2.1 소제목 · § 15.3 · § 15.8 · [04 § 9](04-ux-flow.md) 여러 곳)와 § 14 의 reduced-motion 조건부 룰이 함께 흔들린다 — 그 룰은 「M4 가 있다면 reduce 시 정적」이라 보류여도 거짓이 되지 않으므로 손대지 않았다. 반면 [04 § 9.13](04-ux-flow.md) P3 행은 **작업 로드맵**이라 보류 항목이 서 있으면 안 되는 자리다 — 그래서 거기서만 뺐다.
> 「살아있다」 신호는 **M3(idle 호흡)** 와 **M5(LIVE 뱃지 펄스)** 가 맡는다 — 둘 다 **형태에 무관한 모션**이라 이니셜 배지에도 그대로 얹힌다. 보류하는 것은 M4 하나다.
