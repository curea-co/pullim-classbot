...
**예외 — 모드 식별 시그니처.** 과제 「오답정복」 모드 배지는 키 CTA 가 아니지만 레몬을 쓴다. § 15.6 이 세 모드(연습·오답정복·시험)의 면이 서로 갈려야 한다고 정하는데, 블루 계열 안에서 셋을 가르면 「모두 파랑으로 보이던 회귀」로 되돌아가기 때문이다. 이 예외는 **모드 배지 하나뿐**이며 다른 강조로 넓히지 않는다. 화면당 1~2 곳 제한을 셀 때 이 배지도 함께 센다.
...
| `md` | 14px | **카드·버튼·인풋 표준** |
| `lg` | 20px | 히어로, 큰 컨테이너 |
| `pill` | 9999px | **사람** 아바타(**`[예정]` #308** — **지금 `dev` 는 봇 아바타도 `rounded-full` 이다**: `chat-transcript.tsx:119`), FAB, status chip |

> 16/18/26 사용 금지(lint 룰화 대상). 회귀 사례: 클래스봇 카드 14·말풍선 18·다크 카드 20·이모지 원 26 혼재.

> **봇 아바타 배지의 모서리는 `pill`(9999px)이 아니다 — 8 / 14 / 20px 에서 고른다**(**`[예정]` #308**).
> `pill` 행이 「아바타」를 통째로 가져간 것은 **아바타가 이모지 원이던 시절의 전제**다 — 바로 위 회귀 사례가 그 전제를 스스로 적고 있다(「… **이모지 원 26** 혼재」). 아바타 면이 [§ 14.1.1 예외 2](#1411-아이콘-일관성-예외-emoji-허용-영역) 대로 **과목 이니셜 배지**로 바뀌면 원을 강제할 이유가 사라진다.
> 그래서 `pill` 은 **사람 아바타**만 갖는다 — 사람 얼굴이 원으로 서는 것은 오래된 관습이라 그대로 둔다. 봇 배지가 **둥근 사각**인 것은 **사람과 봇을 형태로 가르는 신호**다 — 같은 목록에 사람과 봇이 나란히 설 때 원/사각이 둘을 먼저 구분한다.
> ⚠ **이 절이 정하는 것은 모서리 하나다 — 배지의 박스 크기는 여기서 정하지 않는다.**
> 그리고 **이 절을 인용할 때는 토큰 이름을 쓰지 말고 px 로 적는다.** 위 표의 `sm`/`md`/`lg` 는 **모서리 이름**인데 `bot-avatar.tsx` 의 같은 세 글자는 **박스 이름**이라 뜻이 다르고, 「봇 배지는 `sm`/`md`/`lg`」라 적으면 독자가 **박스 이름으로 읽는다.** 게다가 **둘은 1:1 이 아니다** — #308 의 배지는 박스가 **다섯**(28 · 32 · 36 · 44 · 56px)이고 모서리는 **셋**(8 · 14 · 20px)이라, 28·32 가 8 을, 36·44 가 14 를, 56 이 20 을 쓴다.
...
- 무한 loop (M2 타이핑·M3 봇 호흡·M5 LIVE 펄스)는 reduce 시 1-frame 정적 (LIVE 뱃지 점은 정적 점으로 유지). *(종전 목록에 있던 `M4 blink` 는 § 15.2 에서 **폐기**했다 — 이니셜 배지에도 모티프 글리프에도 가릴 눈이 없다.)*
...
> ③ **출처 결정문이 이 저장소에 없다.** 아래 「결정 출처」가 가리키는 `proc/archive/2026-05-07_inline-emoji-cleanup.md` 는 `proc/archive/` 에 존재하지 않고, 예외 2 가 적어 둔 위치도 추출 이전 구조(`src/lib/mock/…`)를 가리켰다. 이번에 예외 2 안에서 고친 것은 **경로 표기와, 이 저장소에 존재하지 않는 적용 사례** 둘이다 — 경로는 `apps/classbot/…` 기준으로 옮겼고, 적용 사례에서는 **`TeacherVoice.emoji` 와 그 위치 `src/lib/mock/infinity.ts` 를 뺐다.** `TeacherVoice` 는 `apps/classbot` 전체에 **0건**이고 `infinity.ts` 도 **없다**(전수 확인). 그 결과 예외 2 에 **교사 페르소나 적용 사례가 하나도 남지 않아 제목에서도 「교사」를 뺐다** — 남은 셋(`ClassBot.avatarEmoji` · `LiveSessionRow.botEmoji` · `BotSettingsState.identity.avatarEmoji`)은 전부 **봇** 필드다. `LiveSessionRow.botEmoji` 가 `🧑‍🏫`·`👩‍🏫` 를 담고 있지만 그 행의 이름은 「수학봇」·「영어봇」이라 **봇이지 교사가 아니다**. 교사 페르소나 emoji 가 실제로 생기는 날 예외를 다시 넓히는 것이 순서다 — **없는 것을 근거로 예외를 넓혀 두지 않는다**는 것이 위 ②와 같은 이유다.
...
> 그래서 예외를 **없애지 않고 좁힌다** — **데이터는 보존하고, 렌더만 뺀다.**
> **예외 4(봇 발화 메시지 본문 안 emoji)는 그대로 유효하되, 근거를 예외 2 에서 떼어 자립시켰다.** 종전 예외 4 는 스스로를 「예외 2의 자연스러운 확장」이라 적었는데, 예외 2 가 **데이터 한정 · 렌더 배제**로 좁아지면 **렌더 예외**인 예외 4 는 그 확장일 수 없다 — 방향이 반대다. 그대로 두면 「예외 2 가 데이터 한정이니 예외 4 도 데이터 한정」이라 읽고 봇 발화 본문의 emoji 를 걷어낼 길이 열린다. 예외 4 의 근거는 **페르소나 voice** 하나이고 예외 2 에 매달리지 않는다.
> **이 개정은 문서만 바꾼다 — 렌더 전환은 아직 `dev` 에 없다(`[예정]` #308).** `dev` 의 아바타 자리는 여전히 `bot.avatarEmoji` 를 그린다. **코드가 현재 상태의 기준**이므로([03 § 2](03-features-and-ia.md) `[예정]` 표기 규칙), 좁혀진 예외 2 의 렌더 문장은 **#308 이 들어오기 전까지 통과 기준으로 읽고** 현재 동작 설명으로 읽지 않는다. #308 이 `dev` 에 들어오면 그 자리의 `[예정]` 표기를 지운다.
...
- **예외 2 — 봇 페르소나 식별 (데이터 계약 한정)**: 봇 페르소나의 식별 emoji는 **데이터 자리에서만** 예외다 — DB 컬럼(`class_bots.avatar_emoji`)·직렬화·API 응답·mock 픽스처가 emoji 문자열을 그대로 보존한다. 그 자리에서 emoji 를 지우는 것은 계약을 깨는 일이라 하지 않는다. 적용 사례: `ClassBot.avatarEmoji`, `LiveSessionRow.botEmoji`, `BotSettingsState.identity.avatarEmoji`. 위치: `apps/classbot/lib/db/schema.ts`, `apps/classbot/lib/mock/classbot.ts`.
  **제품의 봇 아바타 렌더는 이 예외에 들지 않는다 — 역할을 가리지 않는다(학생 · 교사 · 학부모, 그리고 앞으로 늘어날 역할도).** 여기서 「렌더」는 **화면에 그리는 아바타 면 하나**를 말한다. 그 밖의 자리(DB 컬럼 · 직렬화 · API 응답 · mock 픽스처)는 **위 단락 그대로 예외**다 — 좁히는 것은 화면이지 데이터가 아니다. 아바타 면에는 emoji 를 그리지 않고 **과목 이니셜**(과목 이름 첫 글자, 과목이 없으면 봇 이름 첫 글자)로 그린다 — **`[예정]` #308**. 과목·이름이 **둘 다 없을 때의 폴백은 lucide `Bot`** 이고, 이 폴백도 **`[예정]` #308** 이 함께 인도한다 — #308 의 `bot-avatar.tsx` 가 실제로 `import { Bot } from 'lucide-react'` 로 구현한다. **그 너머의 목표는 따로 있다** — [`07 § 4.6.2`](07-branding.md) 가 봇마다 지정한 **모티프** 글리프이고, 이쪽은 **`[예정]`** · **인도 미배정**이다(#308 은 이니셜과 폴백까지만 간다). **지금 `dev` 의 코드는 아직 emoji 를 그린다** — `app/(student)/classbot/classroom/page.tsx:120` · `app/(teacher)/teacher/bots/page.tsx:108` 등이 `bot.avatarEmoji` 를 그대로 렌더한다. 그 전환이 **이 조항의 인도**이며, 위 문장은 통과 기준이지 현재 동작 설명이 아니다. 렌더가 emoji 를 쓰지 않게 되어도 위 데이터 계약을 함께 걷지 않는다 — **읽지 않을 뿐 지우지 않는다.**
  **부모가 보는 얼굴은 자녀가 보는 얼굴과 같아야 한다 — 그래서 이 전환은 역할별로 나눠 갈 수 없다.** `/parent/self-study` 의 `SelfStudyBotRow` 가 그 계약을 코드 주석으로 적어 두고 있다(`apps/classbot/app/(parent)/parent/self-study/child-self-study-card.tsx` 의 `SelfStudyBotRow` 앞 계약 주석 — 「## 타일 안은 …」 절). **이 인용만 줄 번호를 적지 않는다** — 바로 위 「지금 `dev` 는 …」 줄의 번호는 #308 이 들어오면 **문장째 지워질** 자리라 그 창에서만 참이면 되지만, 이 주석은 #308 이 **고쳐서 남기는** 블록이라 번호만 어긋난 채 살아남는다:
  > ## 타일 안은 **아이가 보는 그 얼굴**이다 — lucide 글리프로 되돌리지 마라
  > 아이는 봇을 🧑‍🔬 로 보고 「과학봇」이라 부른다. 부모 화면에 회색 글리프가 떠 있으면 둘이 같은 봇을 이야기하면서 서로 다른 것을 보고 있는 셈이라, `avatarEmoji` 가 이 자리에 왔다. …
  > 기준은 「빈 값을 어떻게 메우나」가 아니라 **「아이 화면과 같은가」**다. 그 하나만 지키면 된다.

  그 주석이 지키려는 것은 **emoji 가 아니라 일치**다 — 기준을 스스로 「아이 화면과 같은가」라고 적는다. 그러니 학생 화면이 과목 이니셜로 가면 학부모 화면도 **같은 변경에서 함께** 가야 하고, 한쪽만 바꾸면 그 주석이 없애려던 어긋남을 정확히 되살린다. **#308 이 두 화면을 함께 인도한다**(파일 목록에 `child-self-study-card.tsx` 가 들어 있다). 「lucide 글리프로 되돌리지 마라」는 경고도 그대로 유효하되 **새 계약에 맞게 읽는다** — 금지되는 것은 **`subject`·`name` 값이 있는데도 글리프로 때우는 것**이지 **lucide 글리프 일반이 아니다.** 이니셜은 글리프가 아니라 **그 봇의 과목을 말하는 글자**이고, 아이 화면과 부모 화면에 **같은 글자**가 선다. 몇 줄 위에서 지정한 **폴백 lucide `Bot` 은 `subject`·`name` 이 둘 다 없을 때 하나뿐인 정당한 경로**이고, 그 경우에도 두 화면이 **같은 것**을 보므로 이 경고가 지키려던 일치는 깨지지 않는다 — 경고와 폴백은 한 조항 안에서 서로 어긋나지 않는다.
  **값의 출처는 [`03 § 4.4.3`](03-features-and-ia.md) 과 같은 계약이다** — 교사 봇 관리 목록(`/teacher/bots`)의 아바타 행이 `subject` 첫 글자 → `name` 첫 글자 → lucide `Bot` 로 적혀 있고, 두 자리를 함께 고쳐야 모순이 나지 않는다. **면의 radius 는 [§ 4.1](#41-통일-권고-통합-시스템-v01) 이, 면의 배경색은 [`07 § 4.6.2`](07-branding.md) 가 갖는다** — 봇 배지의 radius 는 `pill`(9999px)이 아니라 **8 / 14 / 20px** 중 하나이고, 배경은 봇별 색이 아니라 브랜드 블루 단색이다(둘 다 **`[예정]` #308**). **배지의 박스 크기는 두 곳 다 정하지 않는다** — 구현 몫이다.
...
- **예외 4 — 봇 페르소나 발화 메시지 텍스트 (페르소나 voice 독립 예외)**: **좁아진 예외 2(데이터 계약)와 별개로 자립한 예외다** — 발화 본문 안 emoji 는 그 자체로 예외이고, 예외 2 가 렌더를 뺐다고 해서 여기까지 따라 좁아지지 않는다. 봇·교사 페르소나가 발화하는 카톡 스타일 챗 스트림 메시지 본문 안 emoji는 페르소나 voice 일부라 예외. 적용 사례: `lib/mock/coach.ts` agent message stream(📅/📚/📈/🎯/💡/🌅/🔥 등 12+건), `lib/mock/phase1.ts` 봇 인삿말 안 `🙌`. 위치: `src/lib/mock/coach.ts`, `src/lib/mock/phase1.ts`.
...
| **M4** | 봇 아바타 blink | eye-mask scaleY 1→0.1→1 | 140ms / linear, 4–7s 랜덤 간격 | 사람 같은 디테일 — **⛔ 폐기: 전제 소멸.** 이니셜 배지·모티프 글리프 **어느 쪽에도 눈이 없다**. 표 아래 주 |
...
> **M4(봇 아바타 blink)는 폐기했다 — 행은 남기고 여기서 폐기라고 말한다.**
> M4 는 아바타에 **눈이 있다**는 전제 위에 서 있다(`eye-mask scaleY`). `dev` 에서는 아바타가 **얼굴 emoji** 라 그 전제가 섰는데, **그 전제를 없앤 것이 이 변경**이다 — [§ 14.1.1 예외 2](#1411-아이콘-일관성-예외-emoji-허용-영역) 가 세운 아바타 면은 **과목 이니셜 한 글자 배지**이고, 그 너머의 목표인 [`07 § 4.6.2`](07-branding.md) **모티프 글리프**(`√ x² ÷ 수식 도트` · `한글 자모 ㅎㄱㅁ` · `지도 라인`)에도 눈이 없다. **가릴 눈이 어느 쪽에도 없으니 보류가 아니라 폐기다** — 되살릴 전제가 돌아오지 않는다.
> 「**사람 같은 디테일**」은 방향도 반대다. 예외 2 를 좁힌 근거가 「사람이 아니라는 AI 고지 옆에 **사람 얼굴**을 둔다」였다 — 봇을 사람처럼 보이게 하는 것을 아바타 면에서 걷어내면서 **눈 깜빡임만 남길 수는 없다.**
> **여기에 `[예정]` 을 붙이지 않는다.** [03 § 2](03-features-and-ia.md) 의 `[예정]` 은 「`dev` 에 아직 없다 → **온다**」는 뜻이고, 이 문서가 **모티프 글리프**에 쓰는 표기가 바로 그것이다 — **원하는데 인도만 없는** 것. M4 는 다르다: **원하지 않는다.** 같은 표기를 붙이면 한 표기가 두 뜻을 지고, 다음 사람이 M4 를 「모티프 글리프처럼 언젠가 온다」로 읽는다. 그래서 표기 대신 **상태어**로 적는다.
> **그래도 행은 표에 남긴다.** 행을 빼면 위 제목 「모션 9종」과 행 수가 어긋나고, 제목을 고치자니 앵커와 「9종」을 되뇌는 세 자리가 딸려 오는데 그중 하나는 **다른 변경의 과거 인벤토리 줄**이라 건드릴 것이 아니다. 번호를 당기면 M1~M9 를 번호로 부르는 자리(§ 15.2.1 소제목 · § 15.3 · § 15.8 · [04 § 9](04-ux-flow.md) 여러 곳)가 전부 어긋난다. **제자리에 두고 여기서 폐기라고 말하는 것이 파급이 가장 작다** — 빈 번호도, 셈을 해명하는 각주도 필요 없다.
> 대신 **M4 를 작업으로 세우던 두 자리는 걷었다** — § 14 의 reduced-motion 목록과 [04 § 9.13](04-ux-flow.md) P3 우선순위 행. 폐기한 모션을 fallback 목록과 로드맵에 남겨 두면 없는 것을 가리킨다.
> 「살아있다」 신호는 **M3(idle 호흡)** 와 **M5(LIVE 뱃지 펄스)** 가 맡는다 — 둘 다 **형태에 무관한 모션**이라 이니셜 배지에도 그대로 얹힌다. 폐기하는 것은 M4 하나다.
...
| 상태 | 높이 | 노출 정보 |
|---|---|---|
| **기본 (collapsed)** | 56px | `← [수] 수학이 형 · L3 교과범위        ⓘ ⌃` |
...
- 좌측 `[수]` 는 **과목 이니셜 아바타 배지**다 — emoji 가 아니다(**`[예정]` #308** · § 14.1.1 예외 2). **지금 `dev` 의 코드는 아직 `bot.avatarEmoji` 를 그린다** — 위 표는 통과 기준이지 현재 동작 설명이 아니다.
...
> **2026-08 개정 — 상태를 hue 로 가르지 않는다.**
> 종전 표는 마감 임박에 warning(앰버), 완료에 success(초록)를 썼는데, **같은 문서 § 1.6·§ 14.1 과 정면으로 어긋났다** — § 1.6 은 「레몬은 키 CTA 한정·화면당 1~2곳」, § 14.1 은 「**시맨틱 색만으로 의미 전달 금지**」다. 과제 카드는 한 화면에 여러 장이 깔리므로 상태마다 hue 를 하나씩 배정하면 화면이 무지개가 된다.
> **상태 차이는 한 hue(블루) 안의 명도로 말하고, 뜻은 글자(`semanticLabel`)와 아이콘이 진다.** 시맨틱 hue 는 「지연」의 빨강 하나만 남긴다 — 교사·학생이 놓치면 손해가 나는 유일한 상태다.
> 모드 식별(오답정복 레몬 · 시험 navy)은 상태가 아니라 **모드**라서 남긴다. 그 둘은 아래 「모드 배지」가 갖는다.

| 상태 | progress bar | D-day chip | 카드 좌측 라이너 | 색 말고 무엇이 말하나 |
|---|---|---|---|---|
| 진행 중 | `brand.600` | 회색 + "D-N" | `brand.50` | "D-N" 숫자 |
| 마감 임박 (D-1) | `brand.800` (가장 진한 블루 = 가장 급함) | 진한 블루 chip "내일" | brand.800 | **"내일"** 글자 |
| 지연 | `danger.fg` (`#C03B3F`) | danger chip "지난 N일" | danger | **"지난 N일"** 글자 + 유일한 시맨틱 hue |
| 완료 | `brand.300` (물러남) + 100% | 회색 "완료" | brand.300 | **체크 아이콘** + "완료" + 100% |

**모드 배지** — 상태와 별개다. 진실원은 `lib/tokens/assignment-state.ts` 의 `assignmentModeBadge` 하나이며, 화면마다 복제하지 않는다. 같은 파일의 `getAssignmentVisual()` 은 **상태**(진행 중·마감 임박·지연·완료)의 시각 계약을 돌려주고, 모드 배지는 그와 갈라 둔다 — 상태는 카드가 지금 어떤 처지인지를, 모드는 이 과제가 무엇인지를 말한다.

| 모드 | 면 | 근거 |
|---|---|---|
| 연습 | 옅은 블루 | — |
| 오답정복 | **lime** | 모드 식별 시그니처. § 1.6 「예외 — 모드 식별 시그니처」 조항 |
| 시험 | **navy solid**(`surface.inverse`) | 오류가 아니라 **모드 전환** 신호다. 빨강을 쓰지 않는다 |

- 뱃지 3종(연습/오답정복/시험)이 모두 파랑 계열로 보이던 회귀를 해결. **셋의 면이 서로 달라야 한다** — `lib/tokens/__tests__/assignment-mode-badge.test.ts` 가 지킨다.
- 봇별 그룹 헤더: 카드 위에 `[수] 수학이 형 · 3개` + 봇 시그니처 컬러 점 + 진척 한 줄 (`8/55문항`). 앞의 `[수]` 는 **과목 이니셜 아바타 배지**이고 emoji 가 아니다(**`[예정]` #308** · § 14.1.1 예외 2) — 종전 표기는 바깥 대괄호가 헤더 전체(아바타 + 이름 + 개수)를 감싸고 있었는데, 아바타 배지를 § 15.5 · [04 § 9.2](04-ux-flow.md) 와 같은 `[글자]` 꼴로 통일했다.
