...
- **예외 2 — 봇 페르소나 식별 (데이터 계약 한정)**: 봇 페르소나의 식별 emoji는 **데이터 자리에서만** 예외다 — DB 컬럼(`class_bots.avatar_emoji`)·직렬화·API 응답·mock 픽스처가 emoji 문자열을 그대로 보존한다. 그 자리에서 emoji 를 지우는 것은 계약을 깨는 일이라 하지 않는다. 적용 사례: `ClassBot.avatarEmoji`, `LiveSessionRow.botEmoji`, `BotSettingsState.identity.avatarEmoji`. 위치: `apps/classbot/lib/db/schema.ts`, `apps/classbot/lib/mock/classbot.ts`.
  ...
  **부모가 보는 얼굴은 자녀가 보는 얼굴과 같아야 한다 — 그래서 이 전환은 역할별로 나눠 갈 수 없다.** `/parent/self-study` 의 `SelfStudyBotRow` 가 그 계약을 코드 주석으로 적어 두고 있다(`apps/classbot/app/(parent)/parent/self-study/child-self-study-card.tsx` 의 `SelfStudyBotRow` 앞 계약 주석 — 「## 타일 안은 …」 절). **줄 번호로 인용하지 않는다** — #308 이 이 주석 블록을 함께 고치므로 지금 찍은 줄 번호는 #308 이 `dev` 에 들어오는 순간 엉뚱한 줄을 가리킨다:
  ...
  그 주석이 지키려는 것은 **emoji 가 아니라 일치**다 — 기준을 스스로 「아이 화면과 같은가」라고 적는다. 그러니 학생 화면이 과목 이니셜로 가면 학부모 화면도 **같은 변경에서 함께** 가야 하고, 한쪽만 바꾸면 그 주석이 없애려던 어긋남을 정확히 되살린다. **#308 이 두 화면을 함께 인도한다**(파일 목록에 `child-self-study-card.tsx` 가 들어 있다). 「lucide 글리프로 되돌리지 마라」는 경고도 그대로 유효하되 **새 계약에 맞게 읽는다** — 금지되는 것은 **값이 있는데 글리프로 내려가는 것**이지 lucide 자체가 아니다. 이니셜은 글리프가 아니라 **그 봇의 과목을 말하는 글자**이고, 아이 화면과 부모 화면에 **같은 글자**가 선다. 몇 줄 위에서 지정한 **폴백 lucide `Bot` 은 `subject`·`name` 이 둘 다 없을 때 하나뿐인 정당한 경로**이고, 그 경우에도 두 화면이 **같은 것**을 보므로 이 경고가 지키려던 일치는 깨지지 않는다 — 경고와 폴백은 한 조항 안에서 서로 어긋나지 않는다.
