...
- **예외 2 — 봇·교사 페르소나 식별 (데이터 계약 한정)**: 봇·교사 페르소나의 식별 emoji는 **데이터 자리에서만** 예외다 — DB 컬럼(`class_bots.avatar_emoji`)·직렬화·API 응답·mock 픽스처가 emoji 문자열을 그대로 보존한다. 그 자리에서 emoji 를 지우는 것은 계약을 깨는 일이라 하지 않는다. 적용 사례: `ClassBot.avatarEmoji`, `LiveSessionRow.botEmoji`, `BotSettingsState.identity.avatarEmoji`. 위치: `apps/classbot/lib/db/schema.ts`, `apps/classbot/lib/mock/classbot.ts`.
  **제품의 봇 아바타 렌더는 이 예외에 들지 않는다 — 역할을 가리지 않는다(학생 · 교사 · 학부모, 그리고 앞으로 늘어날 역할도).** 여기서 「렌더」는 **화면에 그리는 아바타 면 하나**를 말한다. 그 밖의 자리(DB 컬럼 · 직렬화 · API 응답 · mock 픽스처)는 **위 단락 그대로 예외**다 — 좁히는 것은 화면이지 데이터가 아니다. 아바타 면에는 emoji 를 그리지 않고 **과목 이니셜**(과목 이름 첫 글자, 과목이 없으면 봇 이름 첫 글자)로 그린다 — **`[예정]` #308**. 목표는 [`07 § 4.6.2`](07-branding.md) 가 봇마다 지정한 **모티프** 글리프이고, 둘 다 없을 때의 폴백만 lucide `Bot` 이다(**`[예정]`** · 인도 미배정). **지금 `dev` 의 코드는 아직 emoji 를 그린다** — `app/(student)/classbot/classroom/page.tsx:120` · `app/(teacher)/teacher/bots/page.tsx:108` 등이 `bot.avatarEmoji` 를 그대로 렌더한다. 그 전환이 **이 조항의 인도**이며, 위 문장은 통과 기준이지 현재 동작 설명이 아니다. 렌더가 emoji 를 쓰지 않게 되어도 위 데이터 계약을 함께 걷지 않는다 — **읽지 않을 뿐 지우지 않는다.**
  ...
  **값의 출처는 [`03 § 4.4.3`](03-features-and-ia.md) 과 같은 계약이다** — 교사 봇 관리 목록(`/teacher/bots`)의 아바타 행이 `subject` 첫 글자 → `name` 첫 글자 → lucide `Bot` 로 적혀 있고, 두 자리를 함께 고쳐야 모순이 나지 않는다.
