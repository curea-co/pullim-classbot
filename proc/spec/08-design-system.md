...
**모드 배지** — 상태와 별개다. 진실원은 `lib/tokens/assignment-state.ts` 의 `assignmentModeBadge` 하나이며, 화면마다 복제하지 않는다.

| 모드 | 면 | 근거 |
|---|---|---|
| 연습 | 옅은 블루 | — |
| 오답정복 | **lime** | 모드 식별 시그니처. § 1.6 「남용 금지」의 명시적 예외 |
| 시험 | **navy solid**(`surface.inverse`) | 오류가 아니라 **모드 전환** 신호다. 빨강을 쓰지 않는다 |

- 뱃지 3종(연습/오답정복/시험)이 모두 파랑 계열로 보이던 회귀를 해결. **셋의 면이 서로 달라야 한다** — 테스트가 지킨다.
