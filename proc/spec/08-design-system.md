...
> **봇 아바타 배지의 radius 는 `pill` 이 아니다 — 위 표의 `sm`(8px) / `md`(14px) / `lg`(20px) 중에서 고른다**(**`[예정]` #308**).
...
> ⚠ **이 절이 정하는 것은 radius 하나다 — 배지의 박스 크기는 여기서 정하지 않는다.** 위 표의 `sm`/`md`/`lg` 는 **radius 토큰 이름**인데, `bot-avatar.tsx` 가 쓰는 같은 세 글자는 **박스 크기 이름**이라 뜻이 다르다(#308 의 배지는 28 · 32 · 36 · 44 · 56px 다섯 칸을 쓰고 radius 는 그와 **따로** 고른다). 이름이 겹치니 **이 절을 인용할 때는 px 로 적는다.**
...
| **M4** | 봇 아바타 blink | eye-mask scaleY 1→0.1→1 | 140ms / linear, 4–7s 랜덤 간격 | 사람 같은 디테일 — **보류·재검토**(**`[예정]`** · 인도 미배정): 이니셜 배지·모티프 글리프 **어느 쪽에도 눈이 없다**. 표 아래 주 |
...
> **M4 는 전제가 사라졌다 — 지우지 않고 보류한다**(**`[예정]`** · **인도 미배정**).
> M4 는 아바타에 **눈이 있다**는 전제 위에 서 있다(`eye-mask scaleY`). 그런데 [§ 14.1.1 예외 2](#1411-아이콘-일관성-예외-emoji-허용-영역) 가 세운 아바타 면은 **과목 이니셜 한 글자 배지**이고, 그 너머의 목표인 [`07 § 4.6.2`](07-branding.md) **모티프 글리프**(`√ x² ÷ 수식 도트` · `한글 자모 ㅎㄱㅁ` · `지도 라인`)에도 눈이 없다 — **가릴 눈이 어느 쪽에도 없다.**
> **방향도 반대다.** M4 의 근거인 「**사람 같은 디테일**」은 예외 2 를 좁힌 근거와 정면으로 어긋난다 — 거기서는 사람 emoji 가 「사람이 아니라는 AI 고지 옆에 **사람 얼굴**을 둔다」는 이유로 빠졌다. 봇을 사람처럼 보이게 하는 것을 아바타 면에서 걷어내면서 **눈 깜빡임만 남길 수는 없다.**
> 그래서 **재검토 전까지 구현하지 않는다.** 「살아있다」 신호를 **눈이 아닌 것으로** 어떻게 말할지가 재검토의 질문이고, 그 자리는 이미 **M3(idle 호흡)** 와 **M5(LIVE 뱃지 펄스)** 가 눈 없이 맡고 있다 — 둘 다 배지 모양과 무관해 이 전환의 영향을 받지 않는다. 인도할 PR 은 아직 없다.
