...
> **M4(봇 아바타 blink)는 폐기했다 — 번호는 비워 둔다.**
> 종전 표의 M4 는 `eye-mask scaleY 1→0.1→1` 로 **눈을 감겼다 뜨는** 모션이었고 효과란은 「사람 같은 디테일」이었다. `dev` 에서는 아바타가 **얼굴 emoji** 라 eye-mask 가 말이 됐는데, **그 전제를 없앤 것이 이 변경**이다 — [§ 14.1.1 예외 2](#1411-아이콘-일관성-예외-emoji-허용-영역) 가 세운 아바타 면은 **과목 이니셜 한 글자 배지**이고, 그 너머의 목표인 [`07 § 4.6.2`](07-branding.md) **모티프 글리프**(`√ x² ÷ 수식 도트` · `한글 자모 ㅎㄱㅁ` · `지도 라인`)에도 눈이 없다. **가릴 눈이 어느 쪽에도 없으니 보류가 아니라 폐기다** — 되살릴 전제가 돌아오지 않는다.
> 「**사람 같은 디테일**」은 방향도 반대다. 예외 2 를 좁힌 근거가 「사람이 아니라는 AI 고지 옆에 **사람 얼굴**을 둔다」였다 — 봇을 사람처럼 보이게 하는 것을 아바타 면에서 걷어내면서 **눈 깜빡임만 남길 수는 없다.**
> **번호는 당기지 않는다.** M5~M9 를 하나씩 올리면 바깥에서 번호로 참조하는 자리가 전부 어긋난다(§ 15.2.1 소제목 · § 15.3 · § 15.8 · [04 § 9](04-ux-flow.md) 여러 곳). **비는 번호가 흠이 아니라 기록**이다 — 위 제목의 「9종」은 **번호 축 M1~M9** 를 가리키고, M4 를 뺐으므로 **살아 있는 모션은 여덟**이다.
> 「살아있다」 신호는 **M3(idle 호흡)** 와 **M5(LIVE 뱃지 펄스)** 가 그대로 맡는다 — 둘 다 **형태에 무관한 모션**이라 이니셜 배지에도 그대로 얹힌다. 지우는 것은 M4 하나다.
