...
| 항목 | 제약 |
|------|------|
| 봇 이름 | **현행** 1~30자(필수) · **#223 이후** 비워도 됨(비우면 과목 기본 이름 — 예 「수학봇」), 적을 거면 2~30자. 상한은 입력칸에서도 막는다 |
| 과목 | 사전 정의 enum. **#223 이후 빌더에서 유일한 필수 항목** — 다른 항목은 전부 기본값이 들어가 있고, 만들기를 막는 것은 과목뿐이다 |
| 학년 | 고1~고3, 재수생 — ⚠️ **문서와 코드가 어긋나 있다.** 빌더 UI 는 초5~고1 을 노출한다. 클래스봇 주 사용자를 초5~고1 로 볼지는 이 표만의 문제가 아니라 제품 타깃 정의(`02-product-definition.md` 타깃 사용자 · `06-content-data.md` `Grade` 타입)와 함께 정해야 하므로, 이 PR 은 손대지 않고 **별도 합의 대상**으로 남긴다 |
...
> **#223 인도 중.** 위 표의 「#223 이후」 서술은 확정된 설계이며 `dev` 시점 코드는 아직 8단계 위저드 규칙(이름 필수 · 루브릭 100% 강제)을 따른다. 각 규칙은 이 문서가 새로 지어낸 것이 아니라 **#223 의 구현에서 읽은 것**이다 — 다만 그 코드 심볼은 아직 `dev` 에 없어 이 문서에서 인용하지 않는다(지금 확인할 수 없는 근거는 근거가 아니다). #223 이 머지되면 「현행」 표기를 지우고 그때 심볼을 적는다.
