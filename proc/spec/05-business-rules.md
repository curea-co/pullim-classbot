...
> **⚠️ 권위 문서보다 넓다 — 별도 승인 사안.** 새 축 둘(`self_study_summary` ·
> `class_assignment_summary`)은 **학부모 열람 범위를 리포트 밖으로 넓힌다.** base 권위는
> 아직 그만큼을 적지 않는다 — [`02 § 4.3`](02-product-definition.md) 은 학부모 권한을
> 「**자녀 리포트 열람만**, 상세 분석은 교사 통해서만」으로 못 박고,
> `input/docs-archive/07_풀림_클래스봇_핸드오프.md` 의 P3 도 「주간 리포트 수신」으로만
> 정의한다.
> **확장은 2026-09-04 사용자 결정**이고 이 절이 그 근거다. 다만 위 두 문서는 **이 PR 이
> 손대지 않았다** — `input/docs-archive/` 는 리포 `CLAUDE.md` 상 read-only 권위라
> **사용자 명시 승인**이 있어야 고칠 수 있고, `02 § 4.3` 도 같은 승인 자리에 함께 올려야
> 한 번에 맞는다. **그러니 지금은 어긋난 채 남는다**: 이 절을 읽는 사람은 새 축 둘이
> **권위 갱신 대기 상태**임을 알고 읽는다. 축 다섯(리포트 계열)에는 이 단서가 붙지 않는다 —
> 그쪽은 권위가 이미 적고 있는 범위다.
