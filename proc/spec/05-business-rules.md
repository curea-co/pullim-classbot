...
> **이 표는 지금 `dev` 에서 확인할 수 있는 제약만 둔다.** 봇 빌더 재편 제안(`03 § 4.3`)이 바꾸려는 규칙 —— 이름을 비워도 됨 · 과목이 유일한 필수 항목 · 빌더에서 루브릭과 시간대 스위치가 빠짐 —— 은 **구현이 들어올 때 이 표를 함께 고친다.** 검증 규칙 자리에 아직 확인할 수 없는 규칙을 미리 박으면 읽는 사람이 확정된 제약으로 오해한다. 설계는 `03 § 4.3` 이 갖는다.
