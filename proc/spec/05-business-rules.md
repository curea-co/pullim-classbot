...
| 항목 | 제약 |
|------|------|
| 봇 이름 | 비워도 됨(비우면 과목 기본 이름 — 예 「수학봇」) · 적으면 2자 이상 |
| 과목 | 사전 정의 enum |
| 학년 | 고1~고3, 재수생 |
| 교안 업로드 | PDF/이미지, 50MB 이하 |
| 답 범위(Scope) | L1~L5 중 하나, 기본 L3. 시간대별 자동 스위치는 없앰 (03 § 4.3) |
| 루브릭 | 빌더에서 뺐음 — 채점 기준은 과제 출제(`/teacher/assignment/new`)에서 정한다 (03 § 4.3) |
