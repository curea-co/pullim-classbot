...
| 항목 | 제약 |
|------|------|
| 봇 이름 | 비워도 됨(비우면 과목 기본 이름 — 예 「수학봇」), 적으면 2자 이상 — 근거: `components/builder/builder-types.ts` `botName()`(빈 값이면 `subjectMeta[subject].botName` 반환) · `isNameValid()`(`length === 0 \|\| length >= 2`) |
