...
2. **부모는 「미동의」와 「활동 없음」을 구별할 수 없다.** 구별되면 그 차이 자체가 정보가 되어
   ...
   **`[2026-09-10 정정]` 그 자녀 목록을 지는 라우트는 `GET /api/parent/children` 하나다.**
   종전 문장은 「가리지 않는다」의 **주체를 적지 않아**, 축별 라우트마다 목록을 다시 실어야
   한다는 뜻으로도 읽혔다. 그렇게 읽으면 안 되는 이유가 이 규칙 자체에 있다 — 축별 응답이
   미동의 자녀를 실으면 **그 자리의 빈 값이 곧 「동의 안 함」**이 되어, 규칙 2 의 본문이
   막으려는 누출을 규칙 2 의 단서가 만든다.

   그래서 둘을 갈라 적는다:

   | 무엇 | 지는 라우트 | 미동의 자녀는 |
   |---|---|---|
   | **자녀 목록**(이름·관계) | `GET /api/parent/children` | **실린다** — 가리지 않는다 |
   | **축별 내용**(반·과제 / 자기주도) | 그 축의 조회 | **애초에 읽지 않는다**(규칙 1) |

   반·과제 축은 목록과 **같은 라우트**에 살아서 「이름은 실리고 내용은 빈 배열」이 한 응답에
   나타난다(#280 · `livingConsentExists` 가 반·과제 질의의 `where` 안에 선다). 자기주도 축은
   **다른 라우트**(`GET /api/parent/children/self-study`)라 같은 규칙이 「미동의 자녀는 결과
   집합에서 빠진다」로 나타난다 — 목록을 가리는 것이 아니라 **그 라우트가 목록을 지지 않는
   것**이다. 부모가 「이어진 자녀가 없다」와 혼동할 자리도 생기지 않는다: 그 화면은 자녀 유무를
   `GET /api/parent/children` 에서 읽는다(같은 사실을 학부모 홈에서 이미 보고 있다).

   ⛔ **축별 라우트에 「이름만 실은 빈 칸」을 만들지 마라.** 그 칸은 `scope_label` ·
   `expires_at` 처럼 **동의 행에서만 오는 값**을 `null` 로 남기게 되는데, `null` 이 곧
   「동의 안 함」이라 규칙 2 가 그 자리에서 깨진다.
