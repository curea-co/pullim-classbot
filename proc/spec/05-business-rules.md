...
> **#223 인도 중.** 위 표의 「#223 이후」 서술은 확정된 설계이며 `dev` 시점 코드는 아직 8단계 위저드 규칙(이름 필수 · 루브릭 100% 강제)을 따른다. **각 「#223 이후」 규칙은 이 문서가 새로 지어낸 것이 아니라 #223 브랜치의 구현에서 읽은 것**이고, 근거 함수·파일을 행마다 적었다. #223 이 머지되면 「현행」 표기를 지운다.
