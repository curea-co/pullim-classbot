...
- **개발 전용 신원 폴백** — 위 해석기에 **한 겹을 더한 것**이고 **`dev` 에 있다**
  ...
  **인증이 아니다** — 서버가 이 쿠키에 주는 것은 `isAuthenticated` 가 아니라
  ...
  경계 셋을 **규칙으로 못 박는다**:
  ① **아는 호스트에서만 읽는다 — 모르면 닫는다**(`isDevIdentityHost`). 판정 순서가 규칙의
  일부다:
    - **배포 환경이 `production` 이면 무조건 닫는다.** 이름에 기대지 않는 방어선이라
      어떤 주소로 닿든·`Host` 를 무엇으로 위조하든 무력이다.
    - **고정 허용 목록**: `localhost` · `127.0.0.1` · `::1` · `dev-classbot.pullim.ai`.
    - **`*.vercel.app` 은 배포 환경이 `preview` 라고 확인됐을 때만 연다.** 이 접미사는
      **production 배포도 받으므로** 「이름을 열어 두고 production 검사로 거른다」는 순서면
      환경변수가 없을 때 production 기본 URL 이 그대로 통과한다. 순서를 뒤집어
      **positive 확인**으로 둔다.
    - **배포 환경을 모르면(둘 다 비어 있으면) 닫는다.** 환경은
      `VERCEL_ENV ?? NEXT_PUBLIC_VERCEL_ENV` 로 읽고 **서버 전용 값이 먼저다** — 권한 판정의
      근거는 빌드 때 치환되지 않고 서버가 런타임에 직접 읽는 값이어야 한다. 열리는 쪽으로
      접히게 두면 그 변수가 빠지는 순간 구멍이 조용히 되살아난다.
    ...
    *(`[2026-09-14 정정]` 종전 이 자리는 「prod 호스트(`classbot.pullim.ai`)에서는 읽지
    ...
    좁혔다. prod 데이터를 쓰는 배포는 그 이름으로만 닿는 것이 아니라서다. 명세가 거부
    목록으로 남아 있으면 그 좁힘을 되돌리는 변경이 이 절을 근거로 통과한다. **환경 확인과
    환경 미확인 시 fail-closed 까지 적는 이유도 같다** — 접미사만 적어 두면 「`.vercel.app`
    이면 연다」로 읽혀 production 기본 URL 이 다시 열린다. 리뷰 지적 · 라운드 1.)*
...
- `/parent/*` — Assistant 권한 + 자녀 매칭(`parent_child_links`) 검증, 그리고
  ...
  **#280**(반·과제 축 게이트) 과 **#288**(자기주도 축 · 학생이 켜는 문)이다.
  *(`[2026-09-14 정정]` 종전 이 자리는 「**`[예정]`** 은 `/parent/self-study` 하나이고 —
  **#271 을 닫고** 서버는 **#288**, 화면은 그 뒤 별건 PR 이 진다」로 적혀 있었다.
  그 별건 PR 이 **#291** 이고 머지됐다.)* 들어온 뒤에도
  **실제 로그인 학부모는 진입 불가**다([03 § 2.3](03-features-and-ia.md))
...
> **이 절은 규칙을 먼저 세웠고 — 구현이 전부 도착했다.**
>
> **`[2026-09-14 인도 현황]`** 이 절이 세운 것은 **`dev` 에 다 있다**:
> `consent_logs` 표(`type` 값 **일곱** — 다섯은 `0000`~`0003`, 축 둘은 **#280** 이 TS 로만
> 더했다 · 규칙 4) · `revoked_at`(**#280** · `0005_breezy_anita_blake`) ·
> `parent_child_links` · `GET /api/parent/children`(**#267** 이 내고 **#280** 이 반·과제 축의
> 동의 게이트를 얹었다) · `app/(parent)` 트리(**#281** + **#291**) ·
> `GET /api/parent/children/self-study` 와 `app/api/me/consents/*`(**#288**) ·
> 화면 `/parent/self-study` 와 `/classbot/me/share`(**#291**).
>
> *(`[2026-09-14 정정]` 종전 이 자리는 「아직 **`[예정]`** 인 것은 자기주도 축 하나다 — 서버는
> **#288**, 화면은 그 뒤 별건 PR」로 적고 있었다. 그 둘이 차례로 머지되며 낡았다. **#271 이
> 서버·화면을 한꺼번에 지고 있었으나 닫혔고**, 서버는 **#288** · 화면은 **#291** 로 갈려
> 들어왔다 — 서버와 화면을 한 PR 에 담지 않는 것이 리포 최상위 규칙이다.)*
>
> 그러니 이 절은 이제 **전부 현재 동작 설명**이다. 여기 적힌 규칙을 되돌리는 변경은
> 「아직 안 들어온 것」이 아니라 **회귀**로 읽는다.
>
...
다섯은 `0000`~`0003` 부터 있었고 **#280** 이 둘을 더해 일곱이 됐다.
**더한 값은 하나가 아니라 둘이다.**)* — § 11.3 「자녀 승인 후
...
이 절이 정하는 것은 둘이다 — ① **축이 둘 늘어난다**(자기주도 · 반·과제) ② 그 둘은
**학생 동의 말고 다른 관문이 없다**. 앞의 넷에는 학생 동의 **위에** 교사·기관의 발송 승인이
한 겹 더 있는데(13 § 4.6 「학부모 발송 승인」 · [13 § 5.3·5.4](13-reports-and-emotion-checkin.md)
`pending-approval → approved → sent`), 자기주도와 반·과제에는 그 승인을 할 교사가 구조적으로
없다. 그래서 **두 관문을 갈라 적는다** — 아래 표의 두 칸은 **and** 다(둘 다 통과해야 나간다),
or 가 아니다.

| 축 (`consent_logs.type`) | 무엇이 나가나 | 학생 동의 (`consent_logs`) | 그 위의 관문 |
|---|---|---|---|
| `weekly_report` · `monthly_report` | 주간·월간 리포트 | **필수** | 교사·기관 **발송 승인** ([13 § 5.3·5.4](13-reports-and-emotion-checkin.md)) |
| `weak_nodes` | 약점 단원 | **필수** | 교사·기관 (리포트에 실려 나간다) |
| `emotion_share` | 감정 평균 (민감) | **필수** · **다른 동의에 딸려 나가지 않는다** | 교사·기관 |
| `realtime_alert` | 학습 시작·완료 알림 | **필수** | 교사·기관 |
| `self_study_summary` *(#280)* | 스스로 담은 봇 · 공부한 날 · 연속일수 | **필수** | **없다** — 자기주도에는 승인할 교사가 구조적으로 없다 |
| `class_assignment_summary` *(#280)* | 참여한 반 · 받은 과제 현황 (답안·점수 제외) | **필수** | **없다** — 같은 이유 |

앞 네 줄에 담긴 **값 다섯**(`weekly_report` · `monthly_report` · `weak_nodes` · `emotion_share` · `realtime_alert`)에 **값 둘**(`self_study_summary` · `class_assignment_summary`)을 **#280** 이 더해 **일곱이 됐다** — 일곱 다 `dev` 의 `consent_logs.type` 에 있다(`lib/db/schema.ts` union · `lib/mock/family.ts` 의 `ConsentType`·`consentTypeMeta`). *(`[2026-09-14 정정]` 종전 뒤 두 줄과 이 문단은 그 둘을 **`[예정]`** 으로 적고 「#280 이 더한다」로 두었다 — 그 PR 이 머지되며 낡았다.)*
...
교사에서 학생으로도 옮기지 않았다. 그 시점의 `dev` 에는 학부모 화면도 `app/api/parent/*` 도
없었으니 이 결정은 **`dev` 의 동작을 고치는 것이 아니라 스택 PR 안에서 갈린 것**이었다 —
당시 스택의 **#268** 이 낼 학부모 과제 화면이 자녀 링크만으로 열렸고, 같은 스택의 자기주도
칸은 동의 뒤에 두기로 되어 있어 **한 화면 안에 인가 모델이 둘**이 될 참이었다. 그 둘을
§ 11.3 쪽으로 맞추는 것이 이 결정이다. *(`[2026-09-14]` **#268 은 닫혔다** — 그 학부모 과제
화면을 실제로 인도한 것은 **#281** 이고, 그 화면은 이 결정이 정한 게이트 뒤에서 열린다.
이 문단은 결정 당시의 정황 기록이라 그대로 두고 인도자만 여기 적는다.)*

**`[2026-09-14 인도됨]`** 반·과제 쪽은 **#280** 이 인도했다 — `GET /api/parent/children` 이
`class_assignment_summary` 의 살아 있는 동의를 **질의 조건 안에서** 본다
(`app/api/_lib/parent-views.ts` → `livingConsentExists`). 자기주도 쪽은 **#288** 이 인도했다 —
`GET /api/parent/children/self-study` 와, 그 축을 학생이 **켤 수 있게 하는 문**
(`app/api/me/consents/*`)을 같은 PR 이 함께 냈다. 게이트만 있고 켜는 문이 없으면 #280 의
그 게이트는 영원히 닫힌 문이었을 것이다. 켜는 화면은 **#291**(`/classbot/me/share`)이다.
*(`[2026-09-14 정정]` 종전 이 문단은 자기주도 쪽을 「**#288**(서버)이 지고 … 같은 PR 이
낸다」는 미래형으로 적고 있었다 — 그 PR 이 머지되며 낡았다.)*

> **권위 문서도 같은 변경세트로 갱신했다 (사용자 명시 승인 · 2026-09-04).** 새 축
> 둘(`self_study_summary` · `class_assignment_summary`)은 학부모가 보는 것을 리포트 밖으로
> 넓히므로, 그것을 정의하는 두 자리를 **이 PR 안에서 함께** 고쳤다 —
> `input/docs-archive/07_풀림_클래스봇_핸드오프.md` **§ 1.3 P3**(v1.2 → v1.3) 와
> [`02 § 4.3`](02-product-definition.md). 승인 범위는 **그 둘뿐**이고 `input/docs-archive/`
> 의 다른 파일·절은 건드리지 않았다([00-index](00-index.md) 변경 이력에 범위를 적었다).
>
> **넓힌 이유** — 자기주도에는 **승인할 교사가 구조적으로 없다.** 그래서 그 축의 승인 주체가
> 학생 본인이 되고, 같은 이유로 반·과제도 같은 게이트 뒤로 옮긴다(사용자 결정).
>
> **읽는 법 — 권한이 넓어진 것이 아니라 게이트가 생긴 것이다.** 반·과제 현황은 종전에
> **자녀 링크만 있으면** 열려 있었고, 이 개정으로 **학생이 켜야만** 열린다. 자기주도 요약도
> 새로 여는 것이 아니라 처음부터 동의 뒤에 둔다. 「학부모가 더 많이 본다」로 읽으면 사실과
> 반대다 — 켠 것이 없으면 **아무것도 못 본다.**

**규칙 넷** — `app/api/parent/*` · `app/api/me/consents/*` 가 지켜야 할 기준이다.

> **`[2026-09-14 인도 현황]`** 이 규칙 넷이 겨냥한 라우트는 **전부 `dev` 에 있다**:
> `GET /api/parent/children`(#267 이 내고 **#280** 이 `class_assignment_summary` 동의 게이트를
> 얹었다) · `consent_logs.revoked_at`(**#280**) · 축 둘(**#280**) ·
> `GET /api/parent/children/self-study` 와 `app/api/me/consents/*`(**#288**).
> 화면은 **#291**(`/parent/self-study` · `/classbot/me/share`).
> *(`[2026-09-14 정정]` 종전 이 박스는 뒤 둘을 **`[예정]`** 으로 두고 「#271 을 닫고 계층을
> 갈라 #288 이 인도하고, 화면은 그 뒤 별건 PR 이 진다」로 적었다 — 그 계층 분리는 실제로
> 그렇게 이뤄졌고 둘 다 머지됐다. 리포 최상위 규칙(FE/BE 를 한 PR 에 담지 않는다)은 그대로다.)*
>
> 그러니 아래 넷은 **통과 기준이자 현재 동작**이다. 새 학부모 라우트가 생기면 같은 넷을
> 그대로 진다 — 규칙 2 가 「주체는 모든 학부모 라우트」라고 못 박은 그대로다.
...
2. **부모는 「미동의」와 「활동 없음」을 구별할 수 없다.** 구별되면 그 차이 자체가 정보가 되어
   ...
   **`[2026-09-10 재정정]` 축별 라우트도 연결 자녀 전원을 싣는다** — 게이트되는 것은
   **내용**이고, 빠져야 할 것은 **동의 정보**다.

   이 자리는 같은 날 두 번 고쳤다. 첫 판(**#289**, 머지됨)은 「자기주도 축은 미동의 자녀를
   결과 집합에서 뺀다」로 적었는데 **그것이 틀렸다.** 그 판은 아래 ① 만 보고, ① 을 그렇게
   막으면 열리는 ② 를 못 봤다. 되돌린다.

   **오라클은 둘이고, 서로 다른 것을 고쳐야 막힌다:**

   ① **`null` 오라클** — 미동의 자녀를 실으면서 `scope_label` · `expires_at` 처럼
      **동의 행에서만 오는 값**을 함께 싣는 경우. 미동의 자녀는 그 둘이 `null` 이라
      `scopeLabel === null` 이 곧 「동의 안 함」이 된다.
      → **막는 법: 그 값을 학부모 응답에서 뺀다.** 자녀를 빼는 것이 **아니다** —
      오라클의 출처는 **동의 정보**이지 자녀 행이 아니다.

   ② **대조 오라클** — 미동의 자녀를 결과 집합에서 빼는 경우. 학부모가
      `GET /api/parent/children`(연결 자녀 **전원**)과 축별 응답의 `children` 을
      **대조하면 빠진 자녀가 곧 미동의 자녀다.** 활동이 0인 자녀는 목록에 남고 미동의
      자녀는 사라지므로, **규칙 2 본문이 막으려는 그 구별이 그대로 성립한다.**
      → **막는 법: 전원을 싣는다.**

   ① 을 「자녀 제거」로 막으면 ② 가 열린다. 둘을 동시에 막는 모양은 하나뿐이다:

   | 무엇 | 어떻게 |
   |---|---|
   | **자녀 목록**(이름·관계) | **연결 자녀 전원**을 싣는다 — **축별 라우트도 그렇다** |
   | **축별 내용**(반·과제 · 담은 봇 · 공부한 날) | 동의 술어를 **조회 조건 안**에 넣는다(규칙 1). 미동의면 0행 → **빈 배열** |
   | **동의 정보**(범위 라벨 · 만료) | **학부모 응답에 싣지 않는다** — ① 의 실제 출처다 |

   그러면 **미동의 자녀와 「동의했지만 활동 0」 자녀의 응답이 완전히 같은 모양**이 된다 —
   이름·관계만 다르고 나머지는 빈 배열로 동일하다. 부모가 대조할 것도, 읽어 낼 `null` 도 없다.

   **이것은 `dev` 에 이미 있는 모양이다** — **#280** 의 `GET /api/parent/children` 이 정확히
   이 구조다(전원 + 게이트된 읽기 + 빈 배열, 동의 정보는 응답에 없다). 그래서 규칙이
   라우트마다 다르게 나타날 이유가 없어진다 — **축이 몇 개든 같은 모양이다.**

   **그러니 「자녀 목록은 가리지 않는다」의 주체는 `모든 학부모 라우트`다.** #289 가 그것을
   `GET /api/parent/children` 하나로 좁혀 적은 것과, 「축별 라우트는 목록을 지지 않는다」는
   서술은 **함께 걷는다.**

   ⛔ **공유 범위·만료를 학부모 화면에 되살리지 마라.** 그 정보가 필요한 곳은 **학생 자기
   화면**(`/classbot/me/share`)이다 — 자기가 무엇을 언제까지 주고 있는지는 학생이 알아야
   하고, 끄는 것도 학생이 한다. 학부모 쪽에 싣는 순간 ① 이 그대로 돌아온다.
3. **철회는 행 삭제도 `expires_at = now()` 도 아니다** — `revoked_at` 을 따로 둔다(감사 기록).
   **`[2026-09-10 인도됨]`** 그 칼럼은 **#280** 이 넣어 `dev` 에 있다
   (`drizzle/0005_breezy_anita_blake.sql`). 종전 「**`[예정]`** · `dev` 의 `consent_logs` 에는
...
4. **부모의 열람은 학생에게 되돌아가지 않는다** — 부모가 언제·몇 번 봤는지 **학생 화면에
   비추지 않는다.** 학생이 「내가 켜 놨으니 보면 알겠지」로 기대하기 쉬운 자리라, 그렇지
   않다는 것을 화면이 그대로 말한다.
   **서버 쪽 열람 계측까지 금지하는 규칙이 아니다** — 발송 대비 열람률은 이 제품의 KPI 이고
   (`07_풀림_클래스봇_핸드오프.md` § KPI 「리포트 열람률 80%」 · [13 § 2 G4](13-reports-and-emotion-checkin.md)),
   [13 § 4.5](13-reports-and-emotion-checkin.md) 의 학부모 플로우도 웹 링크 상세 열람을
   전제한다. 그 집계를 어떤 표로 어떻게 둘지는 **이 절이 정하지 않는다**(리포트 스펙 소관).
   이 절이 정하는 것은 한 가지다 — **그 집계가 학생 화면으로 흘러나오지 않는다.**
