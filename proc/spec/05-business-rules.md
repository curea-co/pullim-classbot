...
| 항목 | 제약 |
|------|------|
| 봇 이름 | **현행** 1~30자(필수) · **#223 이후** 비워도 됨(비우면 과목 기본 이름 — 예 「수학봇」), 적으면 2자 이상 |
| 과목 | 사전 정의 enum. **#223 이후 빌더에서 유일한 필수 항목** |
| 학년 | 고1~고3, 재수생 |
...
> **#223 인도 중.** 위 표의 「#223 이후」 열은 확정된 설계이며 `dev` 시점 코드는 아직 8단계 위저드 규칙(이름 필수 · 루브릭 100% 강제)을 따른다. #223 이 머지되면 「현행」 표기를 지운다.
