  ...
  **인증이 아니다** — 서버가 이 쿠키에 주는 것은 `isAuthenticated` 가 아니라
  ...
  일부다:
    ...
    *(`[2026-09-14 정정]` 종전 이 자리는 「prod 호스트(`classbot.pullim.ai`)에서는 읽지
    ...
    목록으로 남아 있으면 그 좁힘을 되돌리는 변경이 이 절을 근거로 통과한다.
    **`*.vercel.app` 의 preview 확인까지 적는 이유도 같다** — 접미사만 적어 두면
    「`.vercel.app` 이면 연다」로 읽혀 production 기본 URL 이 다시 열린다. 리뷰 지적 ·
    라운드 1·2. **라운드 2 에서 fail-closed 의 범위를 좁혔다** — 처음엔 「배포 환경을 모르면
    닫는다」를 경계 전체의 규칙으로 적었는데, 구현은 고정 허용 목록을 환경 확인 없이
    통과시킨다(`isDevIdentityHost` · `lib/__tests__/dev-identity.test.ts` 가 환경 미설정에서
    그 넷을 `true` 로 검증한다). 그대로 두면 **로컬 개발의 정상 동작을 금지하는 규칙**이 된다.)*
