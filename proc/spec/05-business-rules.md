...
- **개발 전용 신원 폴백** — 위 해석기에 **한 겹을 더한 것**이고 **`dev` 에 있다**
  ...
  **인증이 아니다** — 서버가 이 쿠키에 주는 것은 `isAuthenticated` 가 아니라
  ...
  경계 셋을 **규칙으로 못 박는다**: ① **아는 호스트에서만 읽는다 — 모르면 닫는다**
  (`isDevIdentityHost` 의 허용 목록: `localhost` · `127.0.0.1` · `::1` ·
  `dev-classbot.pullim.ai` · PR preview 접미사. *(`[2026-09-14 정정]` 종전 이 자리는
  「prod 호스트(`classbot.pullim.ai`)에서는 읽지 않는다」는 **거부 목록**으로 적혀 있었다 —
  **#277** 이 그것을 허용 목록 + fail-closed 로 좁혔다. prod 데이터를 쓰는 배포는 그 이름으로만
  닿는 것이 아니라서다. 명세가 거부 목록으로 남아 있으면 그 좁힘을 되돌리는 변경이 이 절을
  근거로 통과한다.)*
