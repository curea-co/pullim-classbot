...
- `/parent/*` — Assistant 권한 + 자녀 매칭(`parent_child_links`) 검증, 그리고
  **자녀 동의**(§ 11.4). 종전 줄의 「Assistant 권한 + 자녀 매칭」은 그대로 두고 동의 한 겹을
  더한 것이다. **`[2026-09-10 인도 현황]`** `app/(parent)` 트리는 **#281** 이 넣어
  `dev` 에 있다(홈 · 자녀 과제). **`[예정]`** 은 `/parent/self-study` 하나이고 —
  **#271 을 닫고** 서버는 **#288**, 화면은 그 뒤 별건 PR 이 진다. 들어온 뒤에도
...
> **이 절은 규칙을 먼저 세웠고 — 구현이 대부분 도착했다.**
>
> **`[2026-09-10 인도 현황]`** 이 절이 세운 것 중 `dev` 에 **있는 것**:
...
> 동의 게이트를 얹었다) · `app/(parent)` 트리(**#281**).
>
> 아직 **`[예정]`** 인 것은 자기주도 축 하나다 —
> `GET /api/parent/children/self-study` · `app/api/me/consents/*` 는 **#288**(서버),
> 화면(`/parent/self-study` · `/classbot/me/share`)은 그 뒤 별건 PR.
> **#271 이 그 둘을 한꺼번에 지고 있었으나 닫혔다** — 서버와 화면을 한 PR 에 담지 않는 것이
> 리포 최상위 규칙이고, 그 head 가 시리즈 전체를 지고 있어 base 를 옮길 수도 없었다.
>
> 그러니 이 절은 **`[예정]` 줄에 대해서는 통과 기준**이고, 그 밖에 대해서는 **현재 동작
> 설명**이다. 두 성격이 섞여 있으므로 인도 표기를 함께 읽을 것.
>
...
#268 이 낸 학부모 과제 화면이 자녀 링크만으로 열렸고, 같은 스택의 자기주도 칸은 동의 뒤에
두기로 되어 있어 **한 화면 안에 인가 모델이 둘**이 될 참이었다. 그 둘을 § 11.3 쪽으로 맞추는
것이 이 결정이다.

**`[2026-09-10 인도됨]`** 반·과제 쪽은 **#280** 이 인도했다 — `GET /api/parent/children` 이
`class_assignment_summary` 의 살아 있는 동의를 **질의 조건 안에서** 본다
(`app/api/_lib/parent-views.ts` → `livingConsentExists`). 자기주도 쪽은 **#288**(서버)이 지고,
그 축을 학생이 **켤 수 있게 하는 문**(`app/api/me/consents/*`)도 같은 PR 이 낸다 —
게이트만 있고 켜는 문이 없으면 #280 의 그 게이트는 영원히 닫힌 문이다.
...
> **`[2026-09-10 인도 현황]`** `dev` 에 **있는 것**: `GET /api/parent/children`(#267 이 내고
> **#280** 이 `class_assignment_summary` 동의 게이트를 얹었다) · `consent_logs.revoked_at` ·
> 축 둘. **`[예정]`**: `GET /api/parent/children/self-study` 와 `app/api/me/consents/*` —
> **#271 을 닫고 계층을 갈라 #288**(서버)이 인도하고, 화면(`/parent/self-study` ·
> `/classbot/me/share`)은 그 뒤 별건 PR 이 진다(리포 최상위 규칙: FE/BE 를 한 PR 에 담지 않는다).
