...
## 6.3 마이그레이션 번호는 명세가 예약하지 않는다 (2026-09-09)

> **왜 이 절이 생겼나** — 종전 명세는 어느 PR 이 어느 번호(`0005`·`0006`·`0007`)를 쓸지
> **미리 못 박아** 두었다. 그 예약은 스택 PR 이 계획대로 순서대로 들어온다는 전제 위에 서
> 있었는데, 실제로는 스택의 base 가 squash 머지로 사라져 순서가 갈렸다. 그러자 리뷰가
> **낡은 예약표를 권위 삼아** 「예약된 `0007` 로 옮겨라」고 요구했고, 그대로 옮기면 아래
> § 실측 이 보여주듯 **테이블이 영영 안 생기는** 상태가 된다. 그래서 번호를 다른 숫자로
> 갈아 끼우는 대신 **규칙을 적는다.**
...
1. **번호는 `dev` 에 먼저 도착한 PR 이 가져간다.** 명세는 어느 PR 이 어떤 **DDL** 을 지는지
   ...
   그 자리는 머지 순서가 정한다. 예약을 적어 두면 순서가 바뀌는 순간 명세가 틀린다.
2. **스택 PR 은 리베이스할 때 재번호한다 — 파일명·저널 `idx` 뿐 아니라 `when` 도 함께 올린다.**
   drizzle 은 **`when`(저널의 created_at) 으로만** 적용 여부를 가른다. 번호만 밀고 `when` 을
   두면, 이미 뒤 번호를 적용한 DB 가 앞 번호를 **경고도 실패도 없이 조용히 건너뛴다.**
3. **`consent_logs.type` 같은 「값만 느는」 변경에는 마이그레이션이 없다.** 실제 DB 의 `type`
   은 CHECK 도 PG enum 도 없는 `text` 다(`drizzle/0000_stiff_ulik.sql` · 실측:
   `information_schema.columns` 가 `text`, `pg_constraint` 에 PK·FK 셋뿐). 값을 늘리는 것은
   `lib/db/schema.ts` 의 union 과 `lib/mock/family.ts` 의 `ConsentType`·`consentTypeMeta` 를
   함께 고치는 **TS 작업**이지 DDL 이 아니다. 명세가 「enum 에 값을 넣는 마이그레이션」으로
   적으면 없는 작업을 요구하게 된다.

### 근거 — drizzle 의 적용 판정 (소스 인용)
...
**파일명도 `idx` 도 보지 않는다.** 「DB 에 적용된 것 중 `created_at` 이 가장 큰 값」보다
`when` 이 **큰** 항목만 돈다 — 작으면 건너뛰고, 건너뛴 사실을 아무 데도 남기지 않는다
(CLI 는 그대로 `migrations applied successfully!` 를 찍고 종료 코드 0 이다).

### 실측 (2026-09-09 · 일회용 DB `mig_probe_a/b/c` · 실행 후 삭제)

`dev`(`0000`~`0004`) 위에서 세 순서를 실제로 돌렸다. `[280]`=`revoked_at`(`when=1788843161576`),
`[SE]`=`self_enrollments`(`1788412277498`), `[SD]`=`self_study_days`(`1788414652159`).
...
머지 충돌처럼 눈에 띄지 않는다 — 개발자의 DB 에서 표 둘이 **조용히 없는 채로** 남는다.

### 지금 `dev` 기준 배정 — 예약이 아니라 **스냅샷**이다

아래는 2026-09-09 시점의 사실이지 앞으로의 약속이 아니다. 순서가 바뀌면 번호도 바뀌고,
**그때 고칠 것은 PR 이 아니라 이 문단이다.**
