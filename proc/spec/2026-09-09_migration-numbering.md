...
### 4.1 `[2026-09-10]` 현재 — 규칙 1 이 실제로 그렇게 굴러갔다
...
**자기주도 축의 남은 PR(#288 — `/api/parent/children/self-study` ·
`app/api/me/consents/*`)은 마이그레이션이 없다.** `revoked_at` 은 `0005` 가 이미 만들고,
축 둘은 규칙 4 대로 DDL 이 아니다. 스택 시절 그 브랜치가 들고 있던
`0007_silent_nick_fury.sql`(같은 `ADD COLUMN`)은 걷어냈다 — `dev` 의 `0007` 은 이미
`0007_boring_wasp` 이고, 그 파일이 스스로 적어 둔 존재 이유(「`meta/0007_snapshot.json`
이 `revoked_at` 을 가진 유일한 체인 머리」)도 `dev` 의 그 스냅샷이 그 열을 갖게 되면서
사라졌다. 일회용 DB 로 확인했다 — `0000`부터 8개 적용, `drizzle-kit generate` 가
「No schema changes」.
