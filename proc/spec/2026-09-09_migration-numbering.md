...
아래는 **한 시점의 사실**이지 앞으로의 약속이 아니다. 순서가 바뀌면 번호도 바뀌고,
**그때 고칠 것은 PR 이 아니라 이 절이다.** 그 말대로 여기서 한 번 고친다.

### 4.1 `[2026-09-14]` 현재 — 규칙 1 이 실제로 그렇게 굴러갔다

- `dev` 의 마이그레이션은 **`0000`~`0007`**. 다음 빈자리는 **`0008`** 이다.

| 번호 | 무엇 | 인도 |
|---|---|---|
| `0005_breezy_anita_blake` | `consent_logs.revoked_at` | **#280** |
| `0006_lethal_omega_red` | `self_enrollments` | **#270** |
| `0007_boring_wasp` | `self_study_days` | **#270** |

**#280 이 먼저 도착해 `0005` 를 가져갔고**, #270 은 리베이스에서 `0006`·`0007` 로 밀며
`when` 을 함께 올렸다 — § 3.1 의 **C** 그대로다. 규칙 1·2 가 예측한 순서가 실제로 나왔다.

`consent_logs` 의 축 둘(`self_study_summary` · `class_assignment_summary`)도 #280 이
세웠고, 규칙 4 대로 **DDL 은 없다**(`lib/db/schema.ts` union · `lib/mock/family.ts` 의
`ConsentType`·`consentTypeMeta` 셋만 고쳤다).

**셋 다 이제 `dev` 에 있으므로 규칙 3 의 동결 대상이다** — 파일명도 `when` 도 바꾸지 않는다.

**`[2026-09-14]` 스택은 다 들어왔고 번호는 `0007` 에서 멈췄다.** 종전 이 자리는
「자기주도 축의 **남은 PR**(#288)은 마이그레이션이 없다」로 적혀 있었다 — **#288 은 머지됐고,
정말로 마이그레이션 없이 들어왔다.** 뒤이은 **#291**(화면)도 `app/`·`components/`·`hooks/`
만 만지고 `drizzle/**` 은 0줄이다.
그래서 위 표 셋이 `dev` 의 마지막 마이그레이션이고 **다음 빈자리는 여전히 `0008`** 이다.
근거는 그대로다 — `revoked_at` 은 `0005` 가 이미 만들었고, `consent_logs` 의 축 둘은 규칙 4
대로 DDL 이 아니다. 스택 시절 #271 브랜치가 들고 있던 `0007_silent_nick_fury.sql`(같은
`ADD COLUMN`)은 걷어냈다 — `dev` 의 `0007` 은 이미 `0007_boring_wasp` 이고, 그 파일이 스스로
적어 둔 존재 이유(「`meta/0007_snapshot.json` 이 `revoked_at` 을 가진 유일한 체인 머리」)도
`dev` 의 그 스냅샷이 그 열을 갖게 되면서 사라졌다. 일회용 DB 로 확인했다 — `0000`부터 8개
적용, `drizzle-kit generate` 가 「No schema changes」.

### 4.2 `[2026-09-09]` 당시 — 이 절이 처음 적은 예측 (이력)

- `dev` 의 마이그레이션은 **`0000`~`0004`**. 다음 빈자리는 `0005` 다.
- **#280**(학부모 동의 게이트 · 서버)은 `dev` 를 base 로 열려 있고, 지금 머지되면 그 빈자리
  **`0005`** 를 가져간다(`revoked_at`).
...
**§ 4.2 를 남겨 두는 이유**: 규칙 1(「번호는 `dev` 에 먼저 도착한 PR 이 가져간다」)이
**예측으로서 맞았다**는 기록이다. 구현 지시로 읽지 말고 § 4.1 을 현재로 읽는다.
