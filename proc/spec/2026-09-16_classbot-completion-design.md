...
> **진행**: 2026-09-17 04:00 KST 실측 — § 8 표 「상태」 열. 이 리포 PR 0·4-ci·4·5a·6 머지(#349 · #348 · #350 · #351 · #352) · pullim-api PR 0~3 브랜치 넷 로컬 준비(푸시·PR 은 소유자 게이트) · PR 5 는 5a·5b·5c 로 갈랐다 · e2e 트랙 행 신설. *(`[2026-09-17 밤 정정]` — pullim-api PR 0~3 은 #667·#668·#669·#670 으로 그 리포 `dev` 에 들어갔고 dev RDS 마이그레이션은 21:35 KST 에 적용됐다(소유자 보고 · 이 리포에서는 실측 불가) · 5b 는 #355 인도분으로 좁히고 **5d `[예정]`** 을 열었다(api PR 4 = #672 뒤) · PR 7 = #354 머지 · 5b = #355 는 `dev` 위로 rebase 필요 · api PR 4 = pullim-api #672(열림) · ADR 번호는 **ADR-092**(§ 0 머리의 정정))*
...
**BE 정본은 pullim-api `src/classbot`** — ADR-063(2026-07-04) 이 정한 자리 그대로다. 봇 분리(③)는 그 리포의 **ADR-092**(`docs/design/_platform/adr/ADR-092-classbot-bots-분리.yaml` · refines ADR-063 — pullim-api #667 `f6e546a0` 로 그 리포 `dev` 에 머지, 2026-09-17 19:25 KST)이 진다 *(정정 2026-09-17 — 이 문서는 이 ADR 을 「ADR-091 `[예정]` · PR 승인 대기」로 적어 왔다. pullim-api `dev` 에 이미 다른 ADR-091(cloudwatch 오류 다이제스트 lambda)이 있어 머지 시 092 로 번호가 붙었다; 이 문서의 ADR-091 은 전부 092 로 바꿨다)* — 이 문서와 병행 작성됐고, 오류 관례(§ 5)·`classes.subject/grade`(§ 4)·표시명 포트(해소 5)는 그 ADR 을 따른다. 이 리포의 `app/api/**` + Drizzle(세계 B)은 지난 두 달 화면이 실제로 부른 곳이지만 **prod 에 올릴 수 없다** — 취향이 아니라 열쇠다(§ 1.1). 나머지 여섯은 추천안을 그대로 채택했다.

| # | 결정 | 그래서 하는 일 |
|---|---|---|
| ① | 이 리포의 `/api/*` 25개 · Drizzle 31표는 **겹치는 것만** 은퇴 — 반·봇·챗·과제 계열의 라우트와, 그 라우트만 쓰던 표. **표에서 실제로 DROP 되는 것은 넷**(`chat_messages` · `emotion_checkins` · `grading_history` · `wellbeing_snapshots` — `0009` SQL 의 순서 그대로)이다 *(정정 2026-09-17 — 종전 이 칸은 아홉(`classrooms` · `class_bots` · `enrollments` · `join_codes` · `assignments` · `assignment_questions` · `submissions` · `chat_messages` · `interventions`)을 은퇴 목록으로 적었다. PR 8 = **#358** 의 마이그레이션 `apps/classbot/drizzle/0009_retire_world_b_tables.sql` 은 위 넷만 `DROP TABLE … CASCADE` 한다.)*<br><br>**나머지 여덟이 남는 이유 — 표마다 다르다.** 이 목록은 **5d(마켓 축 은퇴)가 다음에 무엇을 DROP 할지 고를 때 읽는 자리**라, 「시드만 넣는다」로 뭉뚱그리면 살아 있는 라우트가 깨진다. #358 브랜치에서 그 표를 `@/lib/db/schema` 로 들여오는 자리를 전부 센 결과다 *(정정 2026-09-17 — 종전 이 칸은 `assignments` 를 「시드가 아직 넣고」 쪽에 세웠다. **틀렸다 — 살아 있는 학부모 라우트가 읽는다.**)*:<br>· **`assignments`** — **`GET /api/parent/children` 이 읽는다**: `app/api/parent/children/route.ts:41`(`listConsentedChildAssignments`) → `app/api/_lib/parent-views.ts:83`(`listVisibleAssignments`) → `app/api/_lib/student-views.ts:105` `.from(assignments)`. `app/api/_lib/assignment-visibility.ts:43` 도 들여온다. **자녀 과제 요약이 이 표 위에 서 있다 — 시드를 고쳐도 DROP 하면 안 된다.**<br>· **`classrooms`** — `_lib/classroom-pair.ts:15` · `_lib/student-views.ts:37`(→ 학부모 반 요약) · `/api/teacher/classrooms/route.ts:16` · `…/[id]/join-codes/route.ts:16`<br>· **`class_bots`** — `_lib/student-views.ts:37` · `/api/marketplace/bots/route.ts:19` · `…/[botId]/route.ts:15` · `/api/me/self-bots/route.ts:22` · `/api/parent/children/self-study/route.ts:72` · `/api/teacher/bots/route.ts:18` · `…/[botId]/publish/route.ts:17` · `/api/teacher/classrooms/route.ts:16` · `…/[id]/join-codes/route.ts:16` — **마켓 축의 척추다**<br>· **`enrollments`** — `_lib/assignment-visibility.ts:43` · `_lib/classroom-pair.ts:15` · `_lib/student-views.ts:37` · `/api/marketplace/bots/*` · `/api/teacher/classrooms/route.ts:16` · `…/[id]/students/route.ts:13`<br>· **`join_codes`** — `_lib/classroom-pair.ts:15` · `lib/join-code.ts:21` · `/api/teacher/classrooms/[id]/join-codes/route.ts:16`<br>· **`assignment_questions`** — 스키마에서 들여오는 파일이 `scripts/seed.ts` 하나뿐. **시드만이다**<br>· **`submissions`·`interventions`** — 어느 파일도 스키마에서 들여오지 않는다. 표와 `seed.ts` 의 truncate 목록(`ALL_TABLES`)에만 남는다<br><br>**5d 때 함께 가는 것은 뒤의 셋(`assignment_questions`·`submissions`·`interventions`)이 아니라, 마켓 축 라우트가 걷히는 만큼이다** — 앞의 다섯은 그 라우트들이 사라진 뒤에야 DROP 할 수 있다. 학부모·동의·담은 봇·자기주도·마켓 계열은 손대지 않고 「로컬 전용」 표시 유지 | FE PR 8 의 범위가 이 목록이다. 개발용 신원 쿠키(`lib/dev-identity.ts`)와 `lib/current-user.ts` 의 서버 해석기도 같이 간다 — 다만 범위 밖 라우트가 그 해석기를 아직 쓰므로, 겹치는 라우트를 지운 뒤 **남는 참조가 0 인 것만** 걷는다 |
...
| ③ | **봇과 반을 갈라낸다** — `bots` 표, 반은 `bot_id` 로 가리킨다. 한 봇이 여러 반에 붙을 수 있고, 반은 한 번에 봇 하나 | pullim-api PR 1 이 `bots` 를 세우고 `class_bot_profiles` 를 이행한다. 「할당·교체」가 `PUT /classes/:id/bot` 이라는 진짜 행위가 된다. 챗 프롬프트 빌더(`PersonaInput`)는 조인만 바뀐다. ADR-063 「bot == classes 행 재사용」은 **ADR-092** 가 개정한다(refines ADR-063 · pullim-api #667 머지) — 근거는 § 1 R4 와 § 4, 이 리포 쪽 기록은 [2026-07-03 amendment](2026-07-03_be-api-m2-amendment.md) 머리 |
...
브라우저 ── ① 비로그인 → os.pullim.ai/login?next=… → 복귀 (Set-Cookie pullim-at · HttpOnly · Domain=.pullim.ai)
   ...
   └─ ② api.pullim.ai/classbot/* (pullim-api)        JwtVerifyGuard → EntitlementGuard('classbot') → CsrfGuard(쓰기)
         operator = classes.operator_id === sub · 남의 반은 403 · 남의 봇·신호는 404 (ADR-092)
...
**route handler 은퇴 범위** — 결정 ① 그대로. 겹치는 계열(반·봇·챗·과제)의 라우트·표·시드·목 파일·스토어 persist 가 FE PR 8 에서 걷힌다. 범위 밖(`/api/me/consents/*` · `/api/me/self-bots/*` · `/api/me/study-days/*` · `/api/parent/*` · `/api/marketplace/*`)은 남되 「로컬 전용」이다 — 배포에서는 지금처럼 익명 401 이다. `lib/current-user.ts` 는 범위 밖 라우트의 참조가 0 이 되는 시점에 걷는다.

*(정정 2026-09-17)* — 종전 이 줄은 `/api/wellness` 를 「남긴다 · 로컬 전용」 쪽에 세웠다. **`/api/wellness` 와 `/api/grades` 는 범위 밖 계열이되 소비자가 0 인 죽은 라우트**라 PR 8 = **#358** 이 함께 걷는다. 실측(2026-09-17) — 웰빙 화면 셋(`app/(student)/classbot/wellness/page.tsx` · `check-in/page.tsx` · `check-in/check-in-form.tsx`)은 `lib/mock` · `lib/store/mode-bots` · `lib/current-user` 만 읽고 **`fetch` 가 하나도 없다** · `hooks/api/read/use-student-reads.ts` 가 여는 문은 `/api/bots` · `/api/assignments` · `/api/assignments/[id]` 셋뿐이라 두 경로를 부르는 훅이 없다. **부르는 자리가 0 이라는 뜻이고, 이름이 적힌 자리가 0 이라는 뜻은 아니다** — `dev` 에서 두 경로를 문자로 담은 자리는 다섯이고 전부 소비자가 아니다: 라우트 단위 테스트 둘(`app/api/__tests__/read-routes.test.ts` · `__tests__/self-route-role-guard.test.ts`) · 주석 둘(`app/api/_lib/guards.ts:141-142` · `lib/api/read-fetch.ts:4`·`:20`) · 그리고 `hooks/api/read/types.ts` 는 **주석이 아니라 살아 있는 export 타입**이다(`GradeReadRow:62` · `GradesReadResponse:87` — 부르는 훅이 없어 함께 걷힌다). 두 라우트가 쓰던 표 셋(`grading_history` · `emotion_checkins` · `wellbeing_snapshots`)이 § 0 결정 ① 의 DROP 넷 중 셋이다. **화면은 그대로 산다** — 목·스토어로 그리던 그대로다.
...
| 테이블 | 지금 | 이번에 | 쓰는 요구 | 비고 |
|---|---|---|---|---|
| `classes` | 있음 | **+ `bot_id`** (nullable) · **+ `subject` · `grade`** (nullable) | R1 R4 | `operator_id` = 교사 sub. 지금은 반이 곧 봇(ADR-063 「bot == classes 행 재사용」 → **ADR-092** 가 갈라낸다). 봇은 선택이라 **반이 과목·학년을 스스로 든다** — 봇 없는 반도 서고, 봇을 갈아도 반의 과목·학년은 남는다 |
...
**이 리포의 Drizzle 31표는**: 반·봇·챗·과제 계열은 A 와 겹치므로 은퇴(결정 ①). 학부모·동의·담은 봇·자기주도·마켓 계열은 범위 밖 — 손대지 않는다. 라이브·리플레이·채점·웰빙·리포트 계열은 어차피 어느 라우트도 안 쓴다 *(정정 2026-09-17 — 그중 `emotion_checkins`·`grading_history`·`wellbeing_snapshots` 셋은 죽은 라우트 둘(`/api/grades` · `/api/wellness`)과 함께 #358 이 DROP 한다. § 0 결정 ① 칸과 § 2 의 정정)*.
...
| PR | 리포 · 층 | 내용 | ← 의존 | 메모 | 상태 (2026-09-18 실측) |
|---|---|---|---|---|---|
| **PR 0** | docs · 두 리포 | 이 문서 + [03 § 2.1·§ 2.2·§ 2.3](03-features-and-ia.md) · [05 § 3·§ 11.1·§ 11.2·§ 11.3](05-business-rules.md) · [2026-07-03 amendment](2026-07-03_be-api-m2-amendment.md) 「bots 분리」 · [`apps/classbot/CLAUDE.md` § 5](../../apps/classbot/CLAUDE.md) 승인 기록 · 루트 `CLAUDE.md` § 1·§ 2 BE 절. pullim-api: `authz.md` 교사 대화 열람 · `data-model.md` `bots`·`risk_signals` | — | **코드 무변경.** 리뷰가 base 스펙을 보므로 맨 앞 | 이 리포 **#349 머지**(2026-09-16). pullim-api 쪽은 로컬 브랜치 `docs/classbot-classroom-spine` `50d04e20`(ADR-092) — 푸시·PR 대기(소유자) → **#667 머지**(2026-09-17 19:25 KST · `f6e546a0`) |
| **PR 1** | pullim-api · schema | `bots` · `classes.bot_id` · `messages.bot_id` · `risk_signals` · `join_codes.expires_at` — 마이그레이션 파일 하나 + `class_bot_profiles → bots` 이행 + `PersonaInput` 로더 조인 교체. 엔티티·리포지토리만 | PR 0 | 라우트 없음 | 로컬 `feat/classbot-bots-schema` `c4a5c685`(엔티티 + `uq_risk_signals_message_kind`) · `origin/dev` `97e0d624` 위 — PR 0 문서 브랜치 `50d04e20` 과 **형제** · 세션 리뷰 「푸시 가능」 · `pnpm migration:generate ClassbotBotsAndRiskSignals` 는 승인 뒤 같은 PR → **#668 머지**(19:25 KST · `4b97257f` · `1789625114547-ClassbotBotsAndRiskSignals.ts` 포함) · dev RDS 적용 21:35 KST(소유자 보고) |
| **PR 2** | pullim-api · routes A | `POST /classes` · `GET /classes/:id/members` · `POST/PATCH /bots` · `PUT /classes/:id/bot` · 참여 만료 410 · 재발급 시 옛 코드 삭제 · `GET /assignments?audience=teacher&classId=` | PR 1 | 가드 순서는 B 에서 옮기되 오류 관례는 pullim-api(남의 반 403 · 남의 봇·신호 404 — ADR-092). 단위 테스트 필수 | 로컬 `feat/classbot-classroom-doors` `6ce7bfd2` · 리뷰 반영 · 세션 리뷰 「푸시 가능」 · PR 1(`c4a5c685`) 위에 쌓임 → **#669 머지**(19:26 KST · `25b9393d`) |
| **PR 3** | pullim-api · routes B | `GET /classes/:id/chat?studentId=`(operator) *(`[2026-09-17 정정]` — 종전 `?student=` · 브랜치 실물 `chat.controller.ts` 는 `studentId`)* · `GET /classes/:id/signals` · `PATCH /signals/:id/ack` · 챗 저장 훅에 규칙 다섯 · 과제 본문 `questions[]` 수용 확인 | PR 1 | PR 2 와 병렬(계획) | 로컬 `feat/classbot-monitoring-doors` `00acb39e` · 세션 리뷰 3회 반영 · 세션 리뷰 「푸시 가능」 · **PR 2(`6ce7bfd2`) 위에 쌓임 — 브랜치 실물은 직렬**, 병렬로 두려면 ② 위로 rebase *(`[2026-09-17 정정]` — 종전 「PR 1 위에 쌓임」은 부모 SHA 와 다르다)* → **#670 머지**(20:17 KST · `324f36fc`) |
| **PR 4** | pullim-api · reads | `GET /classes/:id`(반 상세 — FE 가 새로고침 뒤 `classes.bot_id` 를 다시 읽을 문 · `ClassResponseDto` 봇 합성 · `joinCode` 는 operator 응답에만) · `GET /me/bots`(내 `bots` 최신순 · `classIds`) — 스키마 무변경 | PR 2 | FE 5d 가 기다린다. 구 `GET /bots?role=` · `GET /bots/:id` 는 반 기준 그대로(뜻 개정은 후속) | **#672 머지** — pullim-api `origin/dev` 끝이 `2d24f323`(2026-09-18 실측). FE 5d 가 이 두 문 위에 섰다 |
...
| **PR 5b** | classbot · FE 수업방 (2/2) | 반 만들기(`POST /classes` · 첫 참여 코드와 닫히는 시각 배너) · 반 상세 「명단」 탭(`GET /classes/:id/members` · 해소 5 표시명 투영) · 「봇」 탭(지금 붙은 봇 · 새 봇 만들어 붙이기 `POST /bots` → `PUT /classes/:id/bot` · 봇 떼기 `{ botId: null }` · 이름/인사말/말투/scope 고치기 `PATCH /bots/:id`) · 참여 코드 만료 표시 *(`[2026-09-17 밤 정정]` — 종전 「… 봇 할당/교체 · 반 카드 봇 칸 · 참여 코드 만료 표시(410) · 봇 관리/빌더를 pullim-api `bots` 로 · 목 참여 폴백 제거」 에서 **바꾸기** · 반 카드 봇 칸 · 봇 관리/빌더 정본 전환 · 목 참여 폴백 제거는 **5d** 로 — #355 가 인도한 만큼으로 좁혔다)* | PR 2(#669 ✓) · PR 5a | #355 본문: 내 봇 목록 `GET /bots?role=teacher` 가 아직 반 기준이라 「다른 봇으로 바꾸기」 고르개가 없고 반 카드 봇 칩은 옛 `profile` 기준 그대로 — 그래서 5d | **#355 열림**(`feat/classbot-classroom-spine-2` · `55ed74c` 에서 갈라져 #354 를 안 담는다 — **`dev`(#354 `ad21d11`) 위로 rebase 필요**: `git merge-tree` 로 네 파일 충돌 `[id]/class-detail.tsx` · `[id]/page.tsx` · `[id]/__tests__/class-detail.test.tsx` · `hooks/api/classroom.ts`; rebase 때 「명단」「봇」 을 #354 가 만든 `class-tabs.ts` 에 등록하는 몫이 #355 로 넘어온다 · 리뷰 통과 시 머지) |
...
| **PR 5d** | classbot · FE 수업방 (봇 정본 전환) | 반 카드·반 상세의 봇 칸이 `GET /classbot/classes/:classId` 를 읽는다(세션 캐시가 아니라 — **새로고침 뒤에도 봇을 안다**) · 「봇」 탭 「다른 봇으로 바꾸기」(`GET /classbot/me/bots` 로 붙은 봇을 뺀 내 봇을 늘어놓고 `PUT /classbot/classes/:classId/bot`) · `/teacher/bots` 목록·상세가 정본(`POST`·`PATCH /classbot/bots`) · 빌더가 `POST /classbot/bots` 로 **진짜 만들고** `PUT` 으로 붙인다 | api PR 4 = pullim-api #672(`GET /classes/:id` · `GET /me/bots`) · PR 5b | 종전 5b 의 나머지. 「봇 이름·말투」 탭은 이 PR 이 걷었다(화면이 그 일을 하게 돼서 — [03 § 4.4.2](03-features-and-ia.md) 정정) | **#360 머지**(2026-09-18 · squash `5026813`) |
| **PR 5e** `[예정]` | classbot · FE 마켓 정본 전환 | `/teacher/marketplace`(게시·해제·둘러보기)를 pullim-api 로 옮기고, 그때 `app/api/teacher/{classrooms,bots}` 다섯 라우트를 은퇴시킨다 | **pullim-api 게시 표면**(별건 PR — 아래) | 5d 가 열어 둔 유일한 축. **지금은 막혀 있다** | 대기 — 문이 없다 |
...
| **PR 7** | classbot · FE 모니터링 | 반 상세 「대화」 탭 · 학생별 기록 뷰어 · 신호 배지·확인 · 관제소 목 표 은퇴 | PR 3(#670 ✓) · PR 5a *(`[2026-09-17 정정]` — 종전 「PR 3 · PR 5」)* | 반 상세 탭 등록부 `class-tabs.ts` 는 이 PR 이 새로 만들었다(「과제」「대화」) — 「명단」「봇」 등록은 #355 가 rebase 하며 맡는다 | **#354 머지**(2026-09-17 21:16 KST · `ad21d11`) |
| **PR 8** | classbot · 정리 | B 세계 은퇴 — 겹치는 `app/api/**` 라우트 · Drizzle 표 · 시드 · 개발 쿠키 경로 · `lib/mock` 반·봇·챗·과제 파일 · 스토어 persist. 범위 밖 계열은 남긴다 | PR 5b · 5c · 5d · 6 · 7 *(`[2026-09-17 정정]` — 종전 「PR 5 · 6 · 7」 · 5d 는 밤에 더함)* | **맨 마지막** | **#358 머지**(2026-09-18 실측 · squash `e1b8860`). **5d(#360 `5026813`)보다 먼저 들어갔다** — 표의 「← 5d」 는 계획의 뜻이고 실제 머지 순서는 그 반대다. 그래서 5d 가 고친 화면들은 은퇴 뒤의 나무 위에 섰다 |
...
PR 0✓ ─┬─→ PR 1✓ ─┬─→ PR 2✓ ─┬─→ PR 5b✓ → PR 5c✓ ┐     ✓ = dev 머지(2026-09-18 실측) · PR 0✓ 는 이 리포 #349 와 pullim-api #667 둘 다
       │          │          └→ api PR 4✓ → 5d✓ ┼─→ PR 8✓     api PR 4✓ = pullim-api #672(`2d24f323`) · 5d✓ = #360(`5026813`)
       │          └─→ PR 3✓ ─→ PR 7✓ ───────────┤     교차 의존(선 생략): PR 5b ← PR 5a · PR 7 ← PR 5a · PR 6 ← PR 3 · PR 5d ← PR 5b
       └─→ PR 4-ci✓ ─→ PR 4✓ ─┬─→ PR 5a✓        │     PR 8✓ = #358(`e1b8860`) — **5d 보다 먼저** 들어갔다(선의 뜻은 계획이지 머지 순서가 아니다)
                              └─→ PR 6✓ ────────┘

                            ┌─ pullim-api 게시 표면(별건 PR · 아직 없다) ─→ PR 5e  ← 남은 코드 PR 은 이 하나 + e2e 트랙
```
...
### 8.1 5d 가 남기고 간 것 — 마켓 축은 문이 없어 5e 로 연다 *(2026-09-18)*

**막은 것은 취향이 아니라 문이다.** pullim-api `src/classbot` 의 컨트롤러는 **일곱**이고(`assignment` · `bot` · `chat` · `classroom` · `intervention` · `replay` · `signal` — `origin/dev` `2d24f323` 전수 확인), 그중 `bot.controller.ts` 가 여는 문은 **셋뿐**이다: `GET classbot/me/bots` · `POST classbot/bots` · `PATCH classbot/bots/:id`. **게시하는 문도 해제하는 문도 없다.** 표의 칸은 이미 있다 — `bot.entity.ts` 가 `isPublished`·`publishedAt` 를 들고 머리주석에 「마켓 공개(**후속 표면**)」라 적어 두었다. 칸만 있고 문이 없는 상태다.

그래서 **`/teacher/marketplace` 와 `app/api/teacher/{classrooms,bots}` 는 같은 축으로 함께 남았다** — 게시·해제가 `POST`·`DELETE /api/teacher/bots/[botId]/publish`(`hooks/api/marketplace.ts`)를 부르고, 그 라우트가 로컬 표를 읽는다. 남은 라우트 다섯: `teacher/bots` · `teacher/bots/[botId]/publish` · `teacher/classrooms` · `teacher/classrooms/[id]/join-codes` · `teacher/classrooms/[id]/students`. **PR 8(#358) 이 이 계열을 남긴 것과 같은 판단이다** — 부르는 화면이 살아 있는 라우트는 은퇴시키지 않는다.

**순서는 하나뿐이다**: pullim-api 게시 표면(별건 PR · 그 리포 `dev`) → 그 뒤에 FE 마켓 전환 + 이 트리 은퇴(PR 5e). 앞뒤를 바꾸면 마켓이 통째로 죽는다.

### 8.2 5d 뒤의 후속 목록 — 코드에 남은 자국 *(2026-09-18 실측)*

| 무엇 | 자리 | 왜 후속인가 |
|---|---|---|
| 빌더 요약의 **반 이름·말투 행이 목 학급 표를 읽는다** | `components/builder/builder-types.ts` 의 `classroomLabel()`(`classroomChoices` = `teacherClassrooms` 목) 과 그것을 쓰는 `summaryRows()` 의 `classes` 행 | 정본 반 id 는 그 표에 없어 **교사에게 uuid 가 보인다.** 지금은 두 화면이 「보여주기용 사본」으로 피해 간다(`done-view.tsx` 는 이름을 잇거나 개수로, `builder/[botId]/edit-workspace.tsx` 는 개수로). **주입 조회로 고치면** 요약이 정본을 따르고 그 사본이 없어진다. 같은 파일의 `classAssignments()` 는 **부르는 화면이 이미 0**(테스트만) 이고 머리주석이 아직 「지금은 데모라 저장하지 않지만」이라 적는다 |
| **서버에 칸이 없어 로컬로 남은 것** | 봇별 설정의 안전 등급 **시간대 스케줄** · **이탈 설정**, 빌더의 **수업 자료**(`files`) · **평소에**(`style`) · **틀렸을 때**(`wrong`) | `BotDto`·`CreateBotBody`·`UpdateBotBody`(`lib/api/classbot-dto.ts`)에 그 칸이 없다. **걷지 않고 그대로 묻는다** — 칸이 생기는 날 그대로 실어 보내면 되고, 지금 지우면 교사가 정할 수 있던 것이 먼저 사라진다. 화면이 「새로고침하면 사라져요」라고 말한다 |
...
| 이름 겹침 nit | `components/builder/done-view.tsx` — 바깥 `attached`(`string \| null` · 반 이름 줄)와 안쪽 `attached`(`boolean` · 붙었나) | 한 파일 안에 같은 이름이 뜻을 바꿔 두 번 선다. 안쪽을 `isAttached` 로 |
| 시드 FK 고장 | `apps/classbot/scripts/seed.ts` — [plan 「알려진 고장」](../plan/2026-09-16_classbot-completion-plan.md) | **이미 기록됐다. 그대로 둔다** — 이 축과 무관하고, 고치는 자리도 거기 적혀 있다 |

**반 이름을 못 풀 때의 폴백은 한 규칙이다** — 「이름이 **전부** 풀리면 잇고, 하나라도 못 풀면 개수로 물러선다」. 일부만 이으면 못 찾은 반이 **말없이 사라져** 두 반에 붙은 봇이 한 반에만 붙은 것처럼 보이기 때문이다. 그 폴백이 선 자리는 **셋**이다 — 「방금 만든 봇」 배너(`app/(teacher)/teacher/classbot/page.tsx`) · 「다른 봇으로 바꾸기」 목록(`class-bot-tab.tsx`) · 만든 뒤 요약(`done-view.tsx`). *(정정 2026-09-18 — #360 커밋 메시지는 「네 자리」라 적었다. 네 번째로 센 봇별 설정 머리 부제(`bot-settings-workspace.tsx` 의 `describeBot`)와 다섯 번째 `edit-workspace.tsx` 는 **이름을 시도조차 하지 않고 처음부터 개수로만 말한다** — `BotDto` 에 반 이름이 없어서다. 폴백이 아니라 같은 뜻의 다른 문장이다. 코드 주석 둘이 적은 「세 자리」가 맞다.)*

상태 열의 「세션 리뷰」 = 이 세션(session_01V1F4LGiFNG6JjYBpiDx32G)의 에이전트 리뷰 판정이다. pullim-api 에 PR 이 없어 두 리포 어디에도 리뷰 코멘트 기록이 없다 — 여기 적힌 것이 유일한 기록이고, 소유자가 PR 을 열면 그 PR 의 리뷰가 이것을 대신한다. pullim-api 브랜치 실물(부모 SHA · 2026-09-17 실측): `origin/dev` `97e0d624` ─┬ `50d04e20`(PR 0 문서) · └ `c4a5c685`(PR 1) → `6ce7bfd2`(PR 2) → `00acb39e`(PR 3). *(`[2026-09-17 밤 정정]` — 넷 다 머지됐다: #667 `f6e546a0` · #668 `4b97257f` · #669 `25b9393d` · #670 `324f36fc`(2026-09-17 19:25~20:17 KST). 이제 그 PR 들의 리뷰가 기록이고, api PR 4 는 `feat/classbot-class-and-bot-reads`(문서 `0f08bdb0` + 코드 `6dcac2ba` · `324f36fc` 위)로 pullim-api #672 에 열려 있다)*

## 9. 사람 액션 — 코드 밖에서 해야 할 것

*(`[정정 2026-09-18]` — 종전 「2026-09-17 04:00 KST 기준 **전부 미완**」. 그 뒤로 여섯이 끝났고 둘은 **애초에 할 일이 아니었다.** **남은 것은 다섯이다** — ① pullim-api 게시 표면 ② e2e 시크릿 넷 ③ Vercel 운영(Production) env ④ pullim-api **prod** CORS · dev→main 승격 ⑤ dev-os Deployment Protection 우회. **그중 코드를 막는 것은 ① 하나뿐**이고, PR 5e 가 그것만 기다린다. 이번에 새로 판정한 것은 ②·③ 둘이다.)* 코드가 먼저 막히는 순서로 위에 둔다.

- **pullim-api 푸시 + PR 넷, 순서대로** — 로컬 브랜치 넷은 세션 리뷰를 지나 「푸시 가능」인데 `origin/` ref 가 없다. 브랜치마다 `git push -u origin <branch>` 뒤 `gh pr create --base dev …`: ① `docs/classbot-classroom-spine`(PR 0 · ADR-092) ② `feat/classbot-bots-schema`(PR 1) ③ `feat/classbot-classroom-doors`(PR 2) ④ `feat/classbot-monitoring-doors`(PR 3). ①·② 는 `origin/dev` `97e0d624` 위의 **형제**, ③ 은 ② 위, ④ 는 **③ 위** — base 는 넷 다 그 리포 `dev`, **머지 순서는 ② → ③ → ④**(④ 의 diff 에는 ③ 이 머지되기 전까지 ③ 의 변경이 함께 실린다. ③·④ 를 병렬로 하려면 ④ 를 ② 위로 rebase 한 뒤 푸시) *(`[2026-09-17 정정]` — 종전 「③·④ 는 ② 위에 쌓인다」)*. **됐다**(2026-09-17 밤): #667 `f6e546a0` → #668 `4b97257f` → #669 `25b9393d` → #670 `324f36fc`.
- ~~**pullim-api PR 4 = #672**~~ — **됐다**: 그 리포 `dev` 끝이 `2d24f323`(2026-09-18 실측). FE 5d(#360)가 그 위에 섰다.
- **pullim-api 게시 표면(별건 PR)** — **→ 이것이 머지돼야 PR 5e 가 착수된다**(`PR 5e ← 게시 표면` · § 8.1 · § 8 표의 「← 의존」 칸). `bot.controller.ts` 에 게시·해제 문이 없다. **남은 다섯 중 코드를 막는 유일한 하나.** 소유자 게이트.
- **`pnpm migration:generate ClassbotBotsAndRiskSignals` 승인** — 생성 파일은 ② 와 같은 PR 에 들어간다(§ 4 「마이그레이션 파일 하나」). **됐다**: `1789625114547-ClassbotBotsAndRiskSignals.ts` 가 #668 에.
- **dev RDS `migration:run` 승인** — ② 가 그 리포 `dev` 에 들어간 뒤. 이것이 없으면 FE PR 5b·7 이 부를 문이 dev-api 에 없다. **됐다**(2026-09-17 21:35 KST · bastion 경유 · `ClassbotBotsAndRiskSignals1789625114547` — 소유자 보고, 이 리포에서는 실측 불가).
...
- ~~**OS 복귀 허용목록**에 `dev-classbot.pullim.ai` · `classbot.pullim.ai`~~ — **할 일이 아니다** *(정정 2026-09-18)*. 종전에는 「목록에 넣어야 한다」고 적었지만, pullim-api `src/common/security/redirect-host-allowlist.ts` 의 `isAllowedRedirectUrl()` 은 **호스트를 열거하지 않는다** — dev/prod 는 `host === 'pullim.ai' || host.endsWith('.pullim.ai')` 에 https 강제 한 줄이다. 두 도메인 다 그 규칙에 이미 든다. **남는 것은 미리보기 도메인**(`*.vercel.app`)뿐이고, 그건 목록 문제가 아니라 규칙 밖이라 커스텀 도메인으로만 푼다([2026-07-01 B-8 핸드오프](../plan/2026-07-01_b8-os-redirect-allowlist-handoff.md)).
- **Vercel env**: `NEXT_PUBLIC_OS_URL` · `NEXT_PUBLIC_OS_API_URL`. **Preview(dev) 와 Development 에는 넣었다**(2026-09-17). **실측으로 확인된다** — 배포된 `https://dev-classbot.pullim.ai/classbot` 이 실어 보내는 클라이언트 청크에 `https://dev-api.pullim.ai` 와 `https://dev-os.pullim.ai` 가 **박혀 있다**(2026-09-18 · 그 값이 없으면 `lib/auth/os-sso.ts` 의 로컬 기본값 `api.pullim.local:3000` 이 박힌다). **남은 것은 운영(Production) env 결정 하나**다 — 값은 `https://os.pullim.ai` · `https://api.pullim.ai`. *(종전 「프로젝트에 환경변수가 하나도 없다」는 2026-09-14 실측이고 더는 현행이 아니다 — [05 § 11.1](05-business-rules.md))*
- ~~**pullim-api CORS** 에 두 오리진~~ — **dev 는 이미 있다** *(정정 2026-09-18)*. 실측: `OPTIONS https://dev-api.pullim.ai/classbot/me/bots` 에 `Origin: https://dev-classbot.pullim.ai` 를 얹으면 **204 + `access-control-allow-origin: https://dev-classbot.pullim.ai` · `allow-credentials: true`** 가 돌아온다. 코드가 아니라 배포 env(`CORS_ALLOWED_ORIGINS`)의 값이고(`src/common/config/cors.ts` 는 **명시 목록만** 낸다 — 와일드카드 금지), **prod 오리진 `classbot.pullim.ai` 는 운영 env 결정과 같이 간다**([런북 § 3-3](../plan/2026-07-01_classbot-sso-dev-deploy-runbook.md)).
