...
| **PR 5d** `[예정]` | classbot · FE 수업방 (봇 정본 전환) | 반 카드 봇 칸(옛 `profile` 칩 → `classes.bot_id` 의 봇) · 봇 관리(`/teacher/bots`)/빌더(`/teacher/builder`)를 pullim-api `bots` 로 · 「봇」 탭 「다른 봇으로 바꾸기」 고르개(내 봇 목록에서 골라 `PUT /classes/:id/bot`) · 목 참여 폴백 제거(#355 본문에 없어 여기로) | api PR 4 = pullim-api #672(`GET /classes/:id` · `GET /me/bots`) · PR 5b | 종전 5b 의 나머지 | 대기 — #672 가 dev-api 에 들어간 뒤 |
...
| **PR 8** | classbot · 정리 | B 세계 은퇴 — 겹치는 `app/api/**` 라우트 · Drizzle 표 · 시드 · 개발 쿠키 경로 · `lib/mock` 반·봇·챗·과제 파일 · 스토어 persist. 범위 밖 계열은 남긴다 | PR 5b · 5c · 5d · 6 · 7 *(`[2026-09-17 정정]` — 종전 「PR 5 · 6 · 7」 · 5d 는 밤에 더함)* | **맨 마지막** | 대기 |
...
```
PR 0✓ ─┬─→ PR 1✓ ─┬─→ PR 2✓ ─┬─→ PR 5b ─→ PR 5c ─┐     ✓ = dev 머지(2026-09-17 밤 실측) · PR 0✓ 는 이 리포 #349 와 pullim-api #667 둘 다
       │          │          └→ api PR 4 → PR 5d ┼─→ PR 8     api PR 4 = pullim-api #672(GET /classes/:id · GET /me/bots) 열림
       │          └─→ PR 3✓ ─→ PR 7✓ ────────────┤     교차 의존(선 생략): PR 5b ← PR 5a · PR 7 ← PR 5a · PR 6 ← PR 3 · PR 5d ← PR 5b
       └─→ PR 4-ci✓ ─→ PR 4✓ ─┬─→ PR 5a✓         │     PR 7✓ = #354(21:16 KST) · 열린 PR 은 #355 하나 — dev(#354) 위로 rebase 필요(네 파일 충돌)
                              └─→ PR 6✓ ─────────┘
```
...
2026-09-17 04:00 KST 기준 **전부 미완**이다. 코드가 먼저 막히는 순서로 위에 둔다. *(`[2026-09-17 밤 정정]` — 아래 「됐다」 표시 셋은 밤에 끝났다)*
...
- **pullim-api PR 4 = #672** — `feat/classbot-class-and-bot-reads`(문서 `0f08bdb0` + 코드 `6dcac2ba`) 열림 · CI 진행 중 · 리뷰·머지는 소유자. 스키마 무변경이라 RDS 작업 없음. FE 5d 의 앞.
