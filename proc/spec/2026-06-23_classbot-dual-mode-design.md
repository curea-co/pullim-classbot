...
>
> > **[2026-09-16 개정] 공식 봇은 마켓에 돌아온다 — 다만 mock 이 아니라 `class_bots` 행으로.**
> > (소유자 지시. 결정문은 [`03 § 4.13.1`](03-features-and-ia.md).)
> > 위 ② 가 **두 가지를 한꺼번에** 정했는데 그중 하나만 되돌린다:
> > **㉠ 정본을 교사 공유 봇으로 옮긴 것 — 그대로다.** 되돌리지 않는다.
> > **㉡ 그 결과 공식 봇이 마켓에서 사라진 것 — 되돌린다.** 풀림이 제공하는 기본 봇 셋이
> > 교사 공유 봇과 **같은 목록**에 서고, 카드가 공식 봇임을 표시한다.
> > **② 의 근거는 조금도 약해지지 않았다** — 「`ot_*` 는 서버에 대응 행이 없어 대화가 저장될 수
> > 없다」는 지금도 참이고, **그래서 공식 봇을 `class_bots` 행으로 넣는다.** mock `ot_*` 를
> > 마켓의 정본으로 되살리는 것이 아니다. 그 셋은 **값의 출처**로만 쓴다(이름·인사말·소개).
> > **`/classbot/learn/[tutorId]` 가 `ot_*` mock 위에서 도는 것은 아래 ③ 대로 그대로다** —
> > 그 화면은 이 개정의 범위가 아니다.
>
