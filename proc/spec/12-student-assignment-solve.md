...
교사가 발사한 과제(연습·시험·오답정복)를 학생이 받았지만, 실제로 풀 수 있는 워크스페이스가 없다. 학생 홈에서 `[시작]`(당시 이름 `[지금 시작하기]`)을 누르면 깨진 링크로 빠지고, 교사·학생 사이의 사이클이 끊긴다.
...
| CTA | `[시작]` (`state === 'todo'`) / `[이어서 풀기 (n/N)]` (`in-progress`) / `[결과]` (`submitted`) — 버튼은 단어로 ([07 § 6.6](07-branding.md), 2026-08-24 이름 변경). **구현 전** — 현행 화면은 `[지금 시작하기]`·`[결과 보기]` 이고 뒤따르는 FE PR 이 맞춘다 |
...
> **시간대별 Scope 우선 규칙은 아직 미구현이다.** 풀이 화면은 과제의 `scopeOverride` 와 과제 메타데이터만 읽고, 시간대 스케줄을 읽어 Scope 를 정하는 경로가 없다. 시간대 스케줄을 **정하는** 자리는 봇 설정(`/teacher/settings` 「안전 등급 시간대 스케줄」)에 있고, 빌더 재편으로 자리가 빌더 § 5 → 봇 설정으로 옮겼다(`03 § 4.3`). 그 값을 풀이 화면이 소비하게 하는 것은 후속 과제다.
