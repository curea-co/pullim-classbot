...
교사가 봇 설정(`/teacher/settings` 「안전 등급 시간대 스케줄」)에서 시간대별 Scope 를 정한 경우 그 값을 우선. (빌더 재편으로 **정하는 자리가 빌더 § 5 → 봇 설정으로 옮겨졌을 뿐** 값 자체는 그대로 있다 — `03 § 4.3`) `scopeOverride`가 있으면 그 값을 우선.
