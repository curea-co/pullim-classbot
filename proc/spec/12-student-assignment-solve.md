...
> **시간대별 Scope 우선 규칙은 아직 미구현이다.** 풀이 화면은 과제의 `scopeOverride` 와 과제 메타데이터만 읽고, 시간대 스케줄을 읽어 Scope 를 정하는 경로가 없다. 시간대 스케줄을 **정하는** 자리는 봇 관리(`/teacher/bots/[botId]` 「안전 등급 시간대 스케줄」)에 있고, 빌더 재편으로 자리가 빌더 § 5 → 그 화면으로 옮겼다(`03 § 4.3`·`03 § 4.4`). 그 값을 풀이 화면이 소비하게 하는 것은 후속 과제다.
