...
> 구현은 PR #223 이 인도한다. `dev` 시점 `/teacher/builder` 는 아직 8단계 위저드다 (`Step2Voice` · `Step6Eval` · `Step8Deploy` · `saveDraft()` 그대로) — #223 이 머지되면 이 문장을 지운다.
> **아직 갱신하지 못한 권위 문서** — `input/docs-archive/05_풀림_수업방_세부기획.md` § 8.1(`Step 6: 학생 초대 → 코드/링크/QR 생성`) · 체크리스트 「봇 빌더 위저드 (6단계)」. 이번 사용자 승인이 핸드오프 § 4.1 로 한정되어 손대지 않았다. **별도 승인 사안**이며 그때까지 어긋난 채 남는다.
