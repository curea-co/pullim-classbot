...
### 4.3 봇 빌더 (`/teacher/builder`) — 한 길 · 세 마당 *(#223 인도 중)*

> **상태 (2026-08-24) — 확정 설계, 구현 인도 중.**
> 이 절이 적는 「한 길 · 세 마당」은 **확정된 설계**이고 구현은 **PR #223** 이 인도한다. `dev` 시점 `/teacher/builder` 는 **아직 8단계 위저드**다 (`currentStep < 8` · `Step2Voice` · `Step6Eval` · `Step8Deploy` · `saveDraft()` 그대로). 이 절을 「현재 화면 설명」이 아니라 **「#223 이 인도할 화면의 명세」**로 읽는다. **#223 이 머지되면 이 상태 줄을 지운다.**
