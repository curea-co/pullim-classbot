...
> **2026-08 재편 2차 — 화면을 한 겹 더 덜어냈다.** (위 개정과 **같은 재편의 두 번째 갈래**다 — 새 개정일을 세우지 않는다.) 「지난 봇에서 가져오기」를 없애고, 배지 체계(`꼭 골라요` / `기본값` / `내가 정함`)를 **필수 `*` / `(선택)`** 로 바꾸고, 「채워진 것」에서 배지와 줄별 「고치기」를 걷어내고, 「단계」를 콘텐츠 위쪽 한 자리로 모으고, 「이대로 만들기」를 페이지 헤더로 올렸다. 아래 본문이 이 개정까지 반영한 정의다.
