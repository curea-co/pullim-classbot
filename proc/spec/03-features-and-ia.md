...
- **앞으로 가는 길은 모두 필수를 지난다** — 「만들기」·「다음」·위쪽 「단계」로 **앞으로** 건너뛰는 것, 셋이 **같은 판정**을 지난다. 필수를 안 채우고도 앞으로 갈 수 있는 창구를 하나라도 남기면 「다음」에서 막은 것이 그 창구로 열린다. **뒤로 가는 길은 막지 않는다** — 「이전」도, 「단계」로 되돌아가는 것도. 막을 때는 그 항목이 사는 마당으로 돌려보내고, 왜 막혔는지 그 항목 밑에서 말하고, 그 자리로 초점을 옮긴다.
  - **지금(`dev`)은 이렇다**: 「만들기」와 「다음」에만 판정이 있다. 위쪽 「단계」로 앞으로 건너뛰는 길은 **아무 검사 없이** 옮긴다.
  - **앞으로는 이렇게 한다**: 세 길이 같은 판정을 지난다. 구현은 뒤따르는 FE PR 이 맞춘다.
  - 위 「단계 점프는 그대로 있다」와 어긋나지 않는다 — 점프도, 대상 셋도, 진행이 보이는 것도 그대로다. 앞으로 가는 점프에 판정이 붙을 뿐이다.
  - 권위: [07 § 4.1](../../input/docs-archive/07_풀림_클래스봇_핸드오프.md) 「앞으로 가는 길은 모두 필수를 지난다」 (2026-08-25 개정 · 사용자 명시 승인).
