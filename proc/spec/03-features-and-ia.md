...
**메모 3 — 표기 통일**: 이 문서에서 **학급 관제소**를 가리키는 말은 `/teacher/monitor` 하나로 쓴다 (리포트 센터 명단이 거르기·정렬 규칙을 읽어 오는 화면이다). 위 운영 라우트 표의 `/teacher/live` 는 base 정의 그대로 둔다 — **운영 IA 표 자체의 정합성 정리는 이 변경의 범위가 아니다.** 표기만 통일하고, 라우트 인벤토리 갱신은 별도 문서 작업으로 남긴다.
...
| 학생 줄 진입 | `/teacher/students/[id]` (대화 기록 + 과정 평가) |

**규칙 공유 — 재사용 + 확장**: 「미도달」·「목표 수준 미달」·「오늘 안 들어옴」 판정과 지름길·이탈·오래된 순 정렬은 학급 관제소(`/teacher/monitor`)의 규칙을 **그대로 읽는다**. 여기에 리포트 센터가 더하는 것은 둘뿐이다 — 교사 홈의 「먼저 볼 학생」 판정을 **거르개로 세우고**, 같은 판정의 순서를 **정렬로 세운다**(기본 정렬. 관제소 기본은 이름순이라 서로 다르다). 판정 자체를 새로 만들지는 않는다. 기준의 뜻은 화면에 문구로 노출한다. 상세: [13 § 3.3.1](13-reports-and-emotion-checkin.md).
