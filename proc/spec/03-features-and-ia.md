...
**메모 3 (채점 허브)**: `/teacher/grading` 은 「채점할 게 있는 학생」이 아니라 **등록된 학생 전체**를 기본으로 보여준다. 채점 대기가 0건인 학생도 목록에 남는다 — 큐만 보이면 오늘 제출하지 않은 학생이 화면에서 사라지기 때문이다. 학생 요약은 배지(도달 · 최근 접속 · 채점 대기)로만 말하고, 학생을 누르면 **새 화면을 만들지 않고** `/teacher/students/[id]` 로 보낸다. 학생 목록으로 들어가는 길은 **학급 관제소와 채점 허브 탭 둘뿐**이고 `/teacher/students` 는 진입점이 아니다 — 명단은 화면마다 따로 세지 않고 데이터 한 곳(`monitoredRoster` — BE enrollment 가 오기 전의 임시 FE 시드)에서 읽는다. 채점 시드도 같은 명단·같은 수업으로 정렬해 두어, 채점 항목에서 학생 상세로 건너가도 같은 학생이다. 자세한 정의는 [11. 채점 허브](11-grading-hub.md § 3.3.0 · § 7.1).
