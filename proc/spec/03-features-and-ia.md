...
> 단, 아래 **「없어진 것」의 초대 코드·QR · 루브릭 진실원 · 임시저장**은 #223 과 무관하게 `dev` 시점에도 사실이다 (종전 문서가 미구현을 구현된 것처럼 적고 있었다).
...
- **초대 코드 · 링크 · QR** — 종전 문서가 「8. 배포」의 산출물로 적었으나 **코드에 구현된 적이 없다**. 반 배정은 만든 뒤 화면(그리고 `/teacher/classbot` 운영 화면)에서 하고, 초대 코드·QR 는 여전히 **미구현**이다.
