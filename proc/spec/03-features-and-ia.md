...
> **2026-08-24 개정** — 8단계 위저드를 「한 길 · 세 마당 + 만든 뒤 화면」으로 바꿨다 (PR #223). 다른 문서가 「빌더 § N 단계」로 가리키던 자리는 아래 대응표를 거쳐 읽는다.
...
- **Scope 시간대별 자동 스위치** — 종전 「5. Scope」의 체크박스. 역시 화면 안 상태로만 있었고 저장되지 않았다. 새 시안에 없다.
