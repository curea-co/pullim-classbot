...
**메모 (채점 허브 · 2026-08-25 KST)**: `/teacher/grading` 은 「채점할 게 있는 학생」이 아니라 **등록된 학생 전체**를 기본으로 보여준다. 채점 대기가 0건인 학생도 목록에 남는다 — 큐만 보이면 오늘 제출하지 않은 학생이 화면에서 사라지기 때문이다. 학생 요약은 배지(도달 · 최근 접속 · 채점 대기)로만 말하고, 학생을 누르면 **새 화면을 만들지 않고** `/teacher/students/[id]` 로 보낸다. 자세한 정의는 [11. 채점 허브](11-grading-hub.md § 3.3.0 · § 7.1).
