...
> **2026-08-25 개정 — 화면을 한 겹 더 덜어냈다.** 「지난 봇에서 가져오기」를 없애고, 배지 체계(`꼭 골라요` / `기본값` / `내가 정함`)를 **필수 `*` / `(선택)`** 로 바꾸고, 「채워진 것」에서 배지와 줄별 「고치기」를 걷어내고, 「단계」를 콘텐츠 위쪽 한 자리로 모으고, 「이대로 만들기」를 페이지 헤더로 올렸다. 아래 본문이 이 개정까지 반영한 정의다.
...
- **지난 봇에서 가져오기** — 마당 1 맨 위에 있던, 봇 카탈로그의 넷(과목 · 학년 · 말투 · 답 범위)을 옮겨 담는 자리. **2026-08-25 개정에서 없앴다** — 아홉 가지 가운데 넷만 옮겨 오는 자리를 첫 화면 맨 위에 두면, 과목 하나만 고르면 끝나는 길이 그만큼 뒤로 밀린다. 종전 「이전 봇 복제」로 거슬러 올라가는 요구사항도 여기서 닫는다 — 다시 붙인다면 빌더 첫 화면이 아니라 봇 운영 화면(`/teacher/classbot`)의 봇 카드가 자리다.
