...
| 만든 뒤 화면 | — | 봇 배정할 반 고르기 + 「채워진 것」 재표시 + 「봇 운영」 / 「고치기」 / 「새 클래스봇」 *(이름 변경 구현 전 — 현행 「봇 운영 화면으로」 / 「이어서 고치기」 / 「새 봇」. 셋째 이름은 `[2026-09-16]` 개정으로 「새 봇」에서 다시 바뀌었다 — § 4.4.5 개정 박스)* |
...
| 자리 | 이름 | 근거 |
|---|---|---|
| 목록 헤더 CTA (봇이 있을 때) | **「새 클래스봇」** | 같은 일을 하는 버튼이 **교사 홈과 운영 화면에서 이미 이 이름**이다. 같은 뜻은 같은 이름으로 (아래 개정 박스) |
...
>
> `dev` 를 실측하면 **새 봇을 만들러 가는 버튼이 세 자리**인데 이름이 둘로 갈려 있었다 —
> 교사 홈(`app/(teacher)/teacher/page.tsx`)과 운영 화면(`app/(teacher)/teacher/classbot/page.tsx`)은
> **「새 클래스봇」**, 봇 관리만 **「새 봇」**. 그리고 그 갈림은 **알고 둔 것**이었다
> (`classbot/page.tsx` 의 주석이 「이름이 옆 화면의 「새 봇」과 갈리는 것은 알고 두는 것이다」라고 적는다).
> **둘 중 하나로 모은다면 이미 둘을 차지한 쪽으로 모으는 것이 맞다** — 한 자리를 고치면 셋이 같아지고,
> 반대로 가면 두 자리를 고쳐야 하는 데다 「클래스봇」이라는 **이 서비스의 이름**이 버튼에서 사라진다.
>
...
| 봇 관리 헤더 CTA 「새 클래스봇」 *(`[2026-09-16]` 이름 개정 — § 4.4.5)* | `app/(teacher)/teacher/bots/page.tsx` | `PageHeader` 의 `action`(`data-testid="bots-new-cta"`) |
...
| 시드 | **`scripts/seed.ts`** 쪽이다(`scripts/demo-reset.ts` 가 아니라) — 데모를 되돌려도 **항상 있어야 하는 봇**이다. 여러 번 돌려도 같은 결과가 되게(멱등) 쓴다 |

**소유자를 비우는 데에는 값싼 이득이 하나 더 있다.** 게시 API 가 `where(id = ? and teacherId = ?)` 로
소유자를 대조하므로, **어느 교사도 공식 봇을 내리거나 고칠 수 없다**(404). 공식 봇에는 그게 맞는 동작이다.
**대신 되돌리는 스크립트도 없다** — 그래서 시드가 멱등해야 한다는 조건이 위 표에 붙는다.
...
**⚠ 배포된 `dev` 에서는 이것이 보이지 않는다.** `dev-classbot.pullim.ai` 에는 `DATABASE_URL` 이 없어
마켓이 익명 401 로 「로그인하면 봇 마켓을 볼 수 있어요」만 그린다(2026-09-16 실측 · 근거는
`lib/dev-identity.ts` 머리주석의 실측 기록). **공식 봇을 시드에 넣어도 그 화면은 달라지지 않는다** —
