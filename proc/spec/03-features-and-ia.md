...
│  │                             **`[예정]`** 여기에 **풀림 공식 기본 봇 셋**이 함께 선다 (§ 4.13.1) —
...
│                                **`[예정]`** 풀림 공식 기본 봇 셋이 같은 목록에 선다 (§ 4.13.1)
...
| 만든 뒤 화면 | — | 봇 배정할 반 고르기 + 「채워진 것」 재표시 + 「봇 운영」 / 「고치기」 / 「새 클래스봇」 *(앞 둘은 `dev` 사실이다 — #242 가 이미 줄였다. 셋째만 구현 전 — 현행 「새 봇」이고 `[2026-09-16]` 개정이 「새 클래스봇」으로 정했다: § 4.4.5 개정 박스)* |
...
근거가 카탈로그에 없어서**, 봇마다 갈라 적으면 그때는 정말 지어내는 것이 된다. **인도**: **`[예정]`** —
이 개정에 뒤따르는 FE PR.
...
| 자리 | 이름 | 근거 |
|---|---|---|
| 목록 헤더 CTA (봇이 있을 때) | **「새 클래스봇」** — **`[예정]`**(`dev` 는 아직 「새 봇」) | 같은 일을 하는 버튼이 **교사 홈과 운영 화면에서 이미 이 이름**이다. 같은 뜻은 같은 이름으로 (아래 개정 박스) |
...
**인도**: **`[예정]`** — 시드·API 는 데이터 PR, 카드 표시는 뒤따르는 FE PR.
...
(「공유된 봇 N개」)이 이미 무엇의 목록인지 말한다. **인도**: **`[예정]`** — FE PR.
...
**인도**: **`[예정]`** — § 4.13.1 의 카드 표시 PR 과 같은 PR.
