...
> **2026-08 재편 2차 — 화면을 한 겹 더 덜어냈다.** (위 개정과 **같은 재편의 두 번째 갈래**다 — 새 개정일을 세우지 않는다. 권위 두 문서의 빌더 절은 **#227 과 같은 승인 범위**이며, 그 절 밖은 손대지 않았다.) 「지난 봇에서 가져오기」를 없애고, 배지 체계(`꼭 골라요` / `기본값` / `내가 정함`)를 **필수 `*` / `(선택)`** 로 바꾸고, 「채워진 것」에서 배지·줄별 「고치기」·직접 정한 개수 카운터를 걷어내고, 「단계」를 콘텐츠 위쪽 한 자리로 모으고, 「이대로 만들기」를 페이지 헤더로 올렸다. 아래 본문이 이 개정까지 반영한 정의다.
> 이 개정은 **문서가 먼저** 들어간다 — 구현(`/teacher/builder`)은 뒤따르는 FE PR 이 맞춘다. 그 PR 이 머지되면 이 줄을 지운다.
> 권위 문서 두 개를 함께 갱신했다 — 핸드오프 § 4.1 과 `05_풀림_수업방_세부기획.md` § 8.1 · § 8.2 · § 13 (둘 다 사용자 명시 승인, 그 자리로 한정). 권위 묶음이 한 정의를 가리킨다.
...
길이 하나라 「어디로 갈지」를 먼저 고르게 하지 않는다. 마당 셋을 지나며 채우되, **어느 마당에서든** 「이대로 만들기」로 끝낼 수 있다 — 그 버튼은 **페이지 헤더 오른쪽에 늘 있다.**
