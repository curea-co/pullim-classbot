...
├─ /teacher/students           ← [id] 의 부모 경로가 빈 화면이 되지 않게 둔 얇은 화면 (어디서도 링크 안 함)
│  └─ /teacher/students/[id]   ← 학생 상세 — 대화 기록 · 막힌 지점 · 과정 평가
│                                 (관제소·채점 허브 두 목록이 함께 보내는 곳)
...
**메모 3 (채점 허브)**: `/teacher/grading` 은 「채점할 게 있는 학생」이 아니라 **등록된 학생 전체**를 기본으로 보여준다. 채점 대기가 0건인 학생도 목록에 남는다 — 큐만 보이면 오늘 제출하지 않은 학생이 화면에서 사라지기 때문이다. 학생 요약은 배지(도달 · 최근 접속 · 채점 대기)로만 말하고, 학생을 누르면 **새 화면을 만들지 않고** `/teacher/students/[id]` 로 보낸다. **학생 목록** 화면은 학급 관제소와 채점 허브 탭 둘이고 `/teacher/students` 는 진입점이 아니다. **학생 상세**(`/teacher/students/[id]`)로 들어오는 길은 그보다 많다 — 관제소 명단 · 교사 홈 「먼저 볼 학생」 · 채점 허브 학생 목록 · 채점 상세의 「대화 기록」 넷이 현행 계약이고, 리포트 명단은 소스에 링크가 있으나 메모 2 대로 **v1 후보라 계약에 넣지 않는다**. 들어온 곳에 따라 되돌아갈 곳이 달라지며 규칙은 [11 § 3.3.3](11-grading-hub.md). 명단은 화면마다 따로 세지 않고 데이터 한 곳에서 읽는다 — 지금은 `monitoredRoster` 이고, 이는 **현재 학생 목록이 읽는 단일 UI 원천**일 뿐 등록 명단(enrollment)의 진실원이 아니다. 채점 시드도 같은 명단·같은 수업으로 정렬해 두어, 채점 항목에서 학생 상세로 건너가도 같은 학생이다. 자세한 정의는 [11. 채점 허브](11-grading-hub.md § 3.3.0 · § 7.1).
