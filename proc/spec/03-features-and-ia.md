...
- **앞으로 가는 길은 모두 필수를 지난다** *(구현: `firstFault` — 마당을 지나는 두 길은 `faultBefore` · `goForward` 를 거쳐 그것을 부른다.)* — 「만들기」·「다음」·위쪽 「단계」로 **앞으로** 건너뛰는 것, 셋이 **같은 판정**을 지난다. 필수를 안 채우고도 앞으로 갈 수 있는 창구를 하나라도 남기면 「다음」에서 막은 것이 그 창구로 열린다. **뒤로 가는 길은 막지 않는다** — 「이전」도, 「단계」로 되돌아가는 것도. 막을 때는 그 항목이 사는 마당으로 돌려보내고, 왜 막혔는지 그 항목 밑에서 말하고, 그 자리로 초점을 옮긴다.
  ...
  - **이것을 지키는 테스트**: `apps/classbot/components/builder/__tests__/builder.test.tsx` 의 `describe('위쪽 「단계」에도 같은 관문이 걸린다')` · `describe('앞으로 가는 길의 판정 (faultBefore)')`.
  - 위 「단계 점프는 그대로 있다」와 어긋나지 않는다 — 점프도, 대상 셋도, 진행이 보이는 것도 그대로다. 앞으로 가는 점프에 판정이 붙을 뿐이다.
  - 권위: [07 § 4.1](../../input/docs-archive/07_풀림_클래스봇_핸드오프.md) 「앞으로 가는 길은 모두 필수를 지난다」 (2026-08-25 개정 · 사용자 명시 승인 · § 4.1 로 한정). 빌더 흐름을 적는 네 자리(07 § 4.1 · 05 § 8.1 · 이 절 · [04 UC-T1](04-ux-flow.md))를 같은 PR 로 함께 맞춘다 — § 4.1 이 「권위 묶음을 함께 갱신한다」고 정한 방식이다. 이 절은 그 결정을 받아 적을 뿐이고, 규칙을 세우는 것은 07 이다.
