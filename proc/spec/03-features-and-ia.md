...
**메모 2**: `/teacher/reports` 는 리포트 6종 검토뿐 아니라 **등록 학생 전원 명단**을 함께 맡는다 (§ 4.11). 리포트는 일부 학생에게만 생성되므로 리포트만 늘어놓으면 리포트가 없는 학생이 화면에서 사라진다. 명단의 거르기·정렬 규칙은 **학급 관제소 화면·교사 홈과 같은 판정**을 읽는다 (라우트 인벤토리는 이 변경에서 건드리지 않는다) — 상세는 [13 § 3.3.1](13-reports-and-emotion-checkin.md).
...
| 등록 학생 명단 | **리포트 유무와 무관하게 등록된 학생 전원** 한 줄씩 — 이름·학년 / 도달 배지 / 지름길 / 범위 이탈 / 최근 접속 배지 |
| 명단 거르개 | 전체 · 먼저 볼 학생 · 미도달 · 목표 수준 미달 · 오늘 안 들어옴 |
| 명단 정렬 | 먼저 볼 순서(기본) · 이름순 · 지름길 많은 순 · 이탈 많은 순 · 활동 오래된 순 |
| 학생 줄 진입 | `/teacher/students/[id]` — **이미 있는 화면**(대화 기록 + 과정 평가)으로 보낸다. 이 변경은 § 2.2 라우트 인벤토리를 바꾸지 않는다 |
