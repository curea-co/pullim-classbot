...
> **2026-08 재편 2차 — 화면을 한 겹 더 덜어냈다.** (위 개정과 **같은 재편의 두 번째 갈래**다 — 새 개정일을 세우지 않는다. 권위 두 문서의 빌더 절은 **#227 과 같은 승인 범위**이며, 그 절 밖은 손대지 않았다.) 「지난 봇에서 가져오기」를 없애고, 배지 체계(`꼭 골라요` / `기본값` / `내가 정함`)를 **필수 `*` / `(선택)`** 로 바꾸고, 「채워진 것」에서 배지와 줄별 「고치기」를 걷어내고, 「단계」를 콘텐츠 위쪽 한 자리로 모으고, 「이대로 만들기」를 페이지 헤더로 올렸다. 아래 본문이 이 개정까지 반영한 정의다.
...
- **「채워진 것」** — 아홉 가지(과목·학년·이름·말투·수업 자료·답 범위·평소에·틀렸을 때·반) 가운데 몇 가지를 교사가 직접 정했는지 세는 패널. 줄은 **항목 이름 : 값** 만 보여준다 — 배지도, 줄별 「고치기」도 없다. 늘 켜진 안전 세 가지(개인정보 가리기·위험한 말 막기·위기 신호 알림)는 교사가 정할 것이 없어 세는 칸 밖 고정 줄로 둔다.
