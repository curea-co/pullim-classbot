...
│  ├─ /classbot/classroom      ← 내 수업방 — 참여한 반 · 참여 코드 넣기 ([예정] #283)
│  ├─ /classbot/discover       ← [지금] 「공식 튜터 마켓」 — mock 공식 튜터 셋 + 「곧 만날 봇」 예고. nav 비노출
│  │                             [예정] #283 이 **교사가 공유한 봇**(마켓)으로 갈아끼우고 nav 를 되살린다
│  │                             (「내가 담은 봇」 바로 뒤). 빈 상태 안내만으로는 이미 반·담은 봇이
│  │                             있는 학생이 새 봇을 찾을 길이 없어 레일에 상시 자리를 준다
│  ├─ /classbot/discover/[botId] ← 마켓 봇 상세 · 담기 ([예정] #283)
│  ├─ /classbot/my-bots        ← 「담은 봇」 — 자기주도가 사는 **장소** ([예정] #283)
...
**메모 (담은 봇·공부한 날의 출처 — `[예정] #283` 의 범위가 어디서 끝나나)** *(`[2026-09-10 정정]` 신설)*: 위 세 줄(`/classbot/my-bots` · `/classbot/discover{,/[botId]}`)에 붙은 **`[예정] #283` 은 화면까지다.** 자기주도 서버(`self_enrollments` · `self_study_days` · `/api/me/self-bots` · `/api/me/study-days`)는 **#270** 으로 **`dev` 에 이미 있지만**, 그 라우트를 부르는 **훅·스토어의 「식별된 사용자 = 서버 정본 / 비로그인은 서버를 부르지 않고 localStorage」 전환은 `[예정] #273`**(훅·스토어 · 변경 파일에 `hooks/api/self-bots.ts` · `lib/store/self-learning.ts`)이 인도한다. **폴백 조건은 「비로그인」이지 「`401`」이 아니다** — 세션 만료·무효 토큰은 재인증·오류로 다룬다. **그래서 #283 의 화면은 로컬 폴백 상태로 온다** — 그 PR 에 서버 소비를 함께 요구하면 층이 섞이고(리포 `CLAUDE.md` 최상위 MUST 「한 PR = 한 계층」) #273 의 몫이 사라진다. 규칙 본문과 배정은 [`2026-06-23_classbot-dual-mode-design.md`](2026-06-23_classbot-dual-mode-design.md) 2026-09-04 박스의 `[2026-09-10 정정]` 항목이 권위다.
