...
├─ /teacher/bots               ← 봇 관리 — 내가 만든 봇 목록
├─ /teacher/bots/[botId]       ← 봇별 설정 (안전 등급 · 이탈 대응)
│
├─ /teacher/replay             ← 교사용 리플레이 리스트
└─ /teacher/replay/[id]        ← 교사용 리플레이 상세
```

**메모**: 6 운영 라우트(live·quiz·reports·grading·templates·bots)는 의도적으로 `/teacher/*` 직속에 위치 (classbot 하위 아님). 교사가 다중 봇·다중 학급을 동시 운영할 때 운영 도구를 분리된 entry로 노출.
...
| 라우트 | 이름 | 하는 일 |
|---|---|---|
| `/teacher/bots` | 봇 관리 | 교사가 만든 봇을 목록으로 보여준다. 봇을 누르면 그 봇의 설정으로 들어간다 |
| `/teacher/bots/[botId]` | 봇별 설정 | 그 봇의 운영 규칙 — 안전 등급 시간대 스케줄 · 이탈 대응 강도. 나머지 넷은 자리만 두고 「준비 중」 |

봇별 설정의 탭 여섯(안전 등급 · 이탈 대응 · 봇 이름·말투 · 수업 자료 · 평가 규칙 · 알림)과 그 안의 내용은 종전 화면 그대로다. **바뀌는 것은 앞에 목록이 한 겹 생기고, 설정이 봇 하나에 매인다는 것뿐이다.**

#### 목록에 무엇을 보여주나

봇 카탈로그(`lib/mock/classbot.ts` 의 `classBots`)가 **이미 아는 것만** 쓴다 — 없는 값을 새로 지어내지 않는다.
...
**운영 사실(운영 중·멈춤 · 붙은 학급 · 등록 인원 · 낸 과제)은 목록에 두지 않는다** — 아래 역할 분리에 따라 운영 화면 몫이다.
...
| 보여주는 것 | 운영 중·멈춤과 그 이유 · 붙은 학급과 인원 · 낸 과제 건수 · 등록 학생 | 봇 정체(이름·과목·학년) · 지금 안전 등급 · 말투 → 눌러서 규칙 고치기 |
| 시간축 | 지금 | 앞으로 |
| 봇 줄 | 운영 사실을 담은 카드 | 규칙만 담은 줄 — **같은 카드를 두 벌 만들지 않는다** |
...
[봇 빌더]는 지금 `/teacher/builder` 에 있고 사이드바에도 제 항목으로 서 있다. **다음 작업에서 [봇 관리] 하위(`/teacher/bots/new`)로 옮긴다. 이번 개정은 옮기지 않고 자리만 잡아 둔다.**

- 옮기면 바뀌는 것: 사이드바에서 [봇 빌더] 항목이 빠지고, [봇 관리] 안의 「새 봇 만들기」가 유일한 진입점이 된다.
- 그때 같이 고칠 곳: `components/shell/nav-config.ts` · 봇 관리 목록과 빈 상태의 「봇 만들기」 링크 · 운영 화면의 「새 클래스봇」 · 위 § 4.3 의 라우트 표기 · 핸드오프 § 3 IA 트리.
