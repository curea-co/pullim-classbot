...
**규칙 공유**: 거르개·정렬 판정은 학급 관제소(`/teacher/monitor`)·교사 홈(`/teacher`)과 같은 곳에서 읽는다.
교사가 같은 말(「미도달」·「목표 수준 미달」·「먼저 볼 학생」)을 화면마다 다르게 배우지 않도록 규칙을 두 벌 두지 않는다.
기준의 뜻은 화면에 문구로 노출한다. 상세: [13 § 3.3.1](13-reports-and-emotion-checkin.md).
