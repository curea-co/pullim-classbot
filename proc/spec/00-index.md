...
- **2026-08-24**: 봇 빌더(`/teacher/builder`) **8단계 위저드 → 한 길 · 세 마당** 재편을 기록 (**PR #223** — 코드와 문서가 한 PR 로 함께 들어간다). 문서가 8단계를 못 박고 있어 코드 변경이 「명세 회귀」로 읽히던 상태를 푸는 것이 목적. 갱신 범위:
  ...
  - **14-teacher-assignment-workspace.md** — § 3.2 진입점 (「테스트 발사」 미구현 명시)
  - **`input/docs-archive/07_풀림_클래스봇_핸드오프.md`** — § 3 IA 트리 + **§ 4.1** (사용자 명시 승인 하에 권위 문서 직접 갱신, v1.0 → v1.1). 종전 8단계 표는 「종전」으로 접어 보존
  참여 코드 — 배포 관문을 없앤 뒤 「교사가 학생에게 무엇을 주나」를 03 § 4.3 · 핸드오프 § 4.1 에서 못 박았다: **코드는 반에 붙으므로 확인·공유 자리는 빌더가 아니라 봇 운영 화면(`/teacher/classbot`)의 반 카드**. 현재 미구현이며(종전 8단계의 「8. 배포」도 코드를 보여주지 않았다 — 이번 재편이 만든 빈자리가 아니다) 반 카드 구현은 별도 과제.
  함께 바로잡은 것 — 종전 문서가 **미구현을 구현된 것처럼** 적고 있던 것들. ① 「배포」 산출물의 코드·링크·QR — 학생의 참여 코드 입력 → enrollment 경로는 있으나(`lib/mock/class-codes.ts` + `joinClass()`), **빌더가 코드를 만들어 보여주는 흐름과 링크·QR 은 구현된 적이 없다** ② 루브릭 5축(빌더 로컬 state, 진실원 아님) ③ 임시저장(저장 없는 데모). 셋 다 재편 이전에도 사실이었다.
