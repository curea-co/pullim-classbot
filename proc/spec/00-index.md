...
- **2026-08-24**: 봇 빌더(`/teacher/builder`) **8단계 위저드 → 한 길 · 세 마당** 재편을 **확정 설계로 기록**. 구현은 **PR #223 이 인도 중**이고 `dev` 시점 코드는 아직 8단계 위저드다 — 갱신된 절마다 그 상태를 명시했고, #223 머지 후 상태 표기를 지운다. 문서가 8단계를 못 박고 있어 #223 이 「명세 회귀」로 읽히던 상태를 푸는 것이 목적. 갱신 범위:
  ...
  **손대지 않았지만 대체되는 문서** — `input/docs-archive/05_풀림_수업방_세부기획.md` § 8.1 `Step 6: 학생 초대 → 코드/링크/QR 생성` 과 § 체크리스트 「봇 빌더 위저드 (6단계)」는 **이 개정으로 대체된다(superseded).** 봇 빌더의 권위는 `03 § 4.3` 과 핸드오프 § 4.1 이며, 참여 코드 확인·공유 자리는 빌더가 아니라 봇 운영 화면 반 카드다. 이 문서를 직접 고치지 않은 것은 `input/docs-archive` 가 read-only 권위이고 **이번 사용자 승인이 `07_핸드오프.md § 4.1` 로 한정**되기 때문이다 — 본문 갱신은 별도 승인 사안이다.
