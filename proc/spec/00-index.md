...
- **2026-08-25**: 「봇 설정」(`/teacher/settings`) → **[봇 관리]** 이름 변경 + **봇 목록(`/teacher/bots`) → 봇별 설정(`/teacher/bots/[botId]`)** 구조 변경을 기록. 봇은 여럿인데 설정 화면이 하나뿐이라 「지금 어느 봇을 고치고 있나」가 화면에 없던 것이 트리거. ⏳ **문서만 들어간다 — `dev` 에는 `/teacher/bots` 라우트가 없고 위 넷 중 하나도 옮겨지지 않았다.** 구현은 뒤따르는 FE PR 이 인도하며, 그 PR 이 머지되면 `03 § 4.4` 의 ⏳ 줄과 「구현」 칸을 지운다. 갱신 범위:
