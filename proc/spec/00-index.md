...
- **2026-08-24**: 봇 빌더(`/teacher/builder`) **8단계 위저드 → 한 길 · 세 마당** 재편을 **제안으로 기록**(아직 권위 아님). 권위 문서(`input/docs-archive/*`)는 여전히 8단계 위자드를 규정하고, **다르면 권위 문서가 앞선다.** 구현은 PR #223 이 들고 있고 `dev` 시점 코드도 아직 8단계다. 권위 전환은 별도 승인·절차이며 그 전환이 끝나야 확정 명세가 된다. 갱신 범위:
