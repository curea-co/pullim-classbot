...
- **2026-08-24**: 「봇 설정」(`/teacher/settings`) → **[봇 관리]** 이름 변경 + **봇 목록(`/teacher/bots`) → 봇별 설정(`/teacher/bots/[botId]`)** 구조 변경을 기록. 봇은 여럿인데 설정 화면이 하나뿐이라 「지금 어느 봇을 고치고 있나」가 화면에 없던 것이 트리거. 문서가 먼저 들어갔고 구현은 **#243** 이 인도했다 — `/teacher/bots` 목록과 `/teacher/bots/[botId]` 봇별 설정이 `dev` 에 있고 링크 넷도 옮겨졌다. **딥링크 둘(`monitor-roster.tsx` · `build-yards.tsx`)도 뒤이어 옮겼다**(아래 「옛 경로」 참고). 갱신 범위:
  ...
  **옛 경로** — `/teacher/settings` 는 목록으로 넘겨보내는 자리로만 남기기로 한다. 이 항목은 지우지 못하는 이유를 다른 작업이 물고 있는 두 자리(`monitor-roster.tsx` · `build-yards.tsx`)의 `?tab=` 딥링크로 적고 「그 두 자리가 옮겨오면 함께 지운다」고 썼는데, **그 조건은 상위 권위(`07` § 3)와 어긋난다** — 07 은 조건 없이 「앱 밖에서 오는 옛 주소가 404 로 끊기지 않게」 남긴다고 적는다. 두 딥링크는 뒤이어 옮겼고, **넘겨보내는 자리는 그 까닭으로 남는다**(`03 § 4.4.6` 정정, 2026-08-27 소유자 확인). 옮길 링크 넷(`nav-config.ts` · 운영 화면 봇 카드 「더보기」 · 교사 홈 처리 대기 · 학생 기록 「이탈 대응 강도」)은 `03 § 4.4.6` 표에 「지금」·「옮긴 뒤」로 갈라 적었다.
