...
- **2026-08-25**: 「봇 설정」(`/teacher/settings`) → **[봇 관리]** 이름 변경 + **봇 목록(`/teacher/bots`) → 봇별 설정(`/teacher/bots/[botId]`)** 구조 변경을 기록. 봇은 여럿인데 설정 화면이 하나뿐이라 「지금 어느 봇을 고치고 있나」가 화면에 없던 것이 트리거. 구현은 뒤따르는 FE PR 이 인도한다. 갱신 범위:
  - **03-features-and-ia.md** — § 2.2 사이트맵 두 줄 + 메모, **§ 4.4 신설**(두 화면 · 목록 항목과 그 출처 · 운영 화면과의 역할 분리표 · 옛 경로 처리 · 봇 빌더 이식 예고). 종전 § 4.4~4.10 은 § 4.5~4.11 로 한 칸씩 밀렸다
  - **09-tech-stack.md** — § 앱 트리 `bots/` 두 줄 + `settings/page.tsx` 를 「넘겨보내는 자리」로 표기
  - **10-roadmap.md** — § 2.1 교사 클래스봇 행에 갈라진 사실 추가
  - **12-student-assignment-solve.md** — § 5.1 시간대 스케줄을 정하는 자리의 경로 갱신
  - **13-reports-and-emotion-checkin.md** — § 5.1 · § 5.4 · § 연계 표의 경로·이름 갱신 (셋 다 여전히 **미구현**)
  **역할 분리** — 운영 화면(`/teacher/classbot`)은 「지금 잘 돌고 있나」, 봇 관리는 「이 봇을 어떻게 굴릴까」. 운영 사실(운영 중·멈춤 · 붙은 학급 · 인원 · 낸 과제)은 운영 화면에만 두고 봇 관리 목록에 옮겨 적지 않는다 — 같은 카드를 두 벌 만들지 않기 위해서다.
  **옛 경로** — `/teacher/settings` 는 목록으로 넘겨보내는 자리로만 남는다. 다른 작업이 물고 있어 이번에 손댈 수 없는 두 자리(`monitor-roster.tsx` · `build-yards.tsx`)의 `?tab=` 딥링크 때문이며, 그 두 자리가 옮겨오면 함께 지운다.
