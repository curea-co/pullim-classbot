...
| 풀림 클래스봇 (교사) | `/teacher/classbot` + 6 운영 라우트 | 🟢 | 메인 운영 + `/teacher/{live,quiz,reports,grading,templates,settings}` 6개 락 해제 (2026-04-29). 6KPI·Scope·라이브 피드·다중 봇·6종 리포트·하이브리드 채점·템플릿 마켓·봇 설정. **2026-08-25 — 앞으로의 정의는 `settings` 자리가 `bots` 로 바뀐 `/teacher/{live,quiz,reports,grading,templates,bots}` 다**: 봇 설정을 [봇 관리]로 갈라 `/teacher/bots` 목록 → `/teacher/bots/[botId]` 봇별 설정이 되고, 옛 `/teacher/settings` 는 목록으로 넘겨보내는 자리로만 남는다 (03 § 4.4). ⏳ **아직 구현 전이다** — `dev` 시점 라우트는 여전히 `/teacher/settings` 한 자리이고, 위 여섯 중 `bots` 만 없다. 뒤따르는 FE PR 이 인도한다 |
