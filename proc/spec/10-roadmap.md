...
| 풀림 클래스봇 (교사) | `/teacher/classbot` + 6 운영 라우트 | 🟢 | 메인 운영 + `/teacher/{live,quiz,reports,grading,templates,settings}` 6개 락 해제 (2026-04-29). 6KPI·Scope·라이브 피드·다중 봇·6종 리포트·하이브리드 채점·템플릿 마켓·봇 설정. **2026-08-24 — 앞으로의 정의는 `settings` 자리가 `bots` 로 바뀐 `/teacher/{live,quiz,reports,grading,templates,bots}` 다**: 봇 설정을 [봇 관리]로 갈라 `/teacher/bots` 목록 → `/teacher/bots/[botId]` 봇별 설정이 되고, 옛 `/teacher/settings` 는 목록으로 넘겨보내는 자리로만 남는다 (03 § 4.4). **구현됐다**(#243) — `/teacher/bots` 목록과 `/teacher/bots/[botId]` 봇별 설정이 있다. 옛 `/teacher/settings` 는 목록으로 넘겨보내는 자리로 남는다 |
| 봇 빌더 (교사) | `/teacher/builder` | 🟢 | **한 길 · 세 마당** (#223, 03 § 4.3) — 「한 길 · 세 마당」(봇 소개 · 보고 답할 것 · 가르치는 법) + 만든 뒤 화면(반 배정) · 과목만 필수 · 점프 대상 8 → 3 · 「채워진 것」 아홉 줄(항목 이름 : 값) · 목소리·루브릭·임시저장·빌더 내 시간대 Scope 스위치 제거 |
