...
| 풀림 클래스봇 (교사) | `/teacher/classbot` + 6 운영 라우트 | 🟢 | 메인 운영 + `/teacher/{live,quiz,reports,grading,templates,settings}` 6개 락 해제 (2026-04-29). 6KPI·Scope·라이브 피드·다중 봇·6종 리포트·하이브리드 채점·템플릿 마켓·봇 설정. **봇 설정은 2026-08-25 에 [봇 관리](`/teacher/bots` 목록 → `/teacher/bots/[botId]` 봇별 설정)로 갈랐다 (03 § 4.4)** |
