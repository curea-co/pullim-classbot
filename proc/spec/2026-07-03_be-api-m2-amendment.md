...
> **(2026-09-16) 정본 스키마 쪽 개정 — `bots` 가 `classes` 에서 갈라진다.** ADR-063(2026-07-04) 이 정한 「bot == classes 행 재사용」(bot 전속 속성은 1:1 `class_bot_profiles`)은 [완성 설계](2026-09-16_classbot-completion-design.md) 결정 ③ · pullim-api **ADR-091 `[예정]`**(`docs/design/_platform/adr/ADR-091-classbot-bots-분리.yaml` · refines ADR-063 — pullim-api 브랜치 `docs/classbot-classroom-spine` 에만 있고 PR 승인 대기, 경로는 머지되면 그대로 풀린다)으로 **개정된다** — `bots` 를 별도 표로 세우고 `classes.bot_id`(nullable) 가 가리킨다. `classes` 는 `subject`·`grade`(nullable)도 스스로 든다 — 봇은 선택이라서다. 한 봇이 여러 반에 붙을 수 있고, 반은 한 번에 봇 하나. 「할당·교체」가 `PUT /classbot/classes/:id/bot` 이라는 행위가 되고, `messages.bot_id` 가 보낸 시점의 봇을 적는다. 근거는 요구 R4(「교사가 반에 클래스봇을 할당한다」 — 지금 구조에는 할당이라는 행위가 없다)와 설계 § 4 표. 위 §1 의 「봇과 반이 같은 교사 소유」 복합 FK 규칙은 `PUT …/bot` 의 술어(봇의 operator == 반의 operator)로 옮겨 간다. 인도는 pullim-api PR 1(스키마·이행) · PR 2(라우트) — 표의 정본은 그 리포 [`data-model.md`](../../../pullim-api/docs/design/services/classbot/data-model.md) 다.
