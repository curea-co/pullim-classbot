...
> 학생이 들어올 때까지 아무것도 낼 수 없다. 빈 방 발사는 `target_student_ids = []`(= 반 전체)
> 로 저장되고, 뒤에 참여한 학생이 그 과제를 받는다.
>
...
> 이 개정을 코드로 옮기는 것은 **#269** 이고(같은 파일에서 `rosterUnknown` 을 세우고
> `targetValid` 를 `!rosterUnknown && (students.length === 0 || selectedIds.length >= 1)` 로
> 바꾼다), 그 PR 이 `dev` 에 들어오면 이 「`[예정]`」 표기를 지운다. 즉 아래 표의 「대상 학생」
