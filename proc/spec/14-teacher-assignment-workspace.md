...
- 봇 빌더 산출물을 **소비**한다 — 봇이 가진 수업 방식·톤·답 범위가 과제 기본값으로 자동 채움. **채점 기준은 봇에서 오지 않는다** — 문항 배점에서 파생해 과제 출제에서 만든다 (`03 § 4.3` 대응표 ⑥).
...
- **G4**: 봇 빌더 산출물(수업 방식·톤·답 범위) 자동 흡수로 입력 부담 1/3
...
- [M2] 봇 선택 시 빌더 산출물 자동 흡수 (수업 방식 → 기본 mode, 답 범위·톤 → 기본값). 채점 기준은 봇이 아니라 **문항 배점에서 파생**한다
- [M3] 모드별 분기 UI:
  - `practice`: 봇 힌트 단계 설정
...
| 자동 채움 안내 | "{봇 이름}의 수업 방식이 기본값으로 들어왔어요." |
