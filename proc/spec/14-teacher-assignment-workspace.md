...
> **[2026-09-04 개정] 「대상 학생 1명 이상」을 걷었다.** 이 표의 원안은 명단이 먼저 있고 교사가
> 거기서 고르는 흐름을 전제했는데, **참여 코드**가 생기면서 「반을 먼저 열고 학생이 뒤에
> 들어오는」 상태가 정상이 됐다. 1명 이상을 요구하면 교사는 반을 열어 놓고 학생이 들어올
> 때까지 아무것도 낼 수 없다. 빈 방 발사는 `target_student_ids = []`(= 반 전체) 로 저장되고,
> 뒤에 참여한 학생이 그 과제를 받는다.
>
> 대신 **명단을 못 읽은 상태**(조회 실패·응답 전)는 발사를 막는다. 그 상태의 빈 배열은
> 「학생이 0명인 반」과 모양이 같아, 막지 않으면 교사가 명단을 한 번도 보지 못한 채 전원에게
> 나간다. 구현: `app/(teacher)/teacher/assignment/new/assignment-form.tsx` 의 `rosterUnknown`.
...
| 대상 학생 | **0명도 허용** (빈 방 = 반 전체로 발사, 뒤에 들어온 학생이 받는다) · 명단을 **못 읽은** 상태는 불가 | `[발사]` 비활성 |
