...
**후속 과제 (진입점 아님)**:
- 봇 빌더 「만든 뒤 화면」에 "테스트 발사" 선택지를 붙이는 안 — **아직 붙이지 않는다.** 그 화면의 구성은 `03 § 4.3` 이 「반 고르기 + 「채워진 것」 재표시 + 「이어서 고치기」 / 「봇 하나 더 만들기」」로 못 박고 있고 여기에 발사 선택지는 없다. 붙일지는 만든 뒤 화면이 인도된 뒤(#223) 따로 정한다
