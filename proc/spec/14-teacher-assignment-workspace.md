...
- 봇 빌더 산출물 소비는 **아직 단원 하나뿐이다** — 봇을 고르면 그 봇의 커리큘럼 첫 단원이 들어온다(`handleBotChange`). 모드는 `practice` 고정 기본값이고, 톤·답 범위는 **소비하지 않는다.** 저장 payload 가 옮기는 것은 `subject` · `grade` · `chapter*` 다.
...
| **② 문항** | 단원 from-to · 문항 수 · 자동 생성 / 수동 선택 토글 · (수동 시) 문항 추가 모달 | 빌더 마당 2 「수업 자료」 RAG 범위에서 단원 자동 |
...
| `chapterFrom`, `chapterTo` | 봇 빌더 마당 2 「수업 자료」 RAG 인덱스에서 선택 (P0 시드는 cb_001 기준 "미적분 III") |
