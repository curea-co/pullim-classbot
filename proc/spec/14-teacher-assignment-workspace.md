...
- [C2] 루브릭 사전 편집 — 발사 전 항목별 가중치 조정 (현재는 문항 배점에서 파생한 기본값)
...
**후속 과제 (진입점 아님)**:
- 봇 빌더 「만든 뒤 화면」에 "테스트 발사" 선택지를 붙이는 안 — **이 문서는 결정하지 않는다.** 그 화면의 구성은 `03 § 4.3` 이 다루고, 붙일지 말지는 그 절과 함께 따로 정한다
...
| **② 문항** | 단원 from-to · 문항 수 · 자동 생성 / 수동 선택 토글 · (수동 시) 문항 추가 모달 | 선택한 봇의 curriculum 첫 단원 자동 선택 |
...
> 학생이 들어올 때까지 아무것도 낼 수 없다. 빈 방 발사는 **빈 배열 = 반 전체**로 저장되고,
...
>
> 대신 **명단을 못 읽은 상태**(조회 실패·응답 전)는 발사를 막는다. 그 상태의 빈 배열은
...
| 대상 학생 | **0명도 허용** (빈 방 = 반 전체로 발사, 뒤에 들어온 학생이 받는다) · 명단을 **못 읽은** 상태(`rosterUnknown`)는 불가 — **#269 로 `dev` 에 있다**. *(`[2026-09-14 정정]` 종전 이 칸은 그 줄을 **`[예정]` (#269)** 로 두고 **`[지금]`** 1명 이상 필수(`targetIds.length >= 1`)를 나란히 적었다.)* | `[발사]` 비활성 |
