...
> **[2026-09-04 개정 · `[예정]`] 「대상 학생 1명 이상」을 걷는다.** 이 표의 원안은 명단이 먼저
...
> 나간다.
>
> **구현 시점** — `dev` 의 `app/(teacher)/teacher/assignment/new/assignment-form.tsx` 는 아직
> `const targetValid = targetIds.length >= 1;` 로 1명 이상을 강제하고 `rosterUnknown` 도 없다.
> 이 개정을 코드로 옮기는 것은 **#269** 다. 그 PR 은 같은 파일에서 `rosterUnknown` 을 세우고
> `targetValid` 를 `!rosterUnknown && (students.length === 0 || selectedIds.length >= 1)` 로
> 바꾼다. **`selectedIds` 는 `targetIds` 를 이름만 바꾼 것이 아니다** — `targetIds` 는
> `string[] | null` 로 바뀌어 「교사가 아직 고르지 않았다」(`null`)를 표현하고, `selectedIds`
> 는 거기서 파생된다(`targetIds ?? students.map(s => s.id)`). 즉 **`targetIds` 는 그대로
> 남고 파생값이 하나 는다** — 후속 작업자가 rename 으로 읽지 않도록 적어 둔다.
> 그 PR 이 `dev` 에 들어오면 이 「`[예정]`」 표기를 지운다. 즉 아래 표의 「대상 학생」
> 줄은 **#269 가 통과해야 할 기준**이지 현재 동작이 아니다.
...
| 대상 학생 | **`[예정]` (#269)** **0명도 허용** (빈 방 = 반 전체로 발사, 뒤에 들어온 학생이 받는다) · 명단을 **못 읽은** 상태는 불가.<br>**`[지금]`** 1명 이상 필수 (`targetIds.length >= 1`) | `[발사]` 비활성 |
