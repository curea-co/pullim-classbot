...
**Batch 1 — G1 의사결정 불요 (병렬 처리)** — [PR #53](https://github.com/curea-co/pullim-classbot/pull/53) 머지

- [x] **B1: F2 `/teacher/replay` 라우트 신규 + broken link 해소** — list + detail + getTeacherReplays/getReplayById accessor + nav 추가
- [x] **B2: F3 빌더 deploy 실효화** — toast + router.push('/teacher/classbot?deployed=<name>') + DeployedBanner
- [x] **B3: F9 enrollment 토글** — /teacher/classbot에 18명 활성/비활성 토글 섹션
- [x] **B4: F10 scope 자동 스위치 학생 인지** — chat 헤더에 scope details disclosure (시간대별 정책 표)

**Batch 2 — D1·D2 의사결정 후 (F1 라이브 묶음)** — [PR #54](https://github.com/curea-co/pullim-classbot/pull/54) 머지

- [x] **B5: 학생 홈 LIVE 헤더 CTA 격상** — 그라데이션 카드 + 첫 라이브 봇 직접 진입
- [x] **B6: `/classbot/live/[botId]` 라우트 골격** — generateStaticParams(isLive), 종료 봇은 chat redirect
- [x] **B7: LiveSessionPanel 4영역** — 슬라이드+음성(D1) / transcript stream / 학생 질문 모더레이션(D2) / 퀴즈

**Batch 2.5 — 즉석 퀴즈** — [PR #55](https://github.com/curea-co/pullim-classbot/pull/55) 머지

- [x] **B8: F4 즉석 퀴즈 생성 UI** — Zustand quiz store + QuizLauncher 모달 + LiveQuizCard 구독

**Batch 3 — D3·D4 의사결정 후** — [PR #56](https://github.com/curea-co/pullim-classbot/pull/56) 머지

- [x] **B9: F2 리플레이 review 단계 액션** — ReplayReviewActions client + replay store(persist) + 핵심메시지 편집/승인 발송
- [x] **B10: F5 위기 신호 개입 액션** — CrisisInterventionPanel + 학생 상세 모달 + 1:1 chat CTA

**Batch 4 — 알림** — PR #57

- [x] **B11: F7 채점 완료 학생 알림** — GradingNotificationCard (최근 5분 내 submission)
