...
/**
 * chat 인삿말 봇 데이터 단일 출처 검증 (2026-05-13).
 *
 * - chat/page.tsx에서 `bot.id === ...` 하드코딩 분기 제거됨
 * - 봇 chip 3개(cb_001 친근 / cb_002 정중 / cb_003 스파르타) 전환 시
 *   인삿말이 각 봇의 `greeting` 필드 톤(반말/존댓말/단호반말)로 자연스럽게 바뀌어야 함
 * - key={bot.id}로 ChatPanel remount되면서 첫 메시지가 새 봇의 greeting으로 교체됨
 */

test.describe('chat 인삿말 봇별 변화', () => {
  test('cb_001 친근(반말) → cb_002 정중(존댓말) → cb_003 스파르타(단호반말) 전환', async ({ page }) => {
    await page.goto('/classbot/chat', { waitUntil: 'networkidle' });

    // 봇 chip strip
    const chips = page.getByRole('button', { pressed: false }).or(page.getByRole('button', { pressed: true }));
