...
/**
 * 과제 출제 화면(`/teacher/assignment/new`)의 제목을 **하이드레이션 뒤에도 남도록** 넣는다.
 *
 * `page.goto` 뒤 곧바로 `fill` 하면 React 가 붙기 전에 닿은 값이 **지워진다** — 제어
 * 컴포넌트라 하이드레이션 직후 `value=''` 로 다시 그려지기 때문이다. 그러면 `titleValid` 가
 * 거짓이라 발사 버튼이 계속 disabled 이고, 뒤따르는 클릭이 30초를 기다리다 죽는다.
 ...
 * 3주째 성공·실패를 오갔다.
 *
 * 그래서 **값이 실제로 남을 때까지** 다시 넣는다. 같은 자리가 스펙 파일 셋에 다섯 벌 있어서
 * 여기 한 곳으로 모았다 — 고칠 자리가 하나여야 다음에 또 놓치지 않는다.
 * @param page - 출제 화면이 열려 있는 페이지
 * @param title - 넣을 과제 제목 (5자 이상이어야 발사 버튼이 열린다)
 */
