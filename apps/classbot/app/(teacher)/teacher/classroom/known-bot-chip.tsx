 ...
 *
 * 원천은 이 세션이 아는 `ClassDto`(`useKnownClassSummary` — 반 생성·봇 할당·코드 재발급 응답)뿐이다. 옛 `profile`
 * (bot == class · `GET /bots*`)로 「봇 없음」을 단정하지 않는다 — `POST /classes` 로 만든 반은 profile 행이 영영 없어
 * `classes.bot_id` 가 차 있어도 늘 「봇 없음」이 되고, 같은 화면의 「봇」 탭과 반대 말을 하게 된다(#355 리뷰 S1).
 * 그래서 **모른다 · 없다 · 이 봇** 셋을 그대로 그린다: 모르면 칩을 비운다(빈 칩은 「값이 비었다」가 아니라 「모른다」로
 * 읽힌다), 없으면 「봇 없음」, 있으면 아바타 + 이름.
 */
export function KnownBotChip({
  known,
  'data-testid': testId,
}: {
  /** `useKnownClassSummary` 결과 — `undefined` 모른다 · `bot: null` 없다. */
