 ...
 * 「무엇을 스스로 골랐나」다. 인사말·톤은 여전히 안 나간다 — 그건 아이와 봇 사이의 것이다.
 *
 * 「아이가 보는 얼굴과 부모가 보는 얼굴이 같아야 한다」는 이제 **화면 쪽이 지킨다** —
 * 역할을 가리지 않고 `components/classbot/bot-avatar.tsx` 한 컴포넌트가 얼굴을 정한다
 * ([08 § 14.1.1] 예외 2). 그래서 `avatarEmoji` 를 **그리는 화면은 지금 없다.**
 * 그래도 계속 내보내는 것은 그 자리가 데이터 계약이기 때문이다 — **읽지 않을 뿐 지우지 않는다.**
