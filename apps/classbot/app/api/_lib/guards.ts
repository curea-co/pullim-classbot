...
/*
  ...
  `resolveActor()`(위)와 달리 이 둘은 **DB 를 보지 않는다** — 기존 라우트가 쓰는
  `getCurrentUserIdFromRequest()` 의 동기 결과를 그대로 받는다.
*/

/** 가드가 보는 최소 모양 — `resolveActor()` 결과와 `getCurrentUserIdFromRequest()` 결과 둘 다 맞는다. */
type RoleCheckable = { role: ActorRole; isIdentified: boolean };
...
export function denyUnlessRole(
  ...
  if (!allowed.includes(actor.role)) return forbidden(forbiddenMessage);
  return null;
}

/** 학생 본인 표면 — 교사·학부모·admin 은 여기로 들어오지 않는다. */
export function denyUnlessStudent(actor: RoleCheckable): NextResponse | null {
