...
/**
 * `consent_logs.type` 이 받는 값 전부 — 스키마의 enum 배열과 **같은 일곱**.
 *
 ...
 *
 * ## ⛔ 여기에 동의 정보를 더하지 마라 — 그것이 누출의 출처다
 *
 * **연결된 자녀는 전원 이 배열에 있다**(05 § 11.4 규칙 2). 미동의 자녀도 이름·관계는
 * 실리고, 내용만 빈다 — `bots: []` 이고 `streak` 은 전부 0 이다. 그래서 **미동의 자녀와
 * 「동의했지만 아직 아무것도 안 한 자녀」의 값이 완전히 같다.**
 *
 * 그 성질은 **동의 정보가 여기 없기 때문에** 성립한다. `scope_label` · `expires_at` 같은
 * **동의 행에서만 오는 값**을 더하면 미동의 자녀는 그것이 `null` 이 되고,
 * `scopeLabel === null` 이 곧 「동의 안 함」이 된다 — 규칙 2 가 그 자리에서 깨진다.
 * 종전 판에 그 둘이 있었고 걷었다(라우트 머리주석 ①).
 *
 * ⛔ **반대로 미동의 자녀를 배열에서 빼는 것도 안 된다.** 학부모는
 * `GET /api/parent/children` 에서 연결 자녀 전원을 이미 받으므로, 두 응답을 **대조하면**
 * 빠진 자녀가 곧 미동의 자녀다(라우트 머리주석 ②). 한 번 그렇게 썼고 되돌렸다.
 *
 * 공유 범위·만료가 필요한 곳은 **학생 자기 화면**이다 — `MyConsentRow` 가 그것을 준다.
 *
 * 그래서 **필드 마스킹도 쓰지 마라.** 새 필드의 기본값은 「안 나감」이어야 하고, 여기에
 * null 자리를 만들면 다음 사람이 그 자리에 감정·대화 요약을 넣는다.
 */
export interface ParentSelfStudyChild {
  id: string;
  name: string;
  relation: 'mother' | 'father' | 'guardian';
  /** 스스로 담은 봇 — 담은 순(오래된 것 먼저). 미동의·무활동이면 **빈 배열**. */
  bots: ParentSelfStudyBot[];
  /** 미동의·무활동이면 `{ count: 0, lastStudyDate: null, thisWeekDays: 0 }` — 같은 값이다. */
 ...
 *
 * `GET /api/parent/children` 과 **경로만 분리했다** — 한 응답에 동의 축이 둘이 되면 다음
 * 사람이 새 필드를 어느 축 뒤에 두어야 할지 알 수 없기 때문이다.
 *
 * **응답 모양은 그쪽과 같다**: 연결 자녀 **전원** + 게이트된 읽기 + 빈 배열, 그리고 동의
 * 정보는 없다(05 § 11.4 규칙 2). 축이 몇 개든 같은 모양이라 규칙이 라우트마다 다르게
 * 나타나지 않는다.
 *
