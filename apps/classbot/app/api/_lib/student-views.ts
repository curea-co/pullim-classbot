/**
 * 학생 시점 읽기 조각 — 지금 부르는 것은 **학부모(`/api/parent/children`) 하나**다.
 *
 * *(`[계획 PR 8 정정]` 종전에는 학생 본인 표면 `GET /api/me/classrooms` 와 같이 썼다. 그
 * 라우트는 정본 `GET /classbot/bots?role=student` 와 겹쳐 걷혔고, 이 조각은 **범위 밖으로
 * 남은 학부모 읽기가 아직 쓰기 때문에** 남는다. 아래 「두 화면이 같은 질의여야 한다」는
 * 설계 이유이자, 학생 본인 표면이 이 리포로 돌아올 일이 없다는 뜻이기도 하다.)*
 *
