...
/**
 * 반·과제 축은 **#280 이 이미 인도했다** — 이 블록은 그 라우트를 다시 만들지 않고,
 * 이 PR 이 켠 스위치(`class_assignment_summary` 부여)가 **그 게이트와 맞물리는지**만 본다.
 *
 * ## 여기서 「동의 조회」를 찾지 마라 — `/api/parent/children` 에는 없다
 * 그 라우트는 동의를 **따로 한 번 읽지 않는다.** 동의 EXISTS 가 반·과제 질의의 `where`
 * **안**에 서고(`_lib/parent-views.ts` → `livingConsentExists`), 미동의면 그 질의가 0행으로
 * 돌아온다. 그래서 「동의를 먼저 읽고 그 결과로 데이터 질의를 부를지 고르는」 두 걸음이
 * 존재하지 않는다 — 05 § 11.4 규칙 1 이 요구하는 모양이 바로 이것이다(두 걸음 사이의 틈에서
 * 학생이 공유를 거두면 이미 통과한 판정이 두 번째 질의를 열어 준다).
 *
 * 그러니 이 블록의 테스트는 **큐에서 무엇이 소비되지 않았는지**를 보지 않는다. 그건 두 걸음
 * 설계에서만 뜻이 있는 관찰이고, 여기서는 질의가 **게이트를 지고 나가는지**를 본다.
 */
describe('반·과제 축은 자녀 동의 뒤에 있다 — 이름은 주고 내용은 안 준다', () => {
  const LINKED = [{ id: 'student_001', name: '서연', relation: 'mother' }];

  /**
   * 큐 순서 = 라우트가 읽는 순서.
   *  ① resolveActor 의 `users.role` → ② 자녀 링크 → ③ 반 ④ 과제
   *
   * ③·④ 는 **동의 여부와 무관하게 나간다.** 게이트가 그 질의 안에 있으므로, 미동의는
   * 「질의를 안 보냄」이 아니라 「0행으로 돌아옴」으로 나타난다.
   * @param views - ③·④ 가 돌려줄 행(미동의를 흉내내려면 빈 배열 둘)
   */
  ...
  it('반·과제 질의 **둘 다** 동의 술어를 지고 나간다 — 받는 사람·타입·철회·만료 전부', async () => {
    ...
    // ① 은 role, ② 는 링크. 데이터 질의는 마지막 둘이다.
    const dataWheres = whereSpy.mock.calls.slice(-2).map((c) => render(c[0]));
    expect(dataWheres).toHaveLength(2);

    for (const where of dataWheres) {
      // 받는 사람까지 맞춘다 — 다른 보호자에게 준 동의로 이 화면이 열리지 않는다.
      expect(where.params).toContain('parent_001');
      // 축은 반·과제 하나뿐. 자기주도 동의로 이쪽이 열리면 안 된다.
      expect(where.params).toContain('class_assignment_summary');
      expect(where.params).not.toContain('self_study_summary');
      expect(where.text).toContain('"revoked_at" is null');
  ...
  it('동의가 있으면(=질의가 행을 돌려주면) 반·과제가 실린다', async () => {
    const res = await callAsParent([
