...
describe('날짜 판정 — 클라이언트가 주장할 수 있는 범위', () => {
  ...
  /**
   * ⛔ 회귀 방지. 2년 전에 **없는 날**(2월 29일)을 `Date.UTC` 에 그대로 넘기면 다음 달로
   * 정규화되어(2026-03-01) 하한이 하루 **뒤로 밀린다** — 창이 넓어지는 게 아니라 좁아지고,
   * 계약상 받아 줘야 할 `2026-02-28` 이 2028년 2월 29일 하루 동안 거절된다.
   */
  it('2년 전에 없는 날(2/29)은 그 달의 마지막 날로 당긴다 — 넘치지 않는다', () => {
    expect(backfillFloor('2028-02-29')).toBe('2026-02-28');
    expect(backfillFloor('2024-02-29')).toBe('2022-02-28');
  });

  it('말일은 그대로 말일이다 — 클램프가 멀쩡한 날짜를 건드리지 않는다', () => {
    expect(backfillFloor('2026-01-31')).toBe('2024-01-31');
    expect(backfillFloor('2026-03-31')).toBe('2024-03-31');
    expect(backfillFloor('2026-12-31')).toBe('2024-12-31');
  });

  it('윤일 하한 당일은 받아 준다 — 하루가 잘려 나가지 않는다', () => {
    // 하한이 2026-03-01 로 밀렸다면 이 줄이 false 다.
    expect(isRecordableDay('2026-02-28', '2028-02-29')).toBe(true);
    expect(isRecordableDay('2026-02-27', '2028-02-29')).toBe(false); // 하한 하루 전
  });
