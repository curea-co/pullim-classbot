...
function CriterionGroup({
  ...
  return (
    <div className="mt-2 first:mt-0">
      <p className="text-pullim-slate-500 inline-flex items-center gap-1 text-2xs font-bold tracking-wider uppercase">
        <ListChecks className="h-3 w-3" />
        {label} {items.length}개
      </p>
      {items.length === 0 ? (
        <p className="text-pullim-slate-500 mt-1 text-2xs">없어요</p>
      ) : (
