 ...
 * 고정 봇 응답 — **이름을 부르지 않는다.**
 *
 * 종전에는 「안녕 **서연**! …」이었다. 이 데모는 신원을 읽지 않으므로(아래 머리주석 — API
 * 미연결) 누가 가입하든 데모 페르소나의 이름으로 맞이했다. 세션 이름을 끌어와 부르는 것도
 * 답이 아니다 — 이 칸은 「봇이 어떻게 답하는지」를 한 번 보여 주는 자리라 신원 배선을 들이면
 * 데모가 세션 복원을 기다리게 된다. 그래서 **부르지 않는 쪽**으로 둔다.
 */
const BOT_REPLY =
  '안녕! 나는 수학봇이야. 오늘 진도·기울기·y절편·그래프 뭐든 편하게 물어봐 😊';
  ...
  return (
    <div className="border-pullim-slate-200 bg-card flex flex-col gap-0 overflow-hidden rounded-xl border text-sm shadow-sm">
      ...
      <div className="bg-pullim-blue-600 flex items-center gap-2 px-3 py-2">
        ...
        <span className="text-pullim-blue-700 ml-auto rounded-full bg-white px-2 py-0.5 text-2xs font-bold">
          데모
        </span>
      </div>
