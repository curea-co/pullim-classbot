 ...
 *     학부모가 받는 `ParentAssignmentItem` 에는 그 칸이 **아예 없다** — 05 § 11.4 가 이 축에서
 *     답안·점수를 뺐기 때문이다(`app/api/_lib/parent-views.ts` 의 칸별 주석).
...
import type { ParentAssignmentItem, ParentChildItem } from '@/hooks/api/types';
...
export function isDueSoon(a: ParentAssignmentItem): boolean {
 ...
 */
export function childAssignmentStatus(a: ParentAssignmentItem): ChildAssignmentStatus {
 ...
 */
export function sortByUrgency(rows: ParentAssignmentItem[]): ParentAssignmentItem[] {
...
export function countChildAssignments(rows: ParentAssignmentItem[]): {
  remaining: number;
  done: number;
  late: number;
} {
 ...
 * ## 왜 「0개」를 그냥 그리면 안 되나
 *
 * 반·과제가 자녀 동의 뒤로 옮겨 간 뒤(05 § 11.4), `GET /api/parent/children` 은 **동의하지
