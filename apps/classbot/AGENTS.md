...
- **UI 소스는 3레인**: ① **PUDS 원격 벤더링**(`app/tokens/*`, `components/ui/{dashboard-shell,os-rail,os-tabbar,page-header,rail-collapse-context,service-switcher,service-icon,breadcrumb,skip-link}.tsx`, `lib/cn.ts`) — **로컬 수정 금지**, 재설치가 덮어쓴다 · ② **로컬 base-ui 프리미티브**(`components/ui/*` 나머지) — **PUDS 프리미티브로 교체 금지**(`shadcn add` 는 머지가 아니라 **덮어쓰기**이고, 이름이 겹치는 16종은 `files[].target` 이 16/16 전부 `components/ui/<name>.tsx` 라 이 리포의 수정이 말없이 사라진다. **엔진이 갈린다는 옛 근거는 PUDS v0.5.0 에서 죽었다** — 이제 양쪽 다 `@base-ui/react` 다) · ③ **서비스 고유**(나머지) — 자유.
  **새 PUDS 아이템 설치 전 확인은 둘이다** — ① `files[].target` 충돌 ② **이 앱에 아직 없는 npm 패키지**를 끌고 오는가. 둘 다 전이 의존(`registryDependencies`)까지 재귀로 펼쳐 본다. 죽은 것은 **대상이 `@radix-ui/*` 하나뿐이던 좁은 의존성 검사**이지 의존성 검사 자체가 아니다 — 대상을 미설치 패키지 전부로 **넓혀서** 남겼다. 실측으로 `combobox` 는 ① 만, `data-table` 은 ② 만 걸린다(서로를 대신하지 못한다).
