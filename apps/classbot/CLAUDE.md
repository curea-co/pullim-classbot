...
| 학부모 라우트 | `app/(parent)/parent/{,assignments}` | `/parent`(자녀 요약) · `/parent/assignments`(자녀 과제) — **#281 로 `dev` 에 있다.** `/parent/self-study` 는 **`[예정]`** #271 이 인도한다(`proc/spec/03 § 2.3`). **금지 영역이 아니라는 것이 2026-09-07 의 변경**이고, 내용은 자녀 동의 뒤에만 보인다(`05 § 11.4` · 게이트는 #280) |
...
- 공유 셸 / UI / nav-config 수정 — 클래스봇 한 도메인만 쓰는 상황이라 보통 안전하지만, role/nav 변경은 보고 후 진행
  ...
  - **이미 승인된 것(2026-09-10)**: **학생 레일에 항목 셋을 더하는 것** —
    ...
    **추가만**이고 기존 항목의 순서·라벨·경로·아이콘, `Role` union, 헤더 표 셋, `studentBottomTabs`(셋 그대로),
    ...
    → **인도됨**: #283(학생 화면)이 `dev` 에 넣었다.
  - **이미 승인된 것(2026-09-10 · 교사 레일)**: **교사 레일에 항목 둘을 더하는 것** —
    `/teacher/classroom`(「내 수업방」 · 홈 대시보드 뒤) · `/teacher/marketplace`(「봇 마켓」 · 봇 관리 뒤).
    **추가만**이고 기존 항목의 순서·라벨·경로·아이콘, `Role` union, 헤더 표 셋, `studentBottomTabs`,
    학생·학부모 레일, breadcrumb 은 **건드리지 않는다.** 상세 화면(`/teacher/marketplace/[botId]`)은
    레일에 싣지 않는다 — 목록에서만 들어간다.
    **자리와 순서를 이 승인이 새로 정한 것이 아니다** — `dev` 의 `nav-config.ts` 가 그 두 자리를 주석으로
    비워 두고 위치까지 적어 두었고(「그 화면이 도착하는 PR 에서 홈 바로 뒤에」·「봇 관리 뒤에」),
    `proc/spec/03-features-and-ia.md` § 2.2 가 세 라우트를 `[예정]` #269 로 등재해 두었다.
    학생 레일과 같은 이유로 **화면과 nav 는 같은 PR 에 있어야 한다** — `nav-config.test.ts` 가
    「레일의 모든 항목에 대응 page 가 있어야 한다」를 강제한다.
    → **인도**: `[예정]` #269(교사 화면).
  - **이미 승인된 것(2026-09-15 · 교사 레일)**: **교사 레일에서 항목 하나를 빼는 것** —
    `/teacher/builder`(「봇 빌더」). 소유자 지시 그대로다 — 「사이드바에 위치한 봇 빌더 메뉴를 제거해
    (다른 진입점이 더 있으며, 더 자연스러워)」.
    앞의 셋이 전부 **더하는 것**이었던 것과 달리 이번은 **빼는 것**이라, 무엇이 안 바뀌는지를 적는다:
    **나머지 항목의 순서·라벨·경로·아이콘, `Role` union, 헤더 `Record<Role,…>` 표 셋,
    `studentBottomTabs`, 학생·학부모 레일, breadcrumb 은 건드리지 않는다.**
    **이번 승인이 새로 정한 것은 자리가 아니라 「묶여 있던 예약을 둘로 가르는 것」이다.**
    예약 자체는 서 있었다 — `proc/spec/03-features-and-ia.md` § 4.4.7 이 「옮기면 사이드바에서
    [봇 빌더] 항목이 빠진다」를, `components/shell/nav-config.ts:122` 가 「TODO(봇 빌더 이식):
    다음 작업에서 이 항목을 걷고 [봇 관리] 하위(`/teacher/bots/new`)로 옮긴다」를 적어 두었다.
    **다만 둘 다 제거를 경로 이동과 묶어 한 덩이로 예약했다** — 2026-09-10 교사 레일 승인이
    「`nav-config.ts` 가 그 두 자리를 주석으로 비워 두고 위치까지 적어 두었다」로 예약과 실행이
    **정확히 같은 단위**였던 것과 다르다. 이번은 그 묶음의 **레일 쪽 절반만** 떼어 하고,
    경로 이동(`/teacher/bots/new`)은 별건으로 남는다(§ 4.4.7 이 「꼭 해소한다」로 적는다
    — **`[예정]` #322**. `dev` 의 § 4.4.7 은 아직 옛 다섯 줄 판본이라 그 문구가 없다).
    **「화면과 nav 는 같은 PR 에 있어야 한다」는 앞 조항은 이번엔 해당 없다** — 빼는 쪽이라
    `nav-config.test.ts`(`:57-64` 의 「레일의 모든 항목에 대응 page 가 있다」)가 깨지지 않고,
    `/teacher/builder` page 는 **그대로 남는다**(앱 안에 그리로 가는 자리가 여럿 더 있다
    — § 4.4.7 의 진입점 표, **`[예정]` #322** 가 세운다. 그 표가 오기 전에도 자리는 실재한다:
