...
- 공유 셸 / UI / nav-config 수정 — 클래스봇 한 도메인만 쓰는 상황이라 보통 안전하지만, role/nav 변경은 보고 후 진행
  ...
  - **이미 승인된 것(2026-09-15 · 교사 레일)**: **교사 레일에서 항목 하나를 빼는 것** —
    ...
    **자리를 이 승인이 새로 정한 것이 아니다 — 예약돼 있던 것을 실행한다.** `proc/spec/03-features-and-ia.md`
    § 4.4.7 이 「옮기면 사이드바에서 [봇 빌더] 항목이 빠진다」를 적어 두었고, `nav-config.ts` 의 그 줄 위에
    「TODO(봇 빌더 이식): 다음 작업에서 이 항목을 걷고 [봇 관리] 하위(`/teacher/bots/new`)로 옮긴다」
    주석이 이미 서 있다. 이번에 하는 것은 **그중 레일 쪽 절반**이고, 경로 이동(`/teacher/bots/new`)은
