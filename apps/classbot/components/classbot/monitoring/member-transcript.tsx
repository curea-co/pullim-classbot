 ...
 *
 * 골격은 학생 상세에 있던 목 뷰어(`students/[id]/transcript-viewer.tsx` — 2026-09-18 에 그 화면과 함께 걷혔다)에서
 * 가져왔다 — 한 줄에 시각·화자·본문, 학생 줄은 slate, 봇 줄은 blue-50. 거기서 **뺀 것**: 범위 이탈·지름길 거르개(서버
 * 원천이 없다 — 주제 이탈은 2차 · 완성 설계 § 7)와 교사 메모(저장할 문이 없다). **더한 것**: 서버 신호가 `messageId`
 * 로 가리키는 자리에 칩을 붙이고, 위 신호 목록에서 「원문 보기」로 그 자리로 뛴다. 그 목 뷰어가 하던 일은 이제 전부
 * 여기가 한다 — 교사가 한 학생의 대화를 읽는 자리는 이 컴포넌트 하나다.
 *
