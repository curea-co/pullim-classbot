 ...
 * 까닭과 되살릴 자리는 `joined-classes-data.ts` 의 `roomNameLine()` 에 적어 두었다.
 *
 * **제목은 카드 밖 `SectionHeading` 이다** — 「내 봇」·「오늘 할 일」과 같은 h2 체계.
 * 그래서 카드 본문은 제목을 되풀이하지 않고, 제목이 못 하는 말(반 이름, 선생님, 규모)만 한다.
 *
...
export function JoinedClasses({ rooms }: { rooms: RoomSlot[] }) {
  ...
  const nameLine = roomNameLine(rooms);
  ...
  const stack = rooms.slice(0, VISIBLE_ROOMS);
      ...
      >
        <span className="flex min-w-0 flex-1 items-center gap-3">
          ...
          <span className="min-w-0">
            ...
            <span className="text-pullim-slate-900 line-clamp-2 text-sm font-bold">{nameLine}</span>
            {teacherLine && (
