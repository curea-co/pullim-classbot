 ...
 *
 * ⚠ **2026-09-18 현재 이 컴포넌트를 그리는 화면이 없다.** 줄이 가리키는 수업 리플레이
 * (`/teacher/replay`)도 같은 날 빈 상태가 됐다 — 정본에 리플레이 문이 없어서다. 원천이 목
 * (`lib/mock/classbot-monitoring` 의 20명 스냅샷)이라 **다시 화면에 붙이지 마라.**
 *
 ...
 * `./roster-table` 을 쓴다 — 같이 쓰던 학생 목록·채점 허브 명단이 걷힌 지금은 이 화면이
 * 그 껍데기의 유일한 소비처다. 칸마다 되풀이하던 이름표(「지름길 6회」)는 머리글로 올리고 값만
 * 남긴다. 배지는 제 글자를 그대로 둔다 — 스스로 무엇인지 말하는 칩이라 머리글과 겹쳐도 읽는 데
 * 방해가 안 된다.
 *
 * **가운데 열은 `./roster-columns` 한 벌과 같지 않다.** 셋이 다르다:
 *   - 도달을 `s.reach` 그대로 읽는다(도달·**부분**·미도달). 그쪽은 사고 수준까지 얹은
