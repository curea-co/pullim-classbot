 ...
 *    기준은 글자 수가 아니라 **값의 자연 폭**이다. `10/10개`·`12/30개`·`99/99개` 같은
 *    `NN/NN개` 꼴(**6글자**)이 **80.11px** 이라 저 구간에서 칸을 넘는다. 5글자인
 *    `7/10개`·`1024명` 은 **67.69px**, 4글자인 `7/7개`·`128건` 은 **55.27px** 라 들어간다.
 *    한 칸이 2줄이 되면 그 바 밑선이 정확히 **30px**(값 line-height) 어긋난다.
 *    **오늘은 안 터진다**: mock 봇이 7개라 최댓값이 `7/7개`(55.27px)이고 768px 에서도
 *    14.98px 남는다. 네 자리 학생 수 `1024명` 도 넘치지 않는다.
 *    **트리거는 「운영 중」이 `NN/NN개` 가 될 때** — 즉 **운영 중인 봇이 10개 이상**일 때다
 *    (`3/10개` 처럼 앞이 한 자리면 5글자라 아직 들어간다).
 *    그때 고쳐라. `tracking-tight` 로는 안 닫힌다(78.02px, 여전히 70.25px 초과) — 저 구간에서
