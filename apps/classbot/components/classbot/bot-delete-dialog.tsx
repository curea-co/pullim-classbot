 ...
 *    화면이 없는 사실을 지어낸다.
 *  - **「있는지 없는지」는 판정 가능하다.** `TeacherBotRow.studentCount` 가 「화면에 쓰는
 *    학생 수의 권위」로 이미 조인돼 있고 바로 옆 `BotOpsCard` 가 그 값을 꺼내 쓴다.
 *
 * ⚠️ 그래서 이 고정 문구에는 **잠재 결함이 하나 있다 — 알고 두는 것이다.**
 * `getTeacherBotRows()` 는 「만들어 두고 아직 학급에 안 붙인 봇」을 `classrooms: []`
 * (→ `studentCount: 0`)으로 떨어뜨린다. 그 갈래의 봇에서는 이 판이 학생이 없는데도
 * 「학습 중인 학생들이 있어요」라고 말한다 — 즉 **학생 0명인 봇에서 이 문장은 거짓이다.**
 * 지금 당장 드러나지 않는 까닭은 고정 mock 5봇이 전부 인원을 갖고 있어서일 뿐이다.
