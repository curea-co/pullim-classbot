 ...
 * **이 문단이 패턴을 적어 둔 줄들 자신이 섞이고**(평문 `<BotAvatar` 로 찾으면 아래
 * `BotAvatarSize` 선언까지 더 붙는다 — `\b` 는 뒤에 비-단어 문자를 요구해 그 `S` 를 안 문다),
 * 뒤를 문자 하나로 좁히면(`[ />]`) **여러 줄로 쓴 JSX 세 곳은 태그 뒤가 개행**이라 빠져
 * **18** 이 된다. 하필 그 셋이 `className` 을 넘기는 유일한 자리들(ring·breath)이라, 좁힌
 * 패턴으로 감사하면 **가장 특별한 셋을 못 본다.** `$` 를 대안에 넣어야 21 이 나온다(`-E` 필요).
 *
 * 그리고 채택한 `( |/|>|$)` 는 **이 파일에서 0줄**을 문다 — 그것이 이 모양을 고른 진짜 이유다.
 * 자기를 무는 패턴은 **개수가 주석을 고칠 때마다 움직여서**, 여기 적은 18·21 이 곧 거짓이 된다.
 * (실제로 그랬다: 앞 판이 `\b` 의 값을 못 박아 뒀는데, 이 문단이 패턴을 한 번 더 적는 순간
 * 그 숫자가 스스로 하나 밀렸다. `lib/tokens/bot-signature.ts` 가 열거를 버린 것과 같은 까닭이다.)
 *
