 ...
 *
 * `grep -rEn '<BotAvatar( |/|>|$)' apps/classbot/{app,components}` 를 실제로 돌려 **21곳을
 * 하나씩 열어 확인한 결과**다. 세는 기준은 「봇을 가리키는 **글자**가 어디 있나」다.
 *
 * 패턴이 이 모양인 까닭은 **양쪽으로 틀리기 쉬워서**다. 태그 이름만 찾으면(`<BotAvatar\b`)
 * 이 파일의 `BotAvatarSize` 선언과 바로 이 문단 자신이 섞여 **22** 가 되고, 뒤를 문자 하나로
 * 좁히면(`[ />]`) **여러 줄로 쓴 JSX 세 곳은 태그 뒤가 개행**이라 빠져 **18** 이 된다.
 * 하필 그 셋이 `className` 을 넘기는 유일한 자리들(ring·breath)이라, 좁힌 패턴으로 감사하면
 * **가장 특별한 셋을 못 본다.** `$` 를 대안에 넣어야 21 이 나온다(`-E` 필요).
 *
