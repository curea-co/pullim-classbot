 ...
 *
 * 면은 **한 농도**다 — 연한 배경(`blue-50`)에 진한 글자(`blue-700`). [08 § 1.3] 이 말하는
 ...
 *
 * `grep -rn '<BotAvatar' apps/classbot/{app,components}` 를 실제로 돌려 **21곳을 하나씩 열어
 * 확인한 결과**다. 세는 기준은 「봇을 가리키는 **글자**가 어디 있나」다.
 *
 ...
 *
 * 대비도 통일하는 쪽이 낫다. `#1D3FA8` on `#EEF3FF` = **8.12:1** 로, 종전 슬레이트 면의
 * 최선(slate-600, 6.72:1)보다 높고 나머지 면과 자릿수가 같다. (슬레이트를 쓰던 동안 이 자리는
