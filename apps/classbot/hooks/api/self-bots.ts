 ...
 *
 * ## ⚠ `401` 은 폴백 조건이 **아니다**
 * 위 갈래는 「비로그인인가」로 갈리고, 비로그인은 서버를 **아예 부르지 않는다.** 그러니
 * 세션 만료·무효 토큰의 `401` 이 왔다면 그건 **신원이 있다고 보고 물어본 뒤 거절당한 것**
 * 이라 재인증·오류로 다뤄야 한다 — 그 응답을 보고 옛 로컬 기록을 정본 자리로 되돌리지
 * 마라. #283 이 이 파일에 적어 둔 규칙이고 명세가 못박는다
 * (`proc/spec/2026-06-23_classbot-dual-mode-design.md` 2026-09-04 개정 박스에서
 * `[2026-09-10 정정]` 으로 신설된 「자기주도 데이터 출처(훅·스토어)」 줄).
 */
 ...
 *
 * ## ⛔ 부르는 화면은 **아직 없다** — mock 커리큘럼에 되잇지 마라
 *
 * 이 PR 은 **출처만** 서버로 갈았다. 명세가 이 PR 의 변경 파일을 「`hooks/api/self-bots.ts` ·
 * `hooks/api/self-server.ts` · `lib/store/self-learning.ts` 이고 **화면 파일(`app/`·
 * `components/`)은 0개**」로 못박았으므로(위 명세 인용과 같은 줄), 호출부를 여기서 들이면
 * 그 줄과 어긋나고 층도 섞인다(리포 `CLAUDE.md` 최상위 MUST). `#283` 이 이 자리에
 * 「쓰는 쪽은 #273 이 들인다」고 적어 두었지만, **「무엇을 공부로 셀지」는 화면·제품
 * 결정**이라 층이 다르다 — 자기주도 대화 표면에 잇는 것은 화면 단위의 별건이다.
 *
 * 되잇지 **말아야** 하는 자리가 하나 분명히 있다: `/classbot/learn/*`(mock 공식 튜터
 * `ot_*`). `#283` 이 그 호출을 걷었고 이유가 이 PR 과 직결된다 — ① 목록을 열어 본 것은
 * 공부가 아니고, ② 그 화면들은 P5 까지 mock 위에서 도는 자리인데 **이 PR 이 그 날짜
 * 배열을 서버(`self_study_days`)로 백필**하므로, mock 활동에서 나온 날짜가 그대로
 * **실제 기록**으로 서버에 올라간다. 걷은 이유가 백필이었으니, 백필이 실제로 들어온
 * 지금은 되잇지 말아야 할 이유가 더 강해졌다.
 *
 * 그때까지 `useSelfStreak()` 은 서버가 아는 날짜만 센다 — 아직 아무 화면도 기록하지
 * 않으므로 새 사용자에게는 0 이고, 홈의 스트릭 칩은 스스로 숨는다.
 * @returns `mutate(date?)` — 날짜를 안 주면 오늘(서버의 KST)
 */
