 ...
 * 상태 코드 동반 반환(`domainFetchWithStatus` — 201/200 멱등 분기가 있는 문용).
 */
import { ApiError } from '@pullim-classbot/api-client';
import { API_BASE } from '@/lib/auth/os-sso';
import {
  domainFetch,
  domainFetchWithStatus,
...
describe('domainFetchWithStatus — 상태 코드가 뜻을 갖는 문', () => {
  it('returns { status, body } so 201(new) and 200(idempotent) can be told apart', async () => {
    fetchMock
      .mockResolvedValueOnce(csrfRes('csrf-1'))
      .mockResolvedValueOnce(domainRes(201, { membershipId: 'm1', classId: 'cls_1' }))
      .mockResolvedValueOnce(csrfRes('csrf-2'))
      .mockResolvedValueOnce(domainRes(200, { membershipId: 'm1', classId: 'cls_1' }));

    const first = await domainFetchWithStatus<{ classId: string }>('/enrollments', {
      method: 'POST',
      body: { code: 'AB3K9M' },
    });
    const second = await domainFetchWithStatus<{ classId: string }>('/enrollments', {
      method: 'POST',
      body: { code: 'AB3K9M' },
    });

    expect(first).toEqual({ status: 201, body: { membershipId: 'm1', classId: 'cls_1' } });
    expect(second.status).toBe(200);
    expect(second.body).toEqual(first.body);
  });

  it('returns null body for an empty 2xx response instead of throwing', async () => {
    fetchMock.mockResolvedValueOnce({ ok: true, status: 204, text: async () => '' } as unknown as Response);

    await expect(domainFetchWithStatus('/bots')).resolves.toEqual({ status: 204, body: null });
  });
});
