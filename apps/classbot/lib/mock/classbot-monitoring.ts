 ...
 *
 * ⛔ **이 파일을 읽는 화면이 없다(2026-09-18).** 읽던 넷 — 교사 홈 「먼저 볼 학생」(#369) · 채점 허브와
 * 리포트 센터 명단(#370) · 학생 목록과 학생 상세(이 PR) — 이 전부 빈 계정에 없는 학생 스무 명을
 * 세우고 있었다. 학급 관제소(`/teacher/monitor`)는 그보다 먼저(계획 PR 7) 정본으로 갈아탔다:
 * 명단은 `GET /classbot/classes/:id/members`, 신호는 `.../signals` 가 준다.
 *
 * **새 화면에서 읽지 마라.** 여기 담긴 열들 — 도달 상태 · 요구 수준 대비 닿은 수준 · 지름길 시도 ·
 * 다시 가르칠 개념 — 은 **서버에 원천이 아예 없다.** 정본이 주는 것은 규칙으로 잡은 신호 다섯 종이고,
 * 그것을 읽는 자리는 `lib/risk-signals.ts` 다.
 *
 * 지우지 않고 남긴 까닭: 아직 이 파일을 물고 있는 공유 컴포넌트가 다섯 있다 — `components/classbot/`
 * 의 `roster-table` · `roster-columns` · `roster-badges` · `class-reach-roster` · `reteach-concepts`.
 * 그 컴포넌트들도 소비처가 0 이라 **함께 걷는 것이 다음 단위**다 — 그때 이 파일도 같이 지운다.
 *
 * 조사 근거로 고른 지표만 담는다.
