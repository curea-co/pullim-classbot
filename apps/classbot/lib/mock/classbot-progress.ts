 ...
 *
 * ⛔ **어느 화면도 이 모듈을 읽지 않는다 (2026-09-18 부터).**
 *
 * `/classbot/me/progress` 가 여기서 읽던 「공부한 시간 4시간 20분 · 끝낸 과제 7건 · 봇과
 * 주고받음 38번 · 평균 달성도 71%」와 「8월 10일 ~ 8월 14일」은 누구의 기록도 아닌 지어낸
 * 값이었다. 학생이 제 기록이라고 읽는 자리라 화면에서 걷었고, 그 화면은 지금 빈 상태 하나로
 * 서 있다. **새 화면을 여기에 물리지 마라** — 이 모듈은 은퇴했고 목 모듈 청소 PR 이 파일째
 * 걷어 간다.
 *
 * 정본(pullim-api)에는 학생 본인의 학습 집계를 주는 문이 아직 없다. 생기면 화면이 그 훅을
 * 직접 읽는다(자리는 `app/(student)/classbot/me/progress/page.tsx` 머리주석에 적어 뒀다).
 *
