...
const repliesFormal: Record<ReplyKey, string> = {
  ...
  today_summary:
    '오늘 수능 빈칸 7유형 — 접속사 논리 관계를 진행했어요. 핵심 메시지 3개:\n' +
    '① 빈칸은 글의 주제 — 반복·재진술 키워드가 단서\n② 역접/인과/예시 접속사로 빈칸 방향 결정\n③ 단어만 같은 함정 오답은 의미 방향으로 거르기\n\n' +
    '내일은 어법·어휘로 이어가요.',
  exam_prep:
    '수능까지 보면 빈칸 추론은 매일 2지문이 안정적이에요. 오답정복 큐를 보니 5~7번 접속사 유형에서 막히는 비율이 높던데, 정복 세트 1개 + 단락 흐름 도식 5분이면 잡힐 거 같아요. ' +
    '플래너에 자동 추가해드릴까요?',
  reassurance:
    '서연 학생, 점검해볼게요. 1) 영어 빈칸 추론 정답률이 지난 모의 대비 +14p 올랐어요. 2) 17일 연속 학습도 우리 반 상위권이에요. ' +
    '접속사 논리만 더 굳히면 다음 모의 안정권이에요. 지금 페이스가 정답이에요.',
};
...
const repliesSpartan: Record<ReplyKey, string> = {
  ...
  today_summary:
    '오늘 통합과학 전기회로 진행했다. 외울 것:\n' +
    '① V=IR — 변형 세 형태 다 외운다\n② 직렬 전류 같다 / 병렬 전압 같다\n③ 합성저항은 안쪽 병렬부터 묶는다\n\n' +
    '내일은 자기장. 예습해라.',
  exam_prep:
    '내신까지 시간 없다. 오답정복 큐에 합성저항 계산이 막혔다. 정복 세트 1개 + 회로 직접 그리기 5분, 그게 답이다. ' +
    '플래너에 자동 추가해두겠다.',
  reassurance:
    '서연. 통합과학 전기회로 정답률 올라온 거 확인했다. 17일 연속도 잘 버텼다. ' +
    '합성저항 한 가지만 더 잡으면 내신 안정권. 페이스 유지해라.',
};
