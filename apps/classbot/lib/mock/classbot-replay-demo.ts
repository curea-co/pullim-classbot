 ...
 * rp_demo_math는 1920s(그래프 그리기, ownedByMe)에 집중 저하 1건도 포함(다시보기/질문만).
 */
import type {
...
const mathSegments: ReplaySegment[] = [
  { at: '0:00', atSec: 0, ratio: 0, type: 'concept', label: '일차함수의 뜻' },
  { at: '18:20', atSec: 1100, ratio: 1100 / 2100, type: 'quiz', label: 'Q3 기울기 구하기', ownedByMe: true, myAnswer: '1/2', correctAnswer: '2' },
  { at: '32:00', atSec: 1920, ratio: 1920 / 2100, type: 'concept', label: '그래프 그리기', ownedByMe: true },
];
// 35분 → 분당 빈, 32분(그래프 그리기)에서 집중 저하
const mathFocus = Array.from({ length: 35 }, (_, i) => (i === 32 ? 28 : 72 + ((i * 7) % 18)));

const engSegments: ReplaySegment[] = [
  { at: '0:00', atSec: 0, ratio: 0, type: 'concept', label: '문장 흐름 잡기' },
  { at: '12:20', atSec: 740, ratio: 740 / 900, type: 'quiz', label: 'Q2 이어질 말 고르기', ownedByMe: true, myAnswer: 'faster', correctAnswer: 'more meaningful' },
];
const engFocus = Array.from({ length: 15 }, (_, i) => 74 + ((i * 5) % 16));

const mathTranscript: TranscriptLine[] = [
  { at: '0:00', atSec: 0, endSec: 1100, speaker: '교사', text: '오늘은 두 점으로 기울기를 구하는 법을 볼게요.' },
  { at: '18:20', atSec: 1100, endSec: 1920, speaker: '봇', text: 'Q3 — 두 점의 변화량을 보고 기울기를 골라봐요.' },
  { at: '32:00', atSec: 1920, endSec: 2100, speaker: '교사', text: '그래프는 y절편을 찍고 기울기로 점을 하나 더 잡으면 그려져요.' },
];
const engTranscript: TranscriptLine[] = [
  { at: '0:00', atSec: 0, endSec: 740, speaker: '교사', text: '문장 흐름은 글의 논리 흐름을 먼저 잡는 게 핵심이에요.' },
  { at: '12:20', atSec: 740, endSec: 900, speaker: '봇', text: 'Q2 — 대조 구조를 보고 빈칸에 들어갈 말을 골라봐요.' },
];

export const demoReplays: Replay[] = [
  makeDemoReplay({
    id: 'rp_demo_math', botId: 'cb_001', botName: '수학봇',
    title: '일차함수 · 기울기', chapter: '중2 수학',
    durationMin: 35, segments: mathSegments, focusBins: mathFocus,
    transcript: mathTranscript, myAccuracy: 67,
    keyTakeaways: [
      '기울기는 (y의 변화량) ÷ (x의 변화량)으로 구한다',
      '나누는 순서를 뒤집으면 값이 뒤집힌다 — y가 위, x가 아래',
      'y절편을 찍고 기울기로 점을 하나 더 잡으면 그래프가 그려진다',
    ],
  }),
  makeDemoReplay({
    id: 'rp_demo_eng', botId: 'cb_002', botName: '영어봇',
    title: '문장 흐름 집중', chapter: '중3 영어 · 읽기',
    durationMin: 15, segments: engSegments, focusBins: engFocus,
    transcript: engTranscript, myAccuracy: 75,
    keyTakeaways: [
      '빈칸 앞뒤의 대조·인과 연결어를 먼저 찾는다',
