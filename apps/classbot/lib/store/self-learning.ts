 ...
 *
 * ## ⛔ P4 에게 — `bots` 는 **절반만** 찌꺼기다. 통째로 지우지 마라
 *
 * 같은 칸이 사람에 따라 뜻이 다르다. 데이터만 봐서는 구별되지 않고, **가르는 것은
 * `botsMigratedUserIds` 다**:
 *
 * | `botsMigratedUserIds` 에 그 id 가 | 그 통의 `bots` 는 | P4 가 |
 * |---|---|---|
 * | **있다** | 서버로 이미 올라간 **찌꺼기** | 지워도 된다 |
 * | **없다** | 서버를 만난 적 없는 **공개 데모의 살아 있는 데이터** | **남겨야 한다** |
 *
 * 아래가 왜 눈으로 구별이 안 되는지다: 공개 데모 방문자는 신원이 없어 데모 폴백
 * `student_001` 이 된다(`lib/current-user.ts`). 그래서 `byUser['student_001'].bots` 가
 * **개발 쿠키로 로그인해 이관을 마친 서연의 찌꺼기**일 수도, **prod 에서 봇을 담아 둔
 * 익명 방문자의 유일한 사본**일 수도 있다. id 로는 못 가른다 — 완료 표시로만 가른다.
 *
 * `byUser[].bots` 를 한 번에 비우면 **prod 공개 데모의 담은 봇이 전부 사라진다.**
 * 정리는 `botsMigratedUserIds` 에 있는 id 의 통에서만 한다.
 * (`studyDays` 는 이 갈래가 없다 — 신원과 무관하게 아직 전부 로컬이고, P4 가 옮기는 대상
 * 자체다. 데모의 `studyDays` 를 어떻게 할지는 P4 가 담은 봇에서 내린 것과 같은 판단을
 * 하게 될 텐데, 그건 P4 의 결정이라 여기서 미리 적지 않는다.)
 *
...
export interface SelfUserRecord {
   ...
   * 소스이자 안전망으로 **지우지 않고 둔다**(위 머리주석).
   *
   * ⛔ 이 칸을 정리할 때 **통째로 비우지 마라** — 절반은 아직 살아 있는 데모 데이터다.
   * 무엇을 지워도 되는지는 위 머리주석 「P4 에게」가 `botsMigratedUserIds` 로 가른다.
   */
  bots: SelfBotRow[];
